...
const ShuttleControlSharedShuttleStatus = (props, context) => {
  ...
  const { shuttle_status, shuttle_state, has_docking, docking_status, docking_override, docking_codes } = data;
  return (
...
const ShuttleControlSharedShuttleControls = (props, context) => {
  ...
  return (
    <Section title="Controls">
      <Flex spacing={1}>
        <Flex.Item grow={1}>
          <Button onClick={() => act('move')} disabled={!can_launch} icon="rocket" fluid>
            Launch Shuttle
          </Button>
        </Flex.Item>
        <Flex.Item grow={1}>
          <Button onClick={() => act('cancel')} disabled={!can_cancel} icon="ban" fluid>
            Cancel Launch
          </Button>
        </Flex.Item>
        <Flex.Item grow={1}>
          <Button onClick={() => act('force')} color="bad" disabled={!can_force} icon="exclamation-triangle" fluid>
            Force Launch
          </Button>
        </Flex.Item>
      </Flex>
      ...
      <Section title="Multishuttle Controls">
        <LabeledList>
          ...
          <LabeledList.Item label="Current Destination">
            <Button icon="taxi" disabled={!can_pick} onClick={() => act('pick')}>
              {destination_name}
            </Button>
          </LabeledList.Item>
        </LabeledList>
...
const ShuttleControlConsoleExploration = (props, context) => {
  const { act, data } = useBackend(context);
  const { can_pick, destination_name, fuel_usage, fuel_span, remaining_fuel } = data;
  return (
    <Fragment>
        ...
        <LabeledList>
          ...
          {(fuel_usage && (
            <Fragment>
              ...
              <LabeledList.Item label="Avg. Delta-V Per Maneuver">{fuel_usage} m/s</LabeledList.Item>
            </Fragment>
          )) ||
            null}
        </LabeledList>
              ...
              {(routes.length &&
                routes.map((route) => (
                  <LabeledList.Item label={route.name} key={route.name}>
                    <Button icon="rocket" onClick={() => act('traverse', { traverse: route.index })}>
                      {route.travel_time}
                    </Button>
                  </LabeledList.Item>
                ))) || (
        ...
        <Section title="Transit ETA">
          <LabeledList>
            <LabeledList.Item label="Distance from target">
              <ProgressBar color="good" minValue={0} maxValue={100} value={travel_progress}>
                {time_left}s
              </ProgressBar>
            </LabeledList.Item>
          </LabeledList>
          ...
          <LabeledList>
            {Object.keys(sensors).map((key) => {
              ...
              return (
                <LabeledList.Item label={key} key={key}>
                  <LabeledList>
                    <LabeledList.Item label="Pressure">{sensor.pressure}kPa</LabeledList.Item>
