...
/datum/element/slosh/proc/handle_footstep(mob/living/source)
	...
	if(ishuman(source))
		...
		source.vore_footstep_volume_cooldown = 0
	if(issilicon(source))
		if(step_count++ % 2)
			choose_vorefootstep(source)
			source.vore_footstep_volume_cooldown = 0
...
/datum/element/slosh/proc/choose_vorefootstep(mob/living/source)
	if(source.vore_footstep_volume_cooldown++ >= 5)

		source.vore_organs_reagents = list()
		var/highest_vol = 0

		for(var/obj/belly/B in source.vore_organs)
			var/total_volume = B.reagents.total_volume
			source.vore_organs_reagents += total_volume

			if(B.show_liquids && B.vorefootsteps_sounds && highest_vol < total_volume)
				highest_vol = total_volume

		if(highest_vol < 20)
			source.vore_footstep_volume = 0
			source.vore_footstep_chance = 0
		else
			source.vore_footstep_volume = 20 + highest_vol * 4/5
			source.vore_footstep_chance = highest_vol/4

		source.vore_footstep_volume_cooldown = 0

	if(!source.vore_footstep_volume || !source.vore_footstep_chance)
		return
...
/datum/element/slosh/proc/handle_vorefootstep(mob/living/source)
	if(!CONFIG_GET(number/vorefootstep_volume) || !source.vore_footstep_volume)
		return

	var/S = pick(GLOB.slosh)
	if(!S) return
	var/volume = CONFIG_GET(number/vorefootstep_volume) * (source.vore_footstep_volume/100)

	if(ishuman(source))
		var/mob/living/carbon/human/human_source = source

		if(!human_source.shoes || human_source.m_intent == "walk")
			volume = CONFIG_GET(number/vorefootstep_volume) * (source.vore_footstep_volume/100) * 0.75
		else if(human_source.shoes)
			var/obj/item/clothing/shoes/feet = human_source.shoes
			if(istype(feet))
				volume = feet.step_volume_mod * CONFIG_GET(number/vorefootstep_volume) * (source.vore_footstep_volume/100) * 0.75
