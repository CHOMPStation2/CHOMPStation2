...
/obj/machinery/chemical_synthesizer
	...
	var/obj/item/reagent_containers/glass/catalyst = null // This is where the user adds catalyst. Usually phoron. // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/chemical_synthesizer/Initialize()
	...
	create_reagents(600)
	catalyst = new /obj/item/reagent_containers/glass/beaker(src) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/chemical_synthesizer/proc/add_cartridge(obj/item/reagent_containers/chem_disp_cartridge/C, mob/user) // CHOMPEdit - Removal of obj/item/weapon
	if(!panel_open)
		if(user)
...
/obj/machinery/chemical_synthesizer/attackby(obj/item/W, mob/user) // CHOMPEdit - Removal of obj/item/weapon
	// Why do so many people code in wrenching when there's already a proc for it?
	if(!busy && default_unfasten_wrench(user, W, 40))
		return

	if(istype(W, /obj/item/reagent_containers/chem_disp_cartridge)) // CHOMPEdit - Removal of obj/item/weapon
	...
	if(panel_open && W.is_screwdriver())
		...
		if(!label)
			return
		var/obj/item/reagent_containers/chem_disp_cartridge/C = remove_cartridge(label) // CHOMPEdit - Removal of obj/item/weapon
		if(C)
	...
	if(istype(W, /obj/item/reagent_containers/glass)) // CHOMPEdit - Removal of obj/item/weapon
		if(catalyst)
		...
		var/obj/item/reagent_containers/RC = W // CHOMPEdit - Removal of obj/item/weapon
	...
	if(--process_tick <= 0)
		...
		for(var/id in dispense_reagents)
			...
			if(!R)
				...
				continue
			var/obj/item/reagent_containers/chem_disp_cartridge/C = cartridges[R.name] // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/chemical_synthesizer/tgui_data(mob/user)
	...
	for(var/label in cartridges)
		var/obj/item/reagent_containers/chem_disp_cartridge/C = cartridges[label] // CHOMPEdit - Removal of obj/item/weapon
	...
	switch(drug_substance)
		if(2) // Pills
			while(reagents.total_volume)
				var/obj/item/reagent_containers/pill/P= new(src.loc) // CHOMPEdit - Removal of obj/item/weapon
				P.name = "[r_id]"
		...
		if(3) // Patches
			while(reagents.total_volume)
				var/obj/item/reagent_containers/pill/patch/P= new(src.loc) // CHOMPEdit - Removal of obj/item/weapon
				P.name = "[r_id]"
				P.pixel_x = rand(-7, 7) // random position
				P.pixel_y = rand(-7, 7)
			...
			while(reagents.total_volume)
				var/obj/item/reagent_containers/glass/bottle/B = new(src.loc) // CHOMPEdit - Removal of obj/item/weapon
				B.name = "[r_id] bottle"
