/*
 proteans
*/
/obj/item/rig/protean // CHOMPEdit - Removal of obj/item/weapon
	...
	open = 1
	cell_type =  /obj/item/cell/protean // CHOMPEdit - Removal of obj/item/weapon
	var/dead = 0
...
/obj/item/rig/protean/relaymove(mob/user, var/direction) // CHOMPEdit - Removal of obj/item/weapon
	if(user.stat || user.stunned)
		return
	forced_move(direction, user, 0)

/obj/item/rig/protean/check_suit_access(mob/living/user) // CHOMPEdit - Removal of obj/item/weapon
	if(user == myprotean)
		return 1
	return ..()

/obj/item/rig/protean/digest_act(atom/movable/item_storage = null) // CHOMPEdit - Removal of obj/item/weapon
	return 0

/obj/item/rig/protean/ex_act(severity) // CHOMPEdit - Removal of obj/item/weapon
	return

/obj/item/rig/protean/New(var/newloc, var/mob/living/carbon/human/P) // CHOMPEdit - Removal of obj/item/weapon
	if(P)
...
/obj/item/rig/protean/Destroy() // CHOMPEdit - Removal of obj/item/weapon
	if(myprotean)
...
/obj/item/rig/proc/AssimilateBag(var/mob/living/carbon/human/P, var/spawned, var/obj/item/storage/backpack/B) // CHOMPEdit - Removal of obj/item/weapon
	if(istype(B,/obj/item/storage/backpack)) // CHOMPEdit - Removal of obj/item/weapon
		if(spawned)
			B = P.back
			P.unEquip(P.back)
...
/obj/item/rig/protean/verb/RemoveBag() // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/rig/protean/attack_hand(mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if (src.loc == user)
		if(rig_storage)
...
/obj/item/rig/protean/attackby(obj/item/W as obj, mob/living/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if(!istype(user))
		return 0
	if(dead)
		switch(dead)
			if(1)
				...
				return
			if(2)
				if(istype(W, /obj/item/protean_reboot))//placeholder // CHOMPEdit - Removal of obj/item/device
					if(do_after(user,50,src,exclusive = TASK_ALL_EXCLUSIVE))
			...
			if(3)
				...
				return
			if(4)
				if(istype(W, /obj/item/shockpaddles)) // CHOMPEdit - Removal of obj/item/weapon
					if(W?:can_use(user))
				...
				return
	if(istype(W,/obj/item/rig)) // CHOMPEdit - Removal of obj/item/weapon
		if(!assimilated_rig)
			AssimilateRig(user,W)
	if(istype(W,/obj/item/tank)) //Todo, some kind of check for suits without integrated air supplies. // CHOMPEdit - Removal of obj/item/weapon
		if(air_supply)
	...
	for(var/obj/item/rig_module/module in installed_modules)
		if(module.accepts_item(W,user)) //Item is handled in this proc
			return
	if(rig_storage)
		var/obj/item/storage/backpack = rig_storage // CHOMPEdit - Removal of obj/item/weapon
		if(backpack.can_be_inserted(W, 1))
			backpack.handle_item_insertion(W)
	else
		if(istype(W,/obj/item/storage/backpack)) // CHOMPEdit - Removal of obj/item/weapon
			AssimilateBag(user,0,W)
	..()

/obj/item/rig/protean/proc/make_alive(var/mob/living/carbon/human/H, var/partial) // CHOMPEdit - Removal of obj/item/weapon
	if(H)
		H.setToxLoss(0)
		H.setOxyLoss(0)
		H.setCloneLoss(0)
...
/obj/item/rig/protean/take_hit(damage, source, is_emp=0) // CHOMPEdit - Removal of obj/item/weapon
	return	//We don't do that here

/obj/item/rig/protean/emp_act(severity_class) // CHOMPEdit - Removal of obj/item/weapon
	return	//Same here

/obj/item/rig/protean/cut_suit() // CHOMPEdit - Removal of obj/item/weapon
	return	//nope

/obj/item/rig/protean/force_rest(var/mob/user) // CHOMPEdit - Removal of obj/item/weapon
	wearer.lay_down()
	to_chat(user, "<span class='notice'>\The [wearer] is now [wearer.resting ? "resting" : "getting up"].</span>")

/obj/item/cell/protean // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/cell/protean/Initialize() //ChompEDIT New --> Initialize // CHOMPEdit - Removal of obj/item/weapon
	charge = maxcharge
	update_icon()
	addtimer(CALLBACK(src, PROC_REF(search_for_protean)), 60)

/obj/item/cell/protean/proc/search_for_protean() // CHOMPEdit - Removal of obj/item/weapon
	if(istype(src.loc, /obj/item/rig/protean)) // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/rig/protean/prig = src.loc // CHOMPEdit - Removal of obj/item/weapon
		charger = prig.wearer
	if(charger)
		START_PROCESSING(SSobj, src)

/obj/item/cell/protean/process() // CHOMPEdit - Removal of obj/item/weapon
	var/C = charge
	if(charger)
...
/obj/item/rig/protean/equipped(mob/living/carbon/human/M) // CHOMPEdit - Removal of obj/item/weapon
	..()
	if(dead)
		canremove = 1
	else
...
/obj/item/rig/protean/ai_can_move_suit(mob/user, check_user_module = 0, check_for_ai = 0) // CHOMPEdit - Removal of obj/item/weapon
	if(check_for_ai)
...
/obj/item/rig/protean/toggle_seals(mob/living/carbon/human/M, instant) // CHOMPEdit - Removal of obj/item/weapon
	M = src.wearer
	..()

/obj/item/rig/protean/toggle_cooling(mob/user) // CHOMPEdit - Removal of obj/item/weapon
	user = src.wearer
	..()

/obj/item/rig/protean/toggle_piece(piece, mob/living/carbon/human/H, deploy_mode, forced) // CHOMPEdit - Removal of obj/item/weapon
	H = src.wearer
	..()

/obj/item/rig/protean/get_description_interaction() // CHOMPEdit - Removal of obj/item/weapon
	if(dead)
...
/obj/item/rig/protean/proc/AssimilateRig(mob/user, var/obj/item/rig/R) // CHOMPEdit - Removal of obj/item/weapon
	if(!R || assimilated_rig)
		return
	if(istype(R, /obj/item/rig/protean)) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/rig/protean/verb/RemoveRig() // CHOMPEdit - Removal of obj/item/weapon
	...
	if(assimilated_rig)
		...
		var/obj/item/rig/tempRig = new /obj/item/rig/protean() // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/rig/protean/MouseDrop(obj/over_object as obj) // CHOMPEdit - Removal of obj/item/weapon
	if(!canremove)
		return
