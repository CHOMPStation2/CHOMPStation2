...
/mob/living/simple_mob/protean_blob
	...
	shock_resist = 1
	nameset = 1
	holder_type = /obj/item/holder/protoblob // CHOMPEdit - Removal of obj/item/weapon
	var/hiding = 0
	vore_icons = 1
	vore_active = 1
...
/mob/living/simple_mob/protean_blob/rad_act(severity)
	if(istype(loc, /obj/item/rig)) // CHOMPEdit - Removal of obj/item/weapon
...
/mob/living/carbon/human/proc/nano_intoblob(force)
	if(!force && !isturf(loc) && !loc == /obj/item/rig/protean) // CHOMPEdit - Removal of obj/item/weapon
	...
	if(force || do_after(src,20,exclusive = TASK_ALL_EXCLUSIVE))
		...
		temporary_form = blob
		var/obj/item/radio/R = null // CHOMPEdit - Removal of obj/item/device
		if(isradio(l_ear))
			R = l_ear
		if(isradio(r_ear))
			R = r_ear
...
/mob/living/proc/usehardsuit()
	...
	if(istype(loc, /obj/item/rig/protean)) // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/rig/protean/prig = loc // CHOMPEdit - Removal of obj/item/weapon
...
/mob/living/carbon/human/proc/nano_outofblob(var/mob/living/simple_mob/protean_blob/blob, force)
	if(!istype(blob))
		return
	if(blob.loc == /obj/item/rig/protean) // CHOMPEdit - Removal of obj/item/weapon
		return
