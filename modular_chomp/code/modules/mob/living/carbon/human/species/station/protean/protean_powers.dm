	...
	if(input == "Rebuild")
		...
		if(do_after(caller, 40,exclusive = TASK_ALL_EXCLUSIVE))
			if(caller.client.prefs)	//Make sure we didn't d/c
				var/obj/item/rig/protean/Rig = species?:OurRig // CHOMPEdit - Removal of obj/item/weapon
...
/mob/living/carbon/human/proc/nano_copy_body()
	...
	for(var/obj/item/grab/G in caller) // CHOMPEdit - Removal of obj/item/weapon
	...
	if(forced || do_after(caller,20,exclusive = TASK_ALL_EXCLUSIVE))
		...
		if(istype(src.species, /datum/species/protean))
			...
			if(S.OurRig) //Do we even have a RIG?
				...
				new /obj/item/rig/protean(src,src) // CHOMPEdit - Removal of obj/item/weapon
	else
...
/mob/living/carbon/human/proc/appearance_switch()
	...
	if(temporary_form)
		if(blobstyle)
			...
			if(istype(temporary_form.loc, /obj/item/holder/protoblob)) // CHOMPEdit - Removal of obj/item/weapon
				var/obj/item/holder/protoblob/PB = temporary_form.loc // CHOMPEdit - Removal of obj/item/weapon
