...
/mob/living/simple_mob/vore/aggressive/corrupthound/swoopie
	...
	var/obj/item/vac_attachment/swoopie/Vac // CHOMPEdit - Removal of obj/item/device
...
/mob/living/simple_mob/vore/aggressive/corrupthound/swoopie/Initialize()
	...
	if(!voremob_loaded)
		voremob_loaded = TRUE
		init_vore()
	Vac = new /obj/item/vac_attachment/swoopie(src) // CHOMPEdit - Removal of obj/item/device
	if(istype(Vac))
...
/mob/living/simple_mob/vore/aggressive/corrupthound/swoopie/ClickOn(var/atom/A, var/params)
	...
	if(stat) //Cant suck if we're not able to...
		return ..()
	if(istype(A, /obj/item/storage)) //Dont put the nossle in bags // CHOMPEdit - Removal of obj/item/weapon
		return ..()
...
/obj/item/vac_attachment/swoopie // CHOMPEdit - Removal of obj/item/device
...
/obj/item/vac_attachment/swoopie/dropped(mob/user) //This should fix it sitting on the ground until the next life() tick // CHOMPEdit - Removal of obj/item/device
	. = ..()
	if(!vac_owner)
		return
	forceMove(vac_owner)
