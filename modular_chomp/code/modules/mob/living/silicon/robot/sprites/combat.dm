...
/datum/robot_sprite/dogborg/wide/combat/handle_extra_icon_updates(var/mob/living/silicon/robot/ourborg)
	if(has_gun_sprite && (istype(ourborg.module_active, /obj/item/gun/energy/laser/mounted) || istype(ourborg.module_active, /obj/item/gun/energy/lasercannon/mounted))) // CHOMPEdit - Removal of obj/item/weapon
		ourborg.add_overlay("[sprite_icon_state]-laser")
	if (has_taser_sprite && (istype(ourborg.module_active, /obj/item/gun/energy/taser/mounted/cyborg/ertgun))) // CHOMPEdit - Removal of obj/item/weapon
		ourborg.add_overlay("[sprite_icon_state]-taser")
	if (has_blade_sprite && (istype(ourborg.module_active, /obj/item/melee/combat_borgblade))) // CHOMPEdit - Removal of obj/item/weapon
		ourborg.add_overlay("[sprite_icon_state]-blade")

/datum/robot_sprite/dogborg/wide/combat/blade/do_equipment_glamour(var/obj/item/robot_module/module) // CHOMPEdit - Removal of obj/item/weapon
	if(!has_custom_equipment_sprites)
		return

	..()

	var/obj/item/melee/combat_borgblade/CBB = locate() in module.modules // CHOMPEdit - Removal of obj/item/weapon
	if(CBB)
		CBB.name = "combat saw"
		CBB.desc = "A high frequency blade attached to the end of a cyborg's tail. It appears to be extremely sharp."
	var/obj/item/melee/borg_combat_shocker/BCS = locate() in module.modules // CHOMPEdit - Removal of obj/item/weapon
	if(BCS)
