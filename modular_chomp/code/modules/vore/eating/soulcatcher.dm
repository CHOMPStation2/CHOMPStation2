...
/obj/soulgem/proc/find_transfer_objects()
	...
	if(isrobot(owner))
		...
		if(istype(R.module_active, /obj/item/sleevemate)) // CHOMPEdit - Removal of obj/item/device
...
/obj/soulgem/proc/transfer_mob(var/mob/M, var/obj/target)
	...
	if(!M || !target) return
	if(istype(target, /obj/item/sleevemate)) // CHOMPEdit - Removal of obj/item/device
		var/obj/item/sleevemate/mate = target // CHOMPEdit - Removal of obj/item/device
		if(!mate.stored_mind)
			...
			if(M.mind == own_mind)
				own_mind = null
			mate.get_mind(M)
	else if(istype(target, /obj/item/mmi)) // CHOMPEdit - Removal of obj/item/device
		var/obj/item/mmi/mm = target // CHOMPEdit - Removal of obj/item/device
