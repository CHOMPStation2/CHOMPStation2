...
/obj/item/walkpod // CHOMPEdit - Removal of obj/item/device
	...
	var/obj/item/headpods/deployed_headpods // CHOMPEdit - Removal of obj/item/device

	w_class = ITEMSIZE_COST_SMALL
	slot_flags = SLOT_BELT

/obj/item/walkpod/Destroy() // CHOMPEdit - Removal of obj/item/device
	remove_listener()
	return ..()

// Icon
/obj/item/walkpod/update_icon() // CHOMPEdit - Removal of obj/item/device
	if(listener)
...
// Listener handling
/obj/item/walkpod/proc/check_listener() // CHOMPEdit - Removal of obj/item/device
	if(loc == listener)
		return TRUE
	return FALSE

/obj/item/walkpod/proc/remove_listener() // CHOMPEdit - Removal of obj/item/device
	if(playing)
		StopPlaying()
...
/obj/item/walkpod/proc/set_listener(mob/living/L) // CHOMPEdit - Removal of obj/item/device
	if(listener)
		remove_listener()
	listener = L
...
/obj/item/walkpod/proc/update_music() // CHOMPEdit - Removal of obj/item/device
...
/obj/item/walkpod/AltClick(mob/living/L) // CHOMPEdit - Removal of obj/item/device
...
/obj/item/walkpod/attack_self(mob/living/L) // CHOMPEdit - Removal of obj/item/device
	if(!istype(L) || loc != L)
		return
	if(!listener)
		set_listener(L)
...
/obj/item/walkpod/process() // CHOMPEdit - Removal of obj/item/device
	if(!check_headpods())
		restore_headpods()
	if(!check_listener())
		remove_listener()
...
/obj/item/walkpod/proc/start_stop_song() // CHOMPEdit - Removal of obj/item/device
...
/obj/item/walkpod/proc/StopPlaying() // CHOMPEdit - Removal of obj/item/device
	playing = 0
	start_stop_song()

/obj/item/walkpod/proc/StartPlaying() // CHOMPEdit - Removal of obj/item/device
	if(!current_track)
		return
	playing = 1
	start_stop_song()
...
/obj/item/walkpod/proc/NextTrack() // CHOMPEdit - Removal of obj/item/device
	var/list/tracks = getTracksList()
...
/obj/item/walkpod/proc/PrevTrack() // CHOMPEdit - Removal of obj/item/device
	var/list/tracks = getTracksList()
	if(!tracks.len) return
	var/curTrackIndex = max(1, tracks.Find(current_track))
...
// UI
/obj/item/walkpod/proc/getTracksList() // CHOMPEdit - Removal of obj/item/device
	return SSmedia_tracks.jukebox_tracks

/obj/item/walkpod/tgui_interact(mob/user, datum/tgui/ui) // CHOMPEdit - Removal of obj/item/device
	ui = SStgui.try_update_ui(user, src, ui)
	if(!ui)
		ui = new(user, src, "Jukebox", "PodZu Music Player")
		ui.open()

/obj/item/walkpod/tgui_data(mob/user, datum/tgui/ui, datum/tgui_state/state) // CHOMPEdit - Removal of obj/item/device
	var/list/data = ..()
...
/obj/item/walkpod/tgui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state) // CHOMPEdit - Removal of obj/item/device
	if(..())
		return TRUE
...
// Silly verb
/obj/item/walkpod/verb/take_headpods() // CHOMPEdit - Removal of obj/item/device
...
/obj/item/walkpod/attackby(obj/item/W, mob/user) // CHOMPEdit - Removal of obj/item/device
	if(W == deployed_headpods)
		restore_headpods(user)
		return
	return ..()

/obj/item/walkpod/proc/restore_headpods(mob/living/potential_holder) // CHOMPEdit - Removal of obj/item/device
	if(!deployed_headpods)
		return
...
/obj/item/walkpod/proc/check_headpods() // CHOMPEdit - Removal of obj/item/device
	if(deployed_headpods && deployed_headpods.loc != loc)
		return FALSE
	return TRUE

/obj/item/headpods // CHOMPEdit - Removal of obj/item/device
