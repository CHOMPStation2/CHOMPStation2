...
/datum/shuttle/autodock/overmap/proc/try_consume_fuel() //returns 1 if sucessful, returns 0 if error (like insufficient fuel)
	...
	var/list/obj/item/tank/fuel_tanks = list() // CHOMPEdit - Removal of obj/item/weapon
	for(var/obj/structure/FP in fuel_ports) //loop through fuel ports and assemble list of all fuel tanks
		var/obj/item/tank/FT = locate() in FP // CHOMPEdit - Removal of obj/item/weapon
		if(FT)
			fuel_tanks += FT
	if(!fuel_tanks.len)
		return 0 //can't launch if you have no fuel TANKS in the ports
	var/total_flammable_gas_moles = 0
	for(var/obj/item/tank/FT in fuel_tanks) // CHOMPEdit - Removal of obj/item/weapon
	...
	for(var/obj/item/tank/FT in fuel_tanks) //loop through tanks, consume their fuel one by one // CHOMPEdit - Removal of obj/item/weapon
...
/obj/structure/fuel_port
	...
	var/opened = 0
	var/parent_shuttle
	var/base_tank = /obj/item/tank/phoron // CHOMPEdit - Removal of obj/item/weapon
...
/obj/structure/fuel_port/heavy
	base_tank = /obj/item/tank/phoron/pressurized // CHOMPEdit - Removal of obj/item/weapon
...
/obj/structure/fuel_port/attackby(obj/item/W as obj, mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
