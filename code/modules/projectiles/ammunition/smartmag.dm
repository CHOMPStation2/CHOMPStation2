...
/obj/item/ammo_magazine/smart
	...
	var/obj/item/gun/holding_gun = null	// What gun are we in, if any? // CHOMPEdit - Removal of obj/item/weapon

	var/obj/item/cell/device/attached_cell = null	// What cell are we using, if any? // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/ammo_magazine/smart/process()
	if(!holding_gun)	// Yes, this is awful, sorry. Don't know a better way to figure out if we've been moved into or out of a gun.
		if(istype(src.loc, /obj/item/gun)) // CHOMPEdit - Removal of obj/item/weapon
			holding_gun = src.loc
...
/obj/item/ammo_magazine/smart/attackby(var/obj/item/I as obj, mob/user)
	if(istype(I, /obj/item/cell/device)) // CHOMPEdit - Removal of obj/item/weapon
		if(attached_cell)
