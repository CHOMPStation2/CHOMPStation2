...
/obj/item/projectile/energy/hook/proc/ranged_disarm(var/mob/living/carbon/human/H)
	if(istype(H))
		...
		for(var/obj/item/gun/W in holding)	// Guns are complex devices, both of a mechanical and electronic nature. A weird gravity ball or other type of object trying to pull or grab it is likely not safe. // CHOMPEdit - Removal of obj/item/weapon
			if(W && prob(holding[W]))
				var/list/turfs = list()
