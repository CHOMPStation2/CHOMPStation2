...
/obj/item/coilgun_assembly // CHOMPEdit - Removal of obj/item/weapon
	name = "coilgun stock"
...
/obj/item/coilgun_assembly/attackby(var/obj/item/thing, var/mob/user) // CHOMPEdit - Removal of obj/item/weapon
	...
	if(istype(thing, /obj/item/tape_roll) && construction_stage == 2) // CHOMPEdit - Removal of obj/item/weapon
	...
	if(thing.has_tool_quality(TOOL_WELDER) && construction_stage == 4)
		var/obj/item/weldingtool/welder = thing.get_welder() // CHOMPEdit - Removal of obj/item/weapon
	...
	if(istype(thing, /obj/item/smes_coil) && construction_stage >= 6 && construction_stage <= 8) // CHOMPEdit - Removal of obj/item/weapon
	...
	if(thing.has_tool_quality(TOOL_SCREWDRIVER) && construction_stage >= 9)
		...
		var/obj/item/gun/magnetic/coilgun = new(loc) // CHOMPEdit - Removal of obj/item/weapon
		var/put_in_hands
		var/mob/M = src.loc
		if(istype(M))
			put_in_hands = M == user
...
/obj/item/coilgun_assembly/proc/increment_construction_stage() // CHOMPEdit - Removal of obj/item/weapon
	if(construction_stage < 9)
		construction_stage++
	icon_state = "coilgun_construction_[construction_stage]"

/obj/item/coilgun_assembly/examine(var/mob/user) // CHOMPEdit - Removal of obj/item/weapon
	. = ..()
