 ...
 * The gun itself
 */
/obj/item/gun/projectile/smartgun // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/gun/projectile/smartgun/make_worn_icon(body_type, slot_name, inhands, default_icon, default_layer, icon/clip_mask) // CHOMPEdit - Removal of obj/item/weapon
	var/image/I = ..()
	if(I)
		I.pixel_x = -16
	return I

/obj/item/gun/projectile/smartgun/loaded // CHOMPEdit - Removal of obj/item/weapon
	magazine_type = /obj/item/ammo_magazine/smartgun

/obj/item/gun/projectile/smartgun/Initialize() // CHOMPEdit - Removal of obj/item/weapon
	. = ..()
	if(!mag_underlay)
		mag_underlay = mutable_appearance(icon, icon_state = "smartgun_mag")

/obj/item/gun/projectile/smartgun/consume_next_projectile() // CHOMPEdit - Removal of obj/item/weapon
	if(!closed)
		return null
	return ..()

/obj/item/gun/projectile/smartgun/load_ammo(var/obj/item/A, mob/user) // CHOMPEdit - Removal of obj/item/weapon
	if(closed)
		to_chat(user, "<span class='warning'>[src] can't be loaded until you un-ready it. (Alt-click)</span>")
		return
	return ..()

/obj/item/gun/projectile/smartgun/unload_ammo(mob/user, var/allow_dump=0) // CHOMPEdit - Removal of obj/item/weapon
	if(closed)
		to_chat(user, "<span class='warning'>[src] can't be unloaded until you un-ready it. (Alt-click)</span>")
		return
	return ..()

/obj/item/gun/projectile/smartgun/AltClick(mob/user) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/gun/projectile/smartgun/proc/toggle_real_state() // CHOMPEdit - Removal of obj/item/weapon
	cycling = FALSE
	closed = !closed

/obj/item/gun/projectile/smartgun/update_icon() // CHOMPEdit - Removal of obj/item/weapon
	. = ..()
	underlays = null
	if(ammo_magazine)
