...
/obj/item/broken_gun // CHOMPEdit - Removal of obj/item/weapon
	desc = "The remains of an unfortunate firearm."

	var/obj/item/gun/my_guntype = null // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/broken_gun/New(var/newloc, var/path) // CHOMPEdit - Removal of obj/item/weapon
	..()
	if(path)
		if(!setup_gun(path))
			qdel(src)
...
/obj/item/broken_gun/Initialize() // CHOMPEdit - Removal of obj/item/weapon
	. = ..()
	spawn(30 SECONDS)
		if(!my_guntype && !QDELETED(src))
			qdel(src)

/obj/item/broken_gun/examine(mob/user) // CHOMPEdit - Removal of obj/item/weapon
	. = ..()
	spawn()
...
/obj/item/broken_gun/proc/setup_gun(var/obj/item/gun/path) // CHOMPEdit - Removal of obj/item/weapon
	if(ispath(path))
...
/obj/item/broken_gun/proc/setup_repair_needs() // CHOMPEdit - Removal of obj/item/weapon
	...
	if(prob(30))
		var/component_needed = pick(/obj/item/stock_parts/gear,/obj/item/stock_parts/spring,/obj/item/stock_parts/manipulator) // CHOMPEdit - Removal of obj/item/weapon
		material_needs[component_needed] = rand(1,3)

	if(ispath(my_guntype, /obj/item/gun/energy) && prob(25)) // CHOMPEdit - Removal of obj/item/weapon
		var/component_needed = pick(/obj/item/stack/cable_coil, /obj/item/stock_parts/scanning_module,/obj/item/stock_parts/capacitor) // CHOMPEdit - Removal of obj/item/weapon
		material_needs[component_needed] = rand(1,3)

	if(ispath(my_guntype, /obj/item/gun/launcher) && prob(50)) // CHOMPEdit - Removal of obj/item/weapon
		var/component_needed = pick(/obj/item/tape_roll, /obj/item/stack/rods, /obj/item/handcuffs/cable) // CHOMPEdit - Removal of obj/item/weapon
		material_needs[component_needed] = 1

	if(ispath(my_guntype, /obj/item/gun/magnetic) && prob(70)) // CHOMPEdit - Removal of obj/item/weapon
		var/component_needed = pick(/obj/item/smes_coil, /obj/item/assembly/prox_sensor, /obj/item/module/power_control) // CHOMPEdit - Removal of obj/item/weapon // CHOMPEdit - Removal of obj/item/device
		material_needs[component_needed] = 1

	material_needs[/obj/item/stack/material/steel] = rand(1,5)

/obj/item/broken_gun/attackby(obj/item/W as obj, mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/broken_gun/proc/can_repair_with(obj/item/I, mob/user) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/broken_gun/proc/repair_with(obj/item/I, mob/user) // CHOMPEdit - Removal of obj/item/weapon
	for(var/path in material_needs)
		if(ispath(path) && istype(I, path))
			if(material_needs[path] > 0)
				if(istype(I, /obj/item/stack))
...
/obj/item/broken_gun/proc/check_complete_repair(mob/user) // CHOMPEdit - Removal of obj/item/weapon
