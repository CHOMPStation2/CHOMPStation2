...
/datum/firemode/New(obj/item/gun/gun, list/properties = null) // CHOMPEdit - Removal of obj/item/weapon
	..()
...
/datum/firemode/proc/apply_to(obj/item/gun/gun) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/gun/CtrlClick(mob/user) // CHOMPEdit - Removal of obj/item/weapon
	if(can_flashlight && ishuman(user) && src.loc == usr && !user.incapacitated(INCAPACITATION_ALL))
		toggle_flashlight()
	else
		return ..()

/obj/item/gun/proc/toggle_flashlight() // CHOMPEdit - Removal of obj/item/weapon
	if(gun_light)
		set_light(0)
		gun_light = FALSE
	else
...
/obj/item/gun/New() // CHOMPEdit - Removal of obj/item/weapon
	..()
	...
	if(!dna_lock)
		verbs -= /obj/item/gun/verb/remove_dna // CHOMPEdit - Removal of obj/item/weapon
		verbs -= /obj/item/gun/verb/give_dna // CHOMPEdit - Removal of obj/item/weapon
		verbs -= /obj/item/gun/verb/allow_dna // CHOMPEdit - Removal of obj/item/weapon

/obj/item/gun/update_twohanding() // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/gun/update_held_icon() // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/gun/proc/special_check(var/mob/user) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/gun/emp_act(severity) // CHOMPEdit - Removal of obj/item/weapon
	for(var/obj/O in contents)
		O.emp_act(severity)

/obj/item/gun/afterattack(atom/A, mob/living/user, adjacent, params) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/gun/attack(atom/A, mob/living/user, def_zone) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/gun/attackby(var/obj/item/A as obj, mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/gun/emag_act(var/remaining_charges, var/mob/user) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/gun/MouseDrop(obj/over_object as obj) // CHOMPEdit - Removal of obj/item/weapon
	if(!canremove)
		return
...
/obj/item/gun/proc/Fire(atom/target, mob/living/user, clickparams, pointblank=0, reflex=0) // CHOMPEdit - Removal of obj/item/weapon
	...
	if(recoil_mode && iscarbon(user))
		...
		if(recoil_mode > 0)
			if(mysize <= 0.60)
				...
				if(!istype(src,/obj/item/gun/energy)) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/gun/proc/Fire_userless(atom/target) // CHOMPEdit - Removal of obj/item/weapon
	if(!target)
		return
...
/obj/item/gun/proc/consume_next_projectile() // CHOMPEdit - Removal of obj/item/weapon
	return null

//used by aiming code
/obj/item/gun/proc/can_hit(atom/target as mob, var/mob/living/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if(!special_check(user))
		return 2
...
/obj/item/gun/proc/handle_click_empty(mob/user) // CHOMPEdit - Removal of obj/item/weapon
	if (user)
...
/obj/item/gun/proc/handle_firing_text(mob/user, atom/target, pointblank = FALSE, reflex = FALSE) // CHOMPEdit - Removal of obj/item/weapon
	if(silenced)
...
/obj/item/gun/proc/handle_post_fire(mob/user, atom/target, var/pointblank=0, var/reflex=0) // CHOMPEdit - Removal of obj/item/weapon
	if(fire_anim)
		flick(fire_anim, src)
...
/obj/item/gun/proc/process_point_blank(obj/projectile, mob/user, atom/target) // CHOMPEdit - Removal of obj/item/weapon
	var/obj/item/projectile/P = projectile
	if(!istype(P))
	...
	if(ismob(target))
		...
		if(M.grabbed_by.len)
			var/grabstate = 0
			for(var/obj/item/grab/G in M.grabbed_by) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/gun/proc/process_accuracy(obj/projectile, mob/living/user, atom/target, var/burst, var/held_twohanded) // CHOMPEdit - Removal of obj/item/weapon
	var/obj/item/projectile/P = projectile
	if(!istype(P))
		return //default behaviour only applies to true projectiles
...
/obj/item/gun/proc/process_projectile(obj/projectile, mob/user, atom/target, var/target_zone, var/params=null) // CHOMPEdit - Removal of obj/item/weapon
	var/obj/item/projectile/P = projectile
	if(!istype(P))
...
/obj/item/gun/proc/play_fire_sound(var/mob/user, var/obj/item/projectile/P) // CHOMPEdit - Removal of obj/item/weapon
...
//Suicide handling.
/obj/item/gun/var/mouthshoot = 0 //To stop people from suiciding twice... >.> // CHOMPEdit - Removal of obj/item/weapon

/obj/item/gun/proc/handle_suicide(mob/living/user) // CHOMPEdit - Removal of obj/item/weapon
	if(!ishuman(user))
		return
...
/obj/item/gun/proc/toggle_scope(var/zoom_amount=2.0) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/gun/zoom() // CHOMPEdit - Removal of obj/item/weapon
	..()
	if(!zoom)
		accuracy = initial(accuracy)
		recoil = initial(recoil)

/obj/item/gun/examine(mob/user) // CHOMPEdit - Removal of obj/item/weapon
	. = ..()
	if(firemodes.len > 1)
		var/datum/firemode/current_mode = firemodes[sel_mode]
		. += "The fire selector is set to [current_mode.name]."

/obj/item/gun/proc/switch_firemodes(mob/user) // CHOMPEdit - Removal of obj/item/weapon
	if(firemodes.len <= 1)
		return null
...
/obj/item/gun/attack_self(mob/user) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/gun/proc/has_ammo_counter() // CHOMPEdit - Removal of obj/item/weapon
	return FALSE

/obj/item/gun/proc/get_ammo_type() // CHOMPEdit - Removal of obj/item/weapon
	return FALSE

/obj/item/gun/proc/get_ammo_count() // CHOMPEdit - Removal of obj/item/weapon
	return FALSE

/obj/item/gun/equipped(mob/living/user, slot) // When a gun is equipped to your hands, we'll add the HUD to the user. Pending porting over TGMC guncode where wielding is far more sensible. // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/gun/dropped(mob/living/user) // Ditto as above, we remove the HUD. Pending porting TGMC code to clean up this fucking nightmare of spaghetti. // CHOMPEdit - Removal of obj/item/weapon
	user.hud_used.remove_ammo_hud(user, src)
