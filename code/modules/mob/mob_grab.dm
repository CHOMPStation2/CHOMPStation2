...
/client/proc/Process_Grab()
	...
	if(isliving(mob))
		...
		for(var/obj/item/grab/G in list(L.l_hand, L.r_hand)) // CHOMPEdit - Removal of obj/item/weapon
			G.reset_kill_state() //no wandering across the station/asteroid while choking someone

/obj/item/grab // CHOMPEdit - Removal of obj/item/weapon
	name = "grab"
...
/obj/item/grab/New(mob/user, mob/victim) // CHOMPEdit - Removal of obj/item/weapon
	..()
	loc = user
	assailant = user
	affecting = victim
	...
	if(assailant.grabbed_by)
		for (var/obj/item/grab/G in assailant.grabbed_by) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/grab/proc/throw_held() // CHOMPEdit - Removal of obj/item/weapon
	if(affecting)
...
/obj/item/grab/proc/synch() //why is this needed? // CHOMPEdit - Removal of obj/item/weapon
	if(QDELETED(src))
		return
	if(affecting)
...
/obj/item/grab/process() // CHOMPEdit - Removal of obj/item/weapon
	...
	if(state <= GRAB_AGGRESSIVE)
		...
		if((assailant.l_hand && assailant.l_hand != src && istype(assailant.l_hand, /obj/item/grab))) // CHOMPEdit - Removal of obj/item/weapon
			var/obj/item/grab/G = assailant.l_hand // CHOMPEdit - Removal of obj/item/weapon
			if(G.affecting != affecting)
				allow_upgrade = 0
		if((assailant.r_hand && assailant.r_hand != src && istype(assailant.r_hand, /obj/item/grab))) // CHOMPEdit - Removal of obj/item/weapon
			var/obj/item/grab/G = assailant.r_hand // CHOMPEdit - Removal of obj/item/weapon
			if(G.affecting != affecting)
				allow_upgrade = 0

		//disallow upgrading past aggressive if we're being grabbed aggressively
		for(var/obj/item/grab/G in affecting.grabbed_by) // CHOMPEdit - Removal of obj/item/weapon
			if(G == src) continue
...
/obj/item/grab/proc/handle_eye_mouth_covering(mob/living/carbon/target, mob/user, var/target_zone) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/grab/attack_self() // CHOMPEdit - Removal of obj/item/weapon
	return s_click(hud)


//Updating pixelshift, position and direction
//Gets called on process, when the grab gets upgraded or the assailant moves
/obj/item/grab/proc/adjust_position() // CHOMPEdit - Removal of obj/item/weapon
	if(!affecting)
		qdel(src)
		return
	if(affecting.buckled)
...
/obj/item/grab/proc/s_click(obj/screen/S) // CHOMPEdit - Removal of obj/item/weapon
	if(QDELETED(src))
		return
	if(!affecting)
		return
...
/obj/item/grab/proc/confirm() // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/grab/attack(mob/M, mob/living/user) // CHOMPEdit - Removal of obj/item/weapon
	if(QDELETED(src))
		return
	if(!affecting)
		return
...
/obj/item/grab/dropped() // CHOMPEdit - Removal of obj/item/weapon
	loc = null
	if(!QDELETED(src))
		qdel(src)

/obj/item/grab/proc/reset_kill_state() // CHOMPEdit - Removal of obj/item/weapon
	if(state == GRAB_KILL)
...
/obj/item/grab/proc/handle_resist() // CHOMPEdit - Removal of obj/item/weapon
	var/grab_name
...
/obj/item/grab/proc/size_difference(mob/A, mob/B) // CHOMPEdit - Removal of obj/item/weapon
	return mob_size_difference(A.mob_size, B.mob_size)

/obj/item/grab/Destroy() // CHOMPEdit - Removal of obj/item/weapon
	animate(affecting, pixel_x = initial(affecting.pixel_x), pixel_y = initial(affecting.pixel_y), 4, 1, LINEAR_EASING)
