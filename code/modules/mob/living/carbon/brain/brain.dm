...
/mob/living/carbon/brain/say_understands(var/other)//Goddamn is this hackish, but this say code is so odd
	if(istype(container, /obj/item/mmi)) // CHOMPEdit - Removal of obj/item/device
		if(issilicon(other))
			return TRUE
	if(ishuman(other))
		return TRUE
...
/mob/living/carbon/brain/update_canmove()
	if(in_contents_of(/obj/mecha) || istype(loc, /obj/item/mmi)) // CHOMPEdit - Removal of obj/item/device
		canmove = 1
		use_me = 1
	else
		canmove = 0
...
/mob/living/carbon/brain/isSynthetic()
	return istype(loc, /obj/item/mmi) // CHOMPEdit - Removal of obj/item/device
