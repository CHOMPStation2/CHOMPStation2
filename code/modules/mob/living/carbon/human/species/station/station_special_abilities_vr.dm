...
/mob/living/carbon/human/proc/succubus_drain()
	...
	var/obj/item/grab/G = src.get_active_hand() // CHOMPEdit - Removal of obj/item/weapon
	if(!istype(G))
...
/mob/living/carbon/human/can_shred()
	//Humans need a grab
	var/obj/item/grab/G = get_active_hand() // CHOMPEdit - Removal of obj/item/weapon
	if(!istype(G))
...
/mob/living/carbon/human/proc/enter_cocoon()
	...
	if(do_after(src, 25, exclusive = TASK_USER_EXCLUSIVE))
		var/obj/item/storage/vore_egg/bugcocoon/C = new(loc) // CHOMPEdit - Removal of obj/item/weapon
		forceMove(C)
		transforming = TRUE
		var/datum/tgui_module/appearance_changer/cocoon/V = new(src, src)
		V.tgui_interact(src)

		var/mob_holder_type = src.holder_type || /obj/item/holder // CHOMPEdit - Removal of obj/item/weapon
		C.w_class = src.size_multiplier * 4 //Egg size and weight scaled to match occupant.
		var/obj/item/holder/H = new mob_holder_type(C, src) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/gun/energy/gun/tongue //This is the 'tongue' gun for admin memery. // CHOMPEdit - Removal of obj/item/weapon
	name = "tongue"
...
/obj/item/gun/energy/gun/tongue/update_icon() //No updating the icon. // CHOMPEdit - Removal of obj/item/weapon
	icon_state = "synthtongue"
	return

/obj/item/gun/energy/bfgtaser/tongue // CHOMPEdit - Removal of obj/item/weapon
