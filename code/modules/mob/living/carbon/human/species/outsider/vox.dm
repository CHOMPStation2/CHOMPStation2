...
/datum/species/vox/equip_survival_gear(var/mob/living/carbon/human/H, var/extendedtank = 0,var/comprehensive = 0)
	...
	if(H.backbag == 1)
		H.equip_to_slot_or_del(new /obj/item/tank/nitrogen(H), slot_back) //CHOMPedit // CHOMPEdit - Removal of obj/item/weapon
		H.internal = H.back
	else
		H.equip_to_slot_or_del(new /obj/item/tank/nitrogen(H), slot_r_hand) //CHOMPedit // CHOMPEdit - Removal of obj/item/weapon
		H.internal = H.r_hand
	H.internal = locate(/obj/item/tank) in H.contents // CHOMPEdit - Removal of obj/item/weapon
