...
/datum/species/proc/produceCopy(var/list/traits, var/mob/living/carbon/human/H, var/custom_base)
	...
	if(selects_bodytype == SELECTS_BODYTYPE_CUSTOM && custom_base) //If race selects a bodytype, retrieve the custom_base species and copy needed variables.
		var/datum/species/S = GLOB.all_species[custom_base]
		S.copy_variables(new_copy, copy_vars)

	if (selects_bodytype == SELECTS_BODYTYPE_SHAPESHIFTER && custom_base)
