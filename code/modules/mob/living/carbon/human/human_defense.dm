	...
	if(P.can_embed())
		...
		if(!prob(armor/2))		//Even if the armor doesn't stop the bullet from hurting you, it might stop it from embedding.
			...
			if(prob(max(hit_embed_chance, 0)))
				var/obj/item/material/shard/shrapnel/SP = new() // CHOMPEdit - Removal of obj/item/weapon
...
/mob/living/carbon/human/shank_attack(obj/item/W, obj/item/grab/G, mob/user, hit_zone) // CHOMPEdit - Removal of obj/item/weapon
