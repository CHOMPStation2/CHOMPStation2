/mob/living
	...
	var/obj/item/back = null//Human/Monkey // CHOMPEdit - Removal of obj/item/weapon
	var/obj/item/tank/internal = null//Human/Monkey // CHOMPEdit - Removal of obj/item/weapon
	var/obj/item/clothing/mask/wear_mask = null//Carbon

/mob/living/equip_to_storage(obj/item/newitem, user_initiated = FALSE)
	// Try put it in their backpack
	if(istype(src.back,/obj/item/storage)) // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/storage/backpack = src.back // CHOMPEdit - Removal of obj/item/weapon
	...
	for(var/obj/item/storage/S in src.contents) // CHOMPEdit - Removal of obj/item/weapon
	...
	if(istype(src.back,/obj/item/rig))	//This would be much cooler if we had componentized storage datums // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/rig/R = src.back // CHOMPEdit - Removal of obj/item/weapon
		if(R.rig_storage)
			var/obj/item/storage/backpack = R.rig_storage // CHOMPEdit - Removal of obj/item/weapon
			if(backpack.can_be_inserted(newitem, 1))
				if(user_initiated)
					backpack.handle_item_insertion(newitem)
				else
...
/mob/living/ret_grab(var/list/L, var/mobchain_limit = 5)
	...
	// Lefty grab!
	if (istype(l_hand, /obj/item/grab)) // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/grab/G = l_hand // CHOMPEdit - Removal of obj/item/weapon
		L |= G.affecting
		if(mobchain_limit-- > 0)
			G.affecting?.ret_grab(L, mobchain_limit) // Recurse! They can update the list. It's the same instance as ours.

	// Righty grab!
	if (istype(r_hand, /obj/item/grab)) // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/grab/G = r_hand // CHOMPEdit - Removal of obj/item/weapon
		L |= G.affecting
		if(mobchain_limit-- > 0)
...
/datum/inventory_panel/tgui_data(mob/user, datum/tgui/ui, datum/tgui_state/state)
	...
	data["internalsValid"] = istype(host.wear_mask, /obj/item/clothing/mask) && istype(host.back, /obj/item/tank) // CHOMPEdit - Removal of obj/item/weapon
...
/datum/inventory_panel/human/tgui_data(mob/user, datum/tgui/ui, datum/tgui_state/state)
	...
	data["internalsValid"] = (istype(H.wear_mask, /obj/item/clothing/mask) || istype(H.head, /obj/item/clothing/head/helmet/space)) && (istype(H.back, /obj/item/tank) || istype(H.belt, /obj/item/tank) || istype(H.s_store, /obj/item/tank)) // CHOMPEdit - Removal of obj/item/weapon
