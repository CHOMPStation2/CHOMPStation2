...
/mob/living/Bump(atom/movable/AM)
	...
	if (istype(AM, /mob/living))
		...
		for(var/mob/living/M in range(tmob, 1))
			if(tmob.pinned.len ||  ((M.pulling == tmob && ( tmob.restrained() && !( M.restrained() ) && M.stat == 0)) || locate(/obj/item/grab, tmob.grabbed_by.len)) ) // CHOMPEdit - Removal of obj/item/weapon
				if ( !(world.time % 5) )
		...
		if(istype(tmob, /mob/living/carbon/human) && (FAT in tmob.mutations))
			if(prob(40) && !(FAT in src.mutations))
				...
				now_pushing = 0
				return
		if(tmob.r_hand && istype(tmob.r_hand, /obj/item/shield/riot)) // CHOMPEdit - Removal of obj/item/weapon
			if(prob(99))
				now_pushing = 0
				return
		if(tmob.l_hand && istype(tmob.l_hand, /obj/item/shield/riot)) // CHOMPEdit - Removal of obj/item/weapon
			if(prob(99))
				now_pushing = 0
				return
	...
	if (!now_pushing)
		...
		if(ishuman(AM) && AM:grabbed_by)
			for(var/obj/item/grab/G in AM:grabbed_by) // CHOMPEdit - Removal of obj/item/weapon
