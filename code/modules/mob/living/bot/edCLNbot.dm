...
/obj/item/secbot_assembly/edCLN_assembly // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/secbot_assembly/edCLN_assembly/attackby(var/obj/item/W as obj, var/mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	..()
	...
	switch(build_step)
		...
		if(2)
			if(istype(W, /obj/item/reagent_containers/glass/bucket)) // CHOMPEdit - Removal of obj/item/weapon
				user.drop_item()
				qdel(W)
				build_step++
				to_chat(user, "<span class='notice'>You add \the [W] to \the [src].</span>")
		...
		if(6)
			if(istype(W, /obj/item/mop)) // CHOMPEdit - Removal of obj/item/weapon
