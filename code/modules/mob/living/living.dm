...
/mob/living/get_contents(var/obj/item/storage/Storage = null) // CHOMPEdit - Removal of obj/item/weapon
	var/list/L = list()
	...
	if(Storage) //If it called itself
		...
		//for(var/obj/item/storage/S in Storage.return_inv()) //Check for storage items // CHOMPEdit - Removal of obj/item/weapon
		//	L += get_contents(S)

		for(var/obj/item/gift/G in Storage.return_inv()) //Check for gift-wrapped items // CHOMPEdit - Removal of obj/item/weapon
			L += G.gift
		...
		for(var/obj/item/smallDelivery/D in Storage.return_inv()) //Check for package wrapped items
			L += D.wrapped
			if(istype(D.wrapped, /obj/item/storage)) //this should never happen // CHOMPEdit - Removal of obj/item/weapon
		...
		L += src.contents
		for(var/obj/item/storage/S in src.contents)	//Check for storage items // CHOMPEdit - Removal of obj/item/weapon
			L += get_contents(S)

		for(var/obj/item/gift/G in src.contents) //Check for gift-wrapped items // CHOMPEdit - Removal of obj/item/weapon
			L += G.gift
			if(istype(G.gift, /obj/item/storage)) // CHOMPEdit - Removal of obj/item/weapon
				L += get_contents(G.gift)
...
/mob/living/proc/resist_grab()
	var/resisting = 0
	for(var/obj/item/grab/G in grabbed_by) // CHOMPEdit - Removal of obj/item/weapon
		resisting++
		G.handle_resist()
	if(resisting)
...
/mob/living/proc/get_restraining_bolt()
	var/obj/item/implant/restrainingbolt/RB = locate() in src // CHOMPEdit - Removal of obj/item/weapon
	if(RB)
		if(!RB.malfunction)
			return TRUE
...
/mob/living/update_canmove()
	...
	/* CHOMPEdit - Allow us to hold stuff while laying down.
		...
		if(r_hand)
			unEquip(r_hand)
		for(var/obj/item/holder/holder in get_mob_riding_slots()) // CHOMPEdit - Removal of obj/item/weapon
			unEquip(holder)
	*/
...
/mob/living/throw_item(atom/target)
	...
	if (istype(item, /obj/item/grab)) // CHOMPEdit - Removal of obj/item/weapon
