GLOBAL_LIST_EMPTY(empty_playable_ai_cores) // CHOMPEdit - Globals

/hook/roundstart/proc/spawn_empty_ai()
	for(var/obj/effect/landmark/start/S in GLOB.landmarks_list) // CHOMPEdit - Globals
		if(S.name != "AI")
			continue
		if(locate(/mob/living) in S.loc)
			continue
		GLOB.empty_playable_ai_cores += new /obj/structure/AIcore/deactivated(get_turf(S)) // CHOMPEdit - Globals
...
/mob/living/silicon/ai/verb/store_core()
	...
	// We warned you.
	GLOB.empty_playable_ai_cores += new /obj/structure/AIcore/deactivated(loc) // CHOMPEdit - Globals
