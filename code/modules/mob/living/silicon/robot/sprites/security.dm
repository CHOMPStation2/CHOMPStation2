...
/datum/robot_sprite/dogborg/security/handle_extra_icon_updates(var/mob/living/silicon/robot/ourborg)
	if(has_laser_sprite && istype(ourborg.module_active, /obj/item/gun/energy/laser/mounted)) // CHOMPEdit - Removal of obj/item/weapon
		ourborg.add_overlay("[sprite_icon_state]-laser")
	if(has_taser_sprite && istype(ourborg.module_active, /obj/item/gun/energy/taser/mounted/cyborg)) // CHOMPEdit - Removal of obj/item/weapon
