...
/datum/robot_sprite/fluff/jademanique/handle_extra_icon_updates(var/mob/living/silicon/robot/ourborg)
	if(istype(ourborg.module_active, /obj/item/gun/energy/laser/mounted)) // CHOMPEdit - Removal of obj/item/weapon
		ourborg.add_overlay("[sprite_icon_state]-laser")
	if(istype(ourborg.module_active, /obj/item/gun/energy/taser/mounted/cyborg)) // CHOMPEdit - Removal of obj/item/weapon
