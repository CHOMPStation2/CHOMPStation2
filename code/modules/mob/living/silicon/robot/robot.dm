...
//3 Modules can be activated at any one time.
	var/obj/item/robot_module/module = null // CHOMPEdit - Removal of obj/item/weapon
	var/module_active = null
	var/module_state_1 = null
	var/module_state_2 = null
	var/module_state_3 = null

	var/obj/item/radio/borg/radio = null // CHOMPEdit - Removal of obj/item/device
	var/obj/item/communicator/integrated/communicator = null // CHOMPEdit - Removal of obj/item/device
	var/mob/living/silicon/ai/connected_ai = null
	var/obj/item/cell/cell = null // CHOMPEdit - Removal of obj/item/weapon
	...
	var/obj/item/mmi/mmi = null // CHOMPEdit - Removal of obj/item/device

	var/obj/item/pda/ai/rbPDA = null // CHOMPEdit - Removal of obj/item/device
	...
	var/obj/item/implant/restrainingbolt/bolt	// The restraining bolt installed into the cyborg. // CHOMPEdit - Removal of obj/item/weapon
...
/mob/living/silicon/robot/New(loc, var/unfinished = 0)
	...
	radio = new /obj/item/radio/borg(src) // CHOMPEdit - Removal of obj/item/device
//	communicator = new /obj/item/communicator/integrated(src) // CHOMPEdit - Removal of obj/item/device
//	communicator.register_device(src)
	...
	if(!cell)
		cell = new /obj/item/cell/robot_station(src) // CHOMPEdit - Removal of obj/item/weapon
	else if(ispath(cell))
		cell = new cell(src)
	...
	for (var/V in components)
		...
		if(!C.wrapped)
			switch(V)
				...
				if("power cell")
					var/list/recommended_cells = list(/obj/item/cell/robot_station, /obj/item/cell/high, /obj/item/cell/super, /obj/item/cell/robot_syndi, /obj/item/cell/hyper, // CHOMPEdit - Removal of obj/item/weapon
						/obj/item/cell/infinite, /obj/item/cell/potato, /obj/item/cell/slime) // CHOMPEdit - Removal of obj/item/weapon
					var/list/cell_names = list()
					for(var/cell_type in recommended_cells)
						var/obj/item/cell/single_cell = cell_type // CHOMPEdit - Removal of obj/item/weapon
...
/mob/living/silicon/robot/proc/setup_PDA()
	if (!rbPDA)
		rbPDA = new/obj/item/pda/ai(src) // CHOMPEdit - Removal of obj/item/device
	rbPDA.set_name_and_job(name,"[modtype] [braintype]")
	add_verb(src, /obj/item/pda/ai/verb/cmd_pda_open_ui) //ChompEDIT - TGPanel // CHOMPEdit - Removal of obj/item/device

/mob/living/silicon/robot/proc/setup_communicator()
	if (!communicator)
		communicator = new/obj/item/communicator/integrated(src) // CHOMPEdit - Removal of obj/item/device
	communicator.register_device(name, "[modtype] [braintype]")
	add_verb(src, /obj/item/communicator/integrated/verb/activate) //ChompEDIT - TGPanel // CHOMPEdit - Removal of obj/item/device
...
/mob/living/silicon/robot/Destroy()
	if(mmi && mind)//Safety for when a cyborg gets dust()ed. Or there is no MMI inside.
		...
		if(T)	mmi.loc = T
		if(mmi.brainmob)
			var/obj/item/robot_module/M = locate() in contents // CHOMPEdit - Removal of obj/item/weapon
			if(M)
...
/mob/living/silicon/robot/proc/update_braintype()
	if(istype(mmi, /obj/item/mmi/digital/posibrain)) // CHOMPEdit - Removal of obj/item/device
		braintype = BORG_BRAINTYPE_POSI
	else if(istype(mmi, /obj/item/mmi/digital/robot)) // CHOMPEdit - Removal of obj/item/device
		braintype = BORG_BRAINTYPE_DRONE
	else if(istype(mmi, /obj/item/mmi/inert/ai_remote)) // CHOMPEdit - Removal of obj/item/device
...
/mob/living/silicon/robot/proc/show_jetpack_pressure()
	...
	var/obj/item/tank/jetpack/current_jetpack = installed_jetpack() // CHOMPEdit - Removal of obj/item/weapon
	if (current_jetpack)
...
/mob/living/silicon/robot/proc/installed_jetpack()
	if(module)
		return (locate(/obj/item/tank/jetpack) in module.modules) // CHOMPEdit - Removal of obj/item/weapon
	return 0
...
/mob/living/silicon/robot/attackby(obj/item/W as obj, mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if (istype(W, /obj/item/handcuffs)) // fuck i don't even know why isrobot() in handcuff code isn't working so this will have to do // CHOMPEdit - Removal of obj/item/weapon
		return
	...
	if(opened) // Are they trying to insert something?
		...
		if(istype(W, /obj/item/implant/restrainingbolt) && !cell) // CHOMPEdit - Removal of obj/item/weapon
			if(bolt)
	...
	if(istype(W, /obj/item/aiModule)) // Trying to modify laws locally. // CHOMPEdit - Removal of obj/item/weapon
		if(!opened)
		...
		var/obj/item/aiModule/M = W // CHOMPEdit - Removal of obj/item/weapon
		M.install(src, user)
		return
	...
	if(W.has_tool_quality(TOOL_WELDER) && user.a_intent != I_HURT)
	...
	else if (istype(W, /obj/item/cell) && opened)	// trying to put a cell inside // CHOMPEdit - Removal of obj/item/weapon
	...
	else if (W.has_tool_quality(TOOL_WIRECUTTER) || istype(W, /obj/item/multitool)) // CHOMPEdit - Removal of obj/item/device
		if (wiresexposed)
			wires.Interact(user)
		else
	...
	else if(istype(W, /obj/item/encryptionkey/) && opened) // CHOMPEdit - Removal of obj/item/device
		...
		if( !(istype(W, /obj/item/robotanalyzer) || istype(W, /obj/item/healthanalyzer)) ) // CHOMPEdit - Removal of obj/item/device
			if(W.force > 0)
				spark_system.start()
		return ..()
...
/mob/living/silicon/robot/get_restraining_bolt()
	var/obj/item/implant/restrainingbolt/RB = bolt // CHOMPEdit - Removal of obj/item/weapon
...
/mob/living/silicon/robot/proc/allowed(mob/M)
	...
	if(istype(M, /mob/living/carbon/human))
		...
		if(check_access(R.get_active_hand()) || istype(R.get_active_hand(), /obj/item/card/robot)) // CHOMPEdit - Removal of obj/item/weapon
			return 1
	return 0
...
/mob/living/silicon/robot/drop_item()
	if(module_active && istype(module_active,/obj/item/gripper)) // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/gripper/G = module_active // CHOMPEdit - Removal of obj/item/weapon
		G.drop_item_nm()
...
/datum/riding/dogborg
	keytype = /obj/item/material/twohanded/riding_crop // Crack! // CHOMPEdit - Removal of obj/item/weapon
...
/mob/living/silicon/robot/proc/has_advanced_upgrade(var/given_type)
	if(given_type == /obj/item/borg/upgrade/advanced/bellysizeupgrade)
		var/obj/item/dogborg/sleeper/T = has_upgrade_module(/obj/item/dogborg/sleeper) // CHOMPEdit - Removal of obj/item/device
	...
	if(given_type == /obj/item/borg/upgrade/advanced/jetpack)
		return has_upgrade_module(/obj/item/tank/jetpack/carbondioxide) // CHOMPEdit - Removal of obj/item/weapon
	if(given_type == /obj/item/borg/upgrade/advanced/advhealth)
		return has_upgrade_module(/obj/item/healthanalyzer/advanced) // CHOMPEdit - Removal of obj/item/device
	if(given_type == /obj/item/borg/upgrade/advanced/sizegun)
		return has_upgrade_module(/obj/item/gun/energy/sizegun/mounted) // CHOMPEdit - Removal of obj/item/weapon
	return null
...
/mob/living/silicon/robot/proc/has_restricted_upgrade(var/given_type)
	...
	if(given_type == /obj/item/borg/upgrade/restricted/tasercooler)
		var/obj/item/gun/energy/taser/mounted/cyborg/T = has_upgrade_module(/obj/item/gun/energy/taser/mounted/cyborg) // CHOMPEdit - Removal of obj/item/weapon
	...
	if(given_type == /obj/item/borg/upgrade/restricted/advrped)
		return has_upgrade_module(/obj/item/storage/part_replacer/adv) // CHOMPEdit - Removal of obj/item/weapon
	if(given_type == /obj/item/borg/upgrade/restricted/diamonddrill)
		return has_upgrade_module(/obj/item/pickaxe/diamonddrill) // CHOMPEdit - Removal of obj/item/weapon
	if(given_type == /obj/item/borg/upgrade/restricted/pka)
		return has_upgrade_module(/obj/item/gun/energy/kinetic_accelerator/cyborg) // CHOMPEdit - Removal of obj/item/weapon
	return null
...
/mob/living/silicon/robot/proc/has_no_prod_upgrade(var/given_type)
	if(given_type == /obj/item/borg/upgrade/no_prod/toygun)
		return has_upgrade_module(/obj/item/gun/projectile/cyborgtoy) // CHOMPEdit - Removal of obj/item/weapon
