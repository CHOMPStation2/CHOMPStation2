...
/obj/item/robotanalyzer/attack(mob/living/M as mob, mob/living/user as mob) // CHOMPEdit - Removal of obj/item/device
	do_scan(M, user)

/obj/item/robotanalyzer/AltClick(mob/user) // CHOMPEdit - Removal of obj/item/device
	mode = !mode
	user.show_message(span_blue("[mode ? "Toggled to cyborg analyzing mode." : "Toggled to cyborg upgrade scan mode."]"), 1)

/obj/item/robotanalyzer/proc/do_scan(mob/living/M as mob, mob/living/user as mob) // CHOMPEdit - Removal of obj/item/device
	...
	switch(scan_type)
		if("robot")
			if(mode)
				...
				var/obj/item/cell/cell = R.get_cell() // CHOMPEdit - Removal of obj/item/weapon
				if(cell)
