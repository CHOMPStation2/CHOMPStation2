/mob/living/silicon/robot/get_jetpack()
	if(module)
		for(var/obj/item/tank/jetpack/J in module.modules) // CHOMPEdit - Removal of obj/item/weapon
			return J
...
/mob/living/silicon/robot/Process_Spaceslipping(var/prob_slip)
	var/obj/item/tank/jetpack/thrust = get_jetpack() // CHOMPEdit - Removal of obj/item/weapon
...
/mob/living/silicon/robot/Moved(atom/old_loc, direction, forced = FALSE)
	...
	if(istype(module_state_1, /obj/item/storage/bag/ore) || istype(module_state_2, /obj/item/storage/bag/ore) || istype(module_state_3, /obj/item/storage/bag/ore)) // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/storage/bag/ore/B = null // CHOMPEdit - Removal of obj/item/weapon
		if(istype(module_state_1, /obj/item/storage/bag/ore)) //First orebag has priority, if they for some reason have multiple. // CHOMPEdit - Removal of obj/item/weapon
			B = module_state_1
		else if(istype(module_state_2, /obj/item/storage/bag/ore)) // CHOMPEdit - Removal of obj/item/weapon
			B = module_state_2
		else if(istype(module_state_3, /obj/item/storage/bag/ore)) // CHOMPEdit - Removal of obj/item/weapon
			B = module_state_3
		var/turf/tile = loc
		if(isturf(tile))
