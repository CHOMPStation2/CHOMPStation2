...
/datum/paiController/Topic(href, href_list[])
	if(href_list["download"])
		...
		var/obj/item/paicard/card = locate(href_list["device"]) // CHOMPEdit - Removal of obj/item/device
		if(card.pai)
			return
		if(istype(card,/obj/item/paicard) && istype(candidate,/datum/paiCandidate)) // CHOMPEdit - Removal of obj/item/device
	...
	if(href_list["new"])
		...
		switch(option)
			...
			if("submit")
				if(candidate)
					candidate.ready = 1
					for(var/obj/item/paicard/p in GLOB.all_pai_cards) //CHOMPedit: Explicit use of GLOB // CHOMPEdit - Removal of obj/item/device
...
/datum/paiController/proc/findPAI(var/obj/item/paicard/p, var/mob/user) // CHOMPEdit - Removal of obj/item/device
