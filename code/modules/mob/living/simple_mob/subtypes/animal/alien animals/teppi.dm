...
//	var/dad_id
	...
	loot_list = list(/obj/item/bone/horn = 100) // CHOMPEdit - Removal of obj/item/weapon
...
/mob/living/simple_mob/vore/alienanimals/teppi/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(stat == DEAD)
		return ..()
	/////GRABS AND HOLDERS/////
	if(istype(O, /obj/item/grab)) // CHOMPEdit - Removal of obj/item/weapon
		return ..()
	...
	if(teppi_wool)
		if(teppi_shear(user, O))
			return
	/////FOOD/////
	if(istype(O, /obj/item/reagent_containers/food)) // CHOMPEdit - Removal of obj/item/weapon
		if(resting)
		...
		return
	/////WEAPONS/////
	if(istype(O, /obj/item/material/knife)) // CHOMPEdit - Removal of obj/item/weapon
		if(client)
			return ..()
		if(resting)
...
/mob/living/simple_mob/vore/alienanimals/teppi/proc/teppi_shear(var/mob/user as mob, tool)
	...
	if(istype(tool, /obj/item/material/knife)) // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/material/knife/K = tool // CHOMPEdit - Removal of obj/item/weapon
		...
		if(K.edge)
			sheartime *= 0.5
	else if(istype(tool, /obj/item/tool/wirecutters)) // CHOMPEdit - Removal of obj/item/weapon
		sheartime *= 2
	else
		return FALSE
