...
/mob/living/simple_mob/animal/solargrub_larva
	...
	meat_amount = 1
	meat_type = /obj/item/reagent_containers/food/snacks/meat/grubmeat // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/multitool/afterattack(obj/O, mob/user, proximity) // CHOMPEdit - Removal of obj/item/device
	if(proximity)
...
/obj/item/melee/baton/afterattack(obj/O, mob/user, proximity) // CHOMPEdit - Removal of obj/item/weapon
	if(proximity)
		if(istype(O, /obj/machinery))
			var/mob/living/simple_mob/animal/solargrub_larva/grub = locate() in O
			if(grub)
