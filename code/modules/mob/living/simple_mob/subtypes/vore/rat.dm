...
/mob/living/simple_mob/vore/aggressive/rat/tame/Life()
	...
	if(hunger > 0 && life_since_foodscan++ > 5) //Only look for floor food when hungry.
		...
		for(var/obj/item/reagent_containers/food/snacks/S in oview(src,3)) //Accept thrown offerings and scavenge surroundings. // CHOMPEdit - Removal of obj/item/weapon
			if(get_dist(src,S) <=1)
...
/mob/living/simple_mob/vore/aggressive/rat/tame/Found(var/atom/found_atom)
	if(!SA_attackable(found_atom))
		...
		for(var/obj/item/reagent_containers/food/snacks/S in H) // CHOMPEdit - Removal of obj/item/weapon
			if(!food)
