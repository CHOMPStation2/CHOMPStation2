		...
		for(H in vore_selected.contents) //Only works for carbons, RIP mice. Should pick the first human the code finds.
			...
			if(do_after(user, rand(3 SECONDS, 7 SECONDS))) //You can just spam click to stack attempts if you feel like abusing it.
				if(prob(15))
					...
					N = 1
					//addtimer(CALLBACK(src, PROC_REF(removeMobFromPreyExcludes), weakref(H)), 1 MINUTES) //At the time of this PR, removeMobFromPreyExcludes breaks prey_excludes by deleting the list which causes problems with the Crossed() vore override before. This can be commented back in if that gets fixed.// CHOMPEdit
					break
				else
