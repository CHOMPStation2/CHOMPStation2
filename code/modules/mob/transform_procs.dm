...
/mob/proc/AIize(var/move = TRUE)
	...
	if(move)
		...
		for(var/obj/effect/landmark/start/sloc in GLOB.landmarks_list) // CHOMPEdit - Globals
			if (sloc.name != "AI")
				continue
			...
			loc_landmark = sloc
		if (!loc_landmark)
			for(var/obj/effect/landmark/tripai in GLOB.landmarks_list) // CHOMPEdit - Globals
