/datum/modifier
	...
	var/obj/item/cell/energy_source = null	// The source of the above. // CHOMPEdit - Removal of obj/item/weapon
...
*/
	var/obj/item/personal_shield_generator/shield_generator //This is the shield generator you're wearing! // CHOMPEdit - Removal of obj/item/device
...
/datum/modifier/shield_projection/check_if_valid() //Let's check to make sure you got the stuff and set the vars. Don't need to modify this for any subtypes!
	if(ishuman(holder)) //Only humans can use this! Other things later down the line might use the same stuff this does, but the shield generator is human only!
		...
		if(istype(H.get_equipped_item(slot_back), /obj/item/personal_shield_generator)) // CHOMPEdit - Removal of obj/item/device
			shield_generator = H.get_equipped_item(slot_back) //Sets the var on the modifier that the shield gen is their back shield gen.
		else if(istype(H.get_equipped_item(slot_belt), /obj/item/personal_shield_generator)) // CHOMPEdit - Removal of obj/item/device
			shield_generator = H.get_equipped_item(slot_belt) //No need for other checks. If they got hit by this, they just turned it on.
		else if(istype(H.get_equipped_item(slot_s_store), /obj/item/personal_shield_generator) ) //Rigsuits. // CHOMPEdit - Removal of obj/item/device
