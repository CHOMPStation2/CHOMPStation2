...
/obj/item/pda/tgui_state(mob/user) // CHOMPEdit - Removal of obj/item/device
	return GLOB.tgui_inventory_state

/obj/item/pda/tgui_interact(mob/user, datum/tgui/ui, datum/tgui/parent_ui) // CHOMPEdit - Removal of obj/item/device
	ui = SStgui.try_update_ui(user, src, ui)
	if(!ui)
		ui = new(user, src, "Pda", "Personal Data Assistant", parent_ui)
		ui.open()

/obj/item/pda/tgui_data(mob/user, datum/tgui/ui, datum/tgui_state/state) // CHOMPEdit - Removal of obj/item/device
	var/list/data = ..()
...
/obj/item/pda/tgui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state) // CHOMPEdit - Removal of obj/item/device
	if(..())
		return TRUE
	...
	switch(action)
		...
		if("Eject")//Ejects the cart, only done from hub.
			if(!isnull(cartridge))
				...
				if(ismob(T))
					T = T.loc
				var/obj/item/cartridge/C = cartridge // CHOMPEdit - Removal of obj/item/weapon
				C.forceMove(T)
