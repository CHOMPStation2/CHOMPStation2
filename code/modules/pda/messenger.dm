...
/datum/data/pda/app/messenger/update_ui(mob/user as mob, list/data)
	...
	if(active_conversation)
		...
		var/convopdas[0]
		var/pdas[0]
		for(var/obj/item/pda/P as anything in PDAs) // CHOMPEdit - Removal of obj/item/device
...
/datum/data/pda/app/messenger/tgui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state)
	...
	switch(action)
		...
		if("Message")
			var/obj/item/pda/P = locate(params["target"]) // CHOMPEdit - Removal of obj/item/device
...
/datum/data/pda/app/messenger/Topic(href, href_list)
	...
	switch(href_list["choice"])
		if("Message")
			var/obj/item/pda/P = locate(href_list["target"]) // CHOMPEdit - Removal of obj/item/device
			create_message(usr, P)
			if(href_list["target"] in conversations)            // Need to make sure the message went through, if not welp.
				active_conversation = href_list["target"]


/datum/data/pda/app/messenger/proc/create_message(var/mob/living/U, var/obj/item/pda/P) // CHOMPEdit - Removal of obj/item/device
...
/datum/data/pda/app/messenger/multicast/receive_message(list/data, ref)
	...
	var/obj/item/pda/multicaster/M = pda // CHOMPEdit - Removal of obj/item/device
	if(!istype(M))
		return
	...
	for(var/obj/item/pda/pda in PDAs) // CHOMPEdit - Removal of obj/item/device
		if(pda.cartridge && pda.owner && is_type_in_list(pda.cartridge, M.cartridges_to_send_to))
			targets |= pda
	if(targets.len)
		for(var/obj/item/pda/target in targets) // CHOMPEdit - Removal of obj/item/device
...
Invoked by /obj/item/pda/proc/createPropFakeConversation_admin(var/mob/M) // CHOMPEdit - Removal of obj/item/device
*/
