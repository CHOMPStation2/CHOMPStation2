...
var/global/list/obj/item/pda/PDAs = list() // CHOMPEdit - Removal of obj/item/device
...
/obj/item/pda // CHOMPEdit - Removal of obj/item/device
	...
	var/obj/item/cartridge/cartridge = null //current cartridge // CHOMPEdit - Removal of obj/item/weapon
	...
	var/obj/item/card/id/id = null //Making it possible to slot an ID card into the PDA so it can function as both. // CHOMPEdit - Removal of obj/item/weapon
	var/ownjob = null //related to above - this is assignment (potentially alt title)
	var/ownrank = null // this one is rank, never alt title

	var/obj/item/paicard/pai = null	// A slot for a personal AI device // CHOMPEdit - Removal of obj/item/device
...
/obj/item/pda/examine(mob/user) // CHOMPEdit - Removal of obj/item/device
	. = ..()
	if(Adjacent(user))
		. += "The time [stationtime2text()] is displayed in the corner of the screen."

/obj/item/pda/CtrlClick() // CHOMPEdit - Removal of obj/item/device
	if(can_use(usr) && !issilicon(usr))
		remove_pen()
		return
	..()

/obj/item/pda/AltClick() // CHOMPEdit - Removal of obj/item/device
	if(issilicon(usr))
		return
...
/obj/item/pda/proc/play_ringtone() // CHOMPEdit - Removal of obj/item/device
	var/S
...
/obj/item/pda/proc/set_ringtone() // CHOMPEdit - Removal of obj/item/device
...
/obj/item/pda/New(var/mob/living/carbon/human/H) // CHOMPEdit - Removal of obj/item/device
	..()
	PDAs += src
	PDAs = sortAtom(PDAs)
	update_programs()
	...
	new /obj/item/pen(src) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/pda/Initialize() // CHOMPEdit - Removal of obj/item/device
	. = ..()
	add_overlay("pda-pen")
//ChompEDIT END

/obj/item/pda/proc/can_use(mob/user) // CHOMPEdit - Removal of obj/item/device
	return (tgui_status(user, GLOB.tgui_inventory_state) == STATUS_INTERACTIVE)

/obj/item/pda/GetAccess() // CHOMPEdit - Removal of obj/item/device
	if(id)
		return id.GetAccess()
	else
		return ..()

/obj/item/pda/GetID() // CHOMPEdit - Removal of obj/item/device
	return id

/obj/item/pda/MouseDrop(obj/over_object as obj, src_location, over_location) // CHOMPEdit - Removal of obj/item/device
	var/mob/M = usr
	if((!istype(over_object, /obj/screen)) && can_use(usr))
		return attack_self(M)
	return

/obj/item/pda/proc/close(mob/user) // CHOMPEdit - Removal of obj/item/device
	SStgui.close_uis(src)

/obj/item/pda/attack_self(mob/user as mob) // CHOMPEdit - Removal of obj/item/device
...
/obj/item/pda/proc/start_program(datum/data/pda/P) // CHOMPEdit - Removal of obj/item/device
	if(P && ((P in programs) || (cartridge && (P in cartridge.programs))))
		return P.start()
	return 0

/obj/item/pda/proc/find_program(type) // CHOMPEdit - Removal of obj/item/device
...
/obj/item/pda/proc/update_shortcuts() // CHOMPEdit - Removal of obj/item/device
	shortcut_cache.Cut()

/obj/item/pda/proc/update_programs() // CHOMPEdit - Removal of obj/item/device
	for(var/datum/data/pda/P as anything in programs)
		P.pda = src

/obj/item/pda/proc/detonate_act(var/obj/item/pda/P) // CHOMPEdit - Removal of obj/item/device
...
/obj/item/pda/proc/remove_id() // CHOMPEdit - Removal of obj/item/device
	if (id)
		if (ismob(loc))
			var/mob/M = loc
			M.put_in_hands(id)
...
/obj/item/pda/proc/remove_pen() // CHOMPEdit - Removal of obj/item/device
	var/obj/item/pen/O = locate() in src // CHOMPEdit - Removal of obj/item/weapon
	if(O)
		if(istype(loc, /mob))
			var/mob/M = loc
...
/obj/item/pda/verb/verb_reset_pda() // CHOMPEdit - Removal of obj/item/device
	set category = "Object"
	set name = "Reset PDA"
	set src in usr
...
/obj/item/pda/verb/verb_remove_id() // CHOMPEdit - Removal of obj/item/device
	set category = "Object"
	set name = "Remove id"
	set src in usr
...
/obj/item/pda/verb/verb_remove_pen() // CHOMPEdit - Removal of obj/item/device
	set category = "Object"
	set name = "Remove pen"
	set src in usr
...
/obj/item/pda/verb/verb_remove_cartridge() // CHOMPEdit - Removal of obj/item/device
	set category = "Object"
...
/obj/item/pda/proc/id_check(mob/user as mob, choice as num)//To check for IDs; 1 for in-pda use, 2 for out of pda use. // CHOMPEdit - Removal of obj/item/device
	if(choice == 1)
		if (id)
			remove_id()
			return 1
			...
			if (istype(I, /obj/item/card/id) && user.unEquip(I)) // CHOMPEdit - Removal of obj/item/weapon
				I.loc = src
				id = I
			return 1
	else
		var/obj/item/card/I = user.get_active_hand() // CHOMPEdit - Removal of obj/item/weapon
		if (istype(I, /obj/item/card/id) && I:registered_name && user.unEquip(I)) // CHOMPEdit - Removal of obj/item/weapon
			var/obj/old_id = id
			I.loc = src
			id = I
...
/obj/item/pda/attackby(obj/item/C as obj, mob/user as mob) // CHOMPEdit - Removal of obj/item/device
	..()
	if(istype(C, /obj/item/cartridge) && !cartridge) // CHOMPEdit - Removal of obj/item/weapon
		cartridge = C
		user.drop_item()
		cartridge.loc = src
		cartridge.update_programs(src)
	...
	else if(istype(C, /obj/item/card/id)) // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/card/id/idcard = C // CHOMPEdit - Removal of obj/item/weapon
	...
	else if(istype(C, /obj/item/paicard) && !src.pai) // CHOMPEdit - Removal of obj/item/device
		user.drop_item()
		C.loc = src
		pai = C
		to_chat(user, "<span class='notice'>You slot \the [C] into \the [src].</span>")
		SStgui.update_uis(src) // update all UIs attached to src
	else if(istype(C, /obj/item/pen)) // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/pen/O = locate() in src // CHOMPEdit - Removal of obj/item/weapon
		if(O)
...
/obj/item/pda/attack(mob/living/C as mob, mob/living/user as mob) // CHOMPEdit - Removal of obj/item/device
	if (istype(C, /mob/living/carbon) && scanmode)
		scanmode.scan_mob(C, user)

/obj/item/pda/afterattack(atom/A as mob|obj|turf|area, mob/user as mob, proximity) // CHOMPEdit - Removal of obj/item/device
	if(proximity && scanmode)
		scanmode.scan_atom(A, user)

/obj/item/pda/proc/explode() //This needs tuning. //Sure did. // CHOMPEdit - Removal of obj/item/device
...
/obj/item/pda/Destroy() // CHOMPEdit - Removal of obj/item/device
	PDAs -= src
...
/obj/item/pda/emp_act(severity) // CHOMPEdit - Removal of obj/item/device
