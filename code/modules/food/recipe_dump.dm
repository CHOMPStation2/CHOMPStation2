/client/proc/recipe_dump()
	...
	for(var/Rp in food_recipes)
		...
		for(var/I in food_recipes[Rp]["Ingredients"])
			var/atom/ing = new I()
			if(istype(ing, /obj/item/reagent_containers/food/snacks)) // only subtypes of this have a coating variable and are checked for it (fruit are a subtype of this, so there's a check for them too later) // CHOMPEdit - Removal of obj/item/weapon
