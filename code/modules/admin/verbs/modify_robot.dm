		...
		switch(modification_choice)
			if(MODIFIY_ROBOT_MODULE_ADD)
				while(TRUE)
					...
					var/obj/item/robot_module/robot/robot_type = new module_type(robot) // CHOMPEdit - Removal of obj/item/weapon
					robot.emag_items = 1
					if(!istype(robot_type, /obj/item/robot_module/robot/)) // CHOMPEdit - Removal of obj/item/weapon
						qdel(robot)
						break
					...
					while(TRUE)
						...
						if(istype(add_item, /obj/item/stack/))
							...
							continue
						if(istype(add_item, /obj/item/matter_decompiler/) || istype(add_item, /obj/item/dogborg/sleeper/compactor/decompiler/)) // CHOMPEdit - Removal of obj/item/weapon // CHOMPEdit - Removal of obj/item/device
							var/obj/item/matter_decompiler/item_with_matter = add_item // CHOMPEdit - Removal of obj/item/weapon
				...
				while(TRUE)
					...
					if(selected_module_upgrade == "Proto-Kinetic Accelerator")
						...
						while(TRUE)
							...
							var/obj/item/gun/energy/kinetic_accelerator/kin = locate() in target.module.modules // CHOMPEdit - Removal of obj/item/weapon
			...
			if(MODIFIY_ROBOT_RADIOC_ADD)
				...
				while(TRUE)
					...
					if(selected_radio_channel == "Raider")
						...
						target.radio.keyslot = new /obj/item/encryptionkey/raider(target) // CHOMPEdit - Removal of obj/item/device
						target.radio.syndie = 1
					if(selected_radio_channel == "Mercenary")
						qdel(target.radio.keyslot)
						target.radio.keyslot = new /obj/item/encryptionkey/syndicate(target) // CHOMPEdit - Removal of obj/item/device
						target.radio.syndie = 1
			...
			if(MODIFIY_ROBOT_TOGGLE_STATION_ACCESS)
				if(target?.idcard?.access)
					var/obj/item/card/id/synthetic/card = target.idcard // CHOMPEdit - Removal of obj/item/weapon
