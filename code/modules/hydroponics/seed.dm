	...
	if(!force_amount && get_trait(TRAIT_YIELD) == 0 && !harvest_sample)
		...
		for(var/i = 0;i<total_yield;i++)
			...
			if(has_mob_product)
				...
				product = new /obj/item/reagent_containers/food/snacks/grown(get_turf(user),name) // CHOMPEdit - Removal of obj/item/weapon
			if (get_trait(TRAIT_PRODUCT_COLOUR))
				if (istype(product,/obj/item/reagent_containers/food)) // CHOMPEdit - Removal of obj/item/weapon
					var/obj/item/reagent_containers/food/food = product // CHOMPEdit - Removal of obj/item/weapon
