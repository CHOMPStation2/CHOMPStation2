...
/obj/machinery/power/shield_generator
	...
	circuit = /obj/item/circuitboard/shield_generator // CHOMPEdit - Removal of obj/item/weapon
	density = TRUE
...
/obj/machinery/power/shield_generator/RefreshParts()
	...
	for(var/obj/item/smes_coil/S in component_parts) // CHOMPEdit - Removal of obj/item/weapon
	...
	mitigation_max = MAX_MITIGATION_BASE + MAX_MITIGATION_RESEARCH * total_component_rating_of_type(/obj/item/stock_parts/capacitor) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/power/shield_generator/attackby(obj/item/O as obj, mob/user as mob)
	...
	if(default_deconstruction_screwdriver(user, O))
		return
	if(O?.has_tool_quality(TOOL_CROWBAR) || O?.has_tool_quality(TOOL_WRENCH) || istype(O, /obj/item/storage/part_replacer)) // CHOMPEdit - Removal of obj/item/weapon
		if(offline_for)
...
/obj/machinery/power/shield_generator/upgraded/Initialize()
	. = ..()
	for(var/obj/item/smes_coil/sc in component_parts) // CHOMPEdit - Removal of obj/item/weapon
		component_parts -= sc
		qdel(sc)

	for(var/obj/item/stock_parts/capacitor/cap in component_parts) // CHOMPEdit - Removal of obj/item/weapon
		component_parts -= cap
		qdel(cap)

	component_parts += new /obj/item/stock_parts/capacitor/hyper(src) // CHOMPEdit - Removal of obj/item/weapon
	component_parts += new /obj/item/smes_coil/super_capacity(src) // CHOMPEdit - Removal of obj/item/weapon
	RefreshParts()
	current_energy = max_energy
