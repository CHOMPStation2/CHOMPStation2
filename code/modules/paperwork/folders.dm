/obj/item/folder // CHOMPEdit - Removal of obj/item/weapon
	name = "folder"
	desc = "A folder."
	icon = 'icons/obj/bureaucracy.dmi' //CHOMPEdit: Continues using new folder sprite, contrary to YW
	icon_state = "folder"
	w_class = ITEMSIZE_SMALL
	pressure_resistance = 2
	drop_sound = 'sound/items/drop/paper.ogg'
	pickup_sound = 'sound/items/pickup/paper.ogg'
	slot_flags = SLOT_BELT | SLOT_HOLSTER

/obj/item/folder/blue // CHOMPEdit - Removal of obj/item/weapon
	desc = "A blue folder."
	icon_state = "folder_blue"

/obj/item/folder/red // CHOMPEdit - Removal of obj/item/weapon
	desc = "A red folder."
	icon_state = "folder_red"

/obj/item/folder/yellow // CHOMPEdit - Removal of obj/item/weapon
	desc = "A yellow folder."
	icon_state = "folder_yellow"

/obj/item/folder/white // CHOMPEdit - Removal of obj/item/weapon
	desc = "A white folder."
	icon_state = "folder_white"

/obj/item/folder/blue_captain // CHOMPEdit - Removal of obj/item/weapon
	desc = "A blue folder with Site Manager markings."
	icon_state = "folder_captain"

/obj/item/folder/blue_hop // CHOMPEdit - Removal of obj/item/weapon
	desc = "A blue folder with HoP markings."
	icon_state = "folder_hop"

/obj/item/folder/white_cmo // CHOMPEdit - Removal of obj/item/weapon
	desc = "A white folder with CMO markings."
	icon_state = "folder_cmo"

/obj/item/folder/white_rd // CHOMPEdit - Removal of obj/item/weapon
	desc = "A white folder with RD markings."
	icon_state = "folder_rd"

/obj/item/folder/white_rd/Initialize() //ChompEDIT New --> Initialize // CHOMPEdit - Removal of obj/item/weapon
	//add some memos
	var/obj/item/paper/P = new() // CHOMPEdit - Removal of obj/item/weapon
	P.name = "Memo RE: proper analysis procedure"
	P.info = "<br>We keep test dummies in pens here for a reason"
	src.contents += P
	update_icon()

/obj/item/folder/yellow_ce // CHOMPEdit - Removal of obj/item/weapon
	desc = "A yellow folder with CE markings."
	icon_state = "folder_ce"

/obj/item/folder/red_hos // CHOMPEdit - Removal of obj/item/weapon
	desc = "A red folder with HoS markings."
	icon_state = "folder_hos"

/obj/item/folder/update_icon() // CHOMPEdit - Removal of obj/item/weapon
	cut_overlays()
	if(contents.len)
		add_overlay("folder_paper")
	return

/obj/item/folder/attackby(obj/item/W as obj, mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if(istype(W, /obj/item/paper) || istype(W, /obj/item/photo) || istype(W, /obj/item/paper_bundle)) // CHOMPEdit - Removal of obj/item/weapon
		user.drop_item()
		W.loc = src
		to_chat(user, "<span class='notice'>You put the [W] into \the [src].</span>")
		update_icon()
	else if(istype(W, /obj/item/pen)) // CHOMPEdit - Removal of obj/item/weapon
		var/n_name = sanitizeSafe(tgui_input_text(user, "What would you like to label the folder?", "Folder Labelling", null, MAX_NAME_LEN), MAX_NAME_LEN)
		if(in_range(user, src) && user.stat == 0)
			name = "folder[(n_name ? text("- '[n_name]'") : null)]"
	return

/obj/item/folder/afterattack(turf/T as turf, mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	for(var/obj/item/paper/P in T) // CHOMPEdit - Removal of obj/item/weapon
		P.loc = src
		update_icon()
		to_chat(user, "<span class='notice'>You tuck the [P] into \the [src].</span>")

/obj/item/folder/attack_self(mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	var/dat = "<title>[name]</title>"

	for(var/obj/item/paper/P in src) // CHOMPEdit - Removal of obj/item/weapon
		dat += "<A href='?src=\ref[src];remove=\ref[P]'>Remove</A> <A href='?src=\ref[src];rename=\ref[P]'>Rename</A> - <A href='?src=\ref[src];read=\ref[P]'>[P.name]</A><BR>"
	for(var/obj/item/photo/Ph in src) // CHOMPEdit - Removal of obj/item/weapon
		dat += "<A href='?src=\ref[src];remove=\ref[Ph]'>Remove</A> <A href='?src=\ref[src];rename=\ref[Ph]'>Rename</A> - <A href='?src=\ref[src];look=\ref[Ph]'>[Ph.name]</A><BR>"
	for(var/obj/item/paper_bundle/Pb in src) // CHOMPEdit - Removal of obj/item/weapon
		dat += "<A href='?src=\ref[src];remove=\ref[Pb]'>Remove</A> <A href='?src=\ref[src];rename=\ref[Pb]'>Rename</A> - <A href='?src=\ref[src];browse=\ref[Pb]'>[Pb.name]</A><BR>"
	user << browse(dat, "window=folder")
	onclose(user, "folder")
	add_fingerprint(user)
	return

/obj/item/folder/Topic(href, href_list) // CHOMPEdit - Removal of obj/item/weapon
	..()
	if((usr.stat || usr.restrained()))
		return

	if(src.loc == usr)

		if(href_list["remove"])
			var/obj/item/P = locate(href_list["remove"])
			if(P && (P.loc == src) && istype(P))
				P.loc = usr.loc
				usr.put_in_hands(P)

		else if(href_list["read"])
			var/obj/item/paper/P = locate(href_list["read"]) // CHOMPEdit - Removal of obj/item/weapon
			if(P && (P.loc == src) && istype(P))
				if(!(istype(usr, /mob/living/carbon/human) || istype(usr, /mob/observer/dead) || istype(usr, /mob/living/silicon)))
					usr << browse("<HTML><HEAD><TITLE>[P.name]</TITLE></HEAD><BODY>[stars(P.info)][P.stamps]</BODY></HTML>", "window=[P.name]")
					onclose(usr, "[P.name]")
				else
					usr << browse("<HTML><HEAD><TITLE>[P.name]</TITLE></HEAD><BODY>[P.info][P.stamps]</BODY></HTML>", "window=[P.name]")
					onclose(usr, "[P.name]")
		else if(href_list["look"])
			var/obj/item/photo/P = locate(href_list["look"]) // CHOMPEdit - Removal of obj/item/weapon
			if(P && (P.loc == src) && istype(P))
				P.show(usr)
		else if(href_list["browse"])
			var/obj/item/paper_bundle/P = locate(href_list["browse"]) // CHOMPEdit - Removal of obj/item/weapon
			if(P && (P.loc == src) && istype(P))
				P.attack_self(usr)
				onclose(usr, "[P.name]")
		else if(href_list["rename"])
			var/obj/item/O = locate(href_list["rename"]) // CHOMPEdit - Removal of obj/item/weapon

			if(O && (O.loc == src))
				if(istype(O, /obj/item/paper)) // CHOMPEdit - Removal of obj/item/weapon
					var/obj/item/paper/to_rename = O // CHOMPEdit - Removal of obj/item/weapon
					to_rename.rename()

				else if(istype(O, /obj/item/photo)) // CHOMPEdit - Removal of obj/item/weapon
					var/obj/item/photo/to_rename = O // CHOMPEdit - Removal of obj/item/weapon
					to_rename.rename()

				else if(istype(O, /obj/item/paper_bundle)) // CHOMPEdit - Removal of obj/item/weapon
					var/obj/item/paper_bundle/to_rename = O // CHOMPEdit - Removal of obj/item/weapon
					to_rename.rename()

		//Update everything
		attack_self(usr)
		update_icon()
	return
