/obj/machinery/photocopier
	...
	power_channel = EQUIP
	circuit = /obj/item/circuitboard/photocopier // CHOMPEdit - Removal of obj/item/weapon
	can_buckle = TRUE
	...
	switch(action)
		...
		if("ai_photo")
			...
			if(toner >= 5)
				...
				var/obj/item/camera/siliconcam/camera = tempAI.aiCamera // CHOMPEdit - Removal of obj/item/device

				if(!camera)
					return
				var/obj/item/photo/selection = camera.selectpicture() // CHOMPEdit - Removal of obj/item/weapon
				if (!selection)
					return

				var/obj/item/photo/p = photocopy(selection) // CHOMPEdit - Removal of obj/item/weapon
				if (p.desc == "")
...
/obj/machinery/photocopier/proc/copy_operation(var/mob/user)
	...
	for(var/i = 0, i < copies, i++)
		...
		if (istype(copyitem, /obj/item/paper)) // CHOMPEdit - Removal of obj/item/weapon
		...
		else if (istype(copyitem, /obj/item/photo)) // CHOMPEdit - Removal of obj/item/weapon
			playsound(src, "sound/machines/copier.ogg", 100, 1)
			sleep(11)
			photocopy(copyitem)
			audible_message("<span class='notice'>You can hear [src] whirring as it finishes printing.</span>", runemessage = "whirr")
			playsound(src, "sound/machines/buzzbeep.ogg", 30)
		else if (istype(copyitem, /obj/item/paper_bundle)) // CHOMPEdit - Removal of obj/item/weapon
			sleep(11)
			playsound(src, "sound/machines/copier.ogg", 100, 1)
			var/obj/item/paper_bundle/B = bundlecopy(copyitem) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/photocopier/attackby(obj/item/O as obj, mob/user as mob)
	if(istype(O, /obj/item/paper) || istype(O, /obj/item/photo) || istype(O, /obj/item/paper_bundle)) // CHOMPEdit - Removal of obj/item/weapon
		if(!copyitem)
			user.drop_item()
			copyitem = O
			O.loc = src
	...
	else if(istype(O, /obj/item/toner)) // CHOMPEdit - Removal of obj/item/device
		if(toner <= 10) //allow replacing when low toner is affecting the print darkness
			user.drop_item()
			to_chat(user, "<span class='notice'>You insert the toner cartridge into \the [src].</span>")
			flick("photocopier_toner", src)
			playsound(loc, 'sound/machines/click.ogg', 50, 1)
			var/obj/item/toner/T = O // CHOMPEdit - Removal of obj/item/device
...
/obj/machinery/photocopier/proc/copy(var/obj/item/paper/copy, var/need_toner=1) // CHOMPEdit - Removal of obj/item/weapon
	var/obj/item/paper/c = new /obj/item/paper (loc) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/photocopier/proc/photocopy(var/obj/item/photo/photocopy, var/need_toner=1) // CHOMPEdit - Removal of obj/item/weapon
	var/obj/item/photo/p = photocopy.copy() // CHOMPEdit - Removal of obj/item/weapon
	p.loc = src.loc
...
/obj/machinery/photocopier/proc/copyass(mob/user)
	...
	if(ishuman(sitter)) // Suit checks are in can_buckle_mobs at the bottom of the file.
		...
		return
	var/obj/item/photo/p = new /obj/item/photo (loc) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/photocopier/proc/bundlecopy(var/obj/item/paper_bundle/bundle, var/need_toner=1) // CHOMPEdit - Removal of obj/item/weapon
	var/obj/item/paper_bundle/p = new /obj/item/paper_bundle (src) // CHOMPEdit - Removal of obj/item/weapon
	for(var/obj/item/W in bundle.pages) // CHOMPEdit - Removal of obj/item/weapon
		...
		if(istype(W, /obj/item/paper)) // CHOMPEdit - Removal of obj/item/weapon
			W = copy(W)
		else if(istype(W, /obj/item/photo)) // CHOMPEdit - Removal of obj/item/weapon
			W = photocopy(W)
		W.loc = p
		p.pages += W
