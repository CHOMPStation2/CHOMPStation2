...
/obj/item/modular_computer/proc/try_install_component(var/mob/living/user, var/obj/item/computer_hardware/H, var/found = 0) // CHOMPEdit - Removal of obj/item/weapon
	// "USB" flash drive.
	if(istype(H, /obj/item/computer_hardware/hard_drive/portable)) // CHOMPEdit - Removal of obj/item/weapon
		if(portable_drive)
			to_chat(user, "This computer's portable drive slot is already occupied by \the [portable_drive].")
			return
		found = 1
		portable_drive = H
	else if(istype(H, /obj/item/computer_hardware/hard_drive)) // CHOMPEdit - Removal of obj/item/weapon
		if(hard_drive)
			to_chat(user, "This computer's hard drive slot is already occupied by \the [hard_drive].")
			return
		found = 1
		hard_drive = H
	else if(istype(H, /obj/item/computer_hardware/network_card)) // CHOMPEdit - Removal of obj/item/weapon
		if(network_card)
			to_chat(user, "This computer's network card slot is already occupied by \the [network_card].")
			return
		found = 1
		network_card = H
	else if(istype(H, /obj/item/computer_hardware/nano_printer)) // CHOMPEdit - Removal of obj/item/weapon
		if(nano_printer)
			to_chat(user, "This computer's nano printer slot is already occupied by \the [nano_printer].")
			return
		found = 1
		nano_printer = H
	else if(istype(H, /obj/item/computer_hardware/card_slot)) // CHOMPEdit - Removal of obj/item/weapon
		if(card_slot)
			to_chat(user, "This computer's card slot is already occupied by \the [card_slot].")
			return
		found = 1
		card_slot = H
	else if(istype(H, /obj/item/computer_hardware/battery_module)) // CHOMPEdit - Removal of obj/item/weapon
		if(battery_module)
			to_chat(user, "This computer's battery slot is already occupied by \the [battery_module].")
			return
		found = 1
		battery_module = H
	else if(istype(H, /obj/item/computer_hardware/processor_unit)) // CHOMPEdit - Removal of obj/item/weapon
		if(processor_unit)
			to_chat(user, "This computer's processor slot is already occupied by \the [processor_unit].")
			return
		found = 1
		processor_unit = H
	else if(istype(H, /obj/item/computer_hardware/tesla_link)) // CHOMPEdit - Removal of obj/item/weapon
		if(tesla_link)
...
/obj/item/modular_computer/proc/uninstall_component(var/mob/living/user, var/obj/item/computer_hardware/H, var/found = 0, var/critical = 0) // CHOMPEdit - Removal of obj/item/weapon
	if(portable_drive == H)
		portable_drive = null
		found = 1
	if(hard_drive == H)
