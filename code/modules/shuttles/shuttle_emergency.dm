	...
	if (current_location == landmark_station)	//leaving the station
		spawn(0)
			...
			if (SSemergency_shuttle.evac)
				...
				GLOB.priority_announcement.Announce(replacetext(replacetext(using_map.shuttle_leaving_dock, "%dock_name%", "[using_map.dock_name]"),  "%ETA%", "[estimated_time] minute\s"), new_sound = ANNOUNCER_MSG_SHUTTLE_ENDROUND_RETURNING) //CHOMP reversion
	..()
