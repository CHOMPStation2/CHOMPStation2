...
/obj/item/spaceflare/attack_self(var/mob/user) // CHOMPEdit - Removal of obj/item/device
	if(!active)
		visible_message("<span class='notice'>[user] pulls the cord, activating the [src].</span>")
		activate()

/obj/item/spaceflare/proc/activate() // CHOMPEdit - Removal of obj/item/device
	if(active)
		return
	var/turf/T = get_turf(src)
	var/mob/M = loc
...
/obj/item/spaceflare/update_icon() // CHOMPEdit - Removal of obj/item/device
	. = ..()
	if(active)
