...
/obj/item/reagent_containers/glass/rag/Initialize() // CHOMPEdit - Removal of obj/item/weapon
	. = ..()
	update_name()

/obj/item/reagent_containers/glass/rag/Destroy() // CHOMPEdit - Removal of obj/item/weapon
	STOP_PROCESSING(SSobj, src) //so we don't continue turning to ash while gc'd
	return ..()

/obj/item/reagent_containers/glass/rag/attack_self(mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if(on_fire)
...
/obj/item/reagent_containers/glass/rag/attackby(obj/item/W, mob/user) // CHOMPEdit - Removal of obj/item/weapon
	if(!on_fire && istype(W, /obj/item/flame)) // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/flame/F = W // CHOMPEdit - Removal of obj/item/weapon
		if(F.lit)
			src.ignite()
			if(on_fire)
...
/obj/item/reagent_containers/glass/rag/proc/update_name() // CHOMPEdit - Removal of obj/item/weapon
	if(on_fire)
...
/obj/item/reagent_containers/glass/rag/update_icon() // CHOMPEdit - Removal of obj/item/weapon
	if(on_fire)
		icon_state = "raglit"
	else
		icon_state = "rag"

	var/obj/item/reagent_containers/food/drinks/bottle/B = loc // CHOMPEdit - Removal of obj/item/weapon
	if(istype(B))
		B.update_icon()

/obj/item/reagent_containers/glass/rag/proc/remove_contents(mob/user, atom/trans_dest = null) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/reagent_containers/glass/rag/proc/wipe_down(atom/A, mob/user) // CHOMPEdit - Removal of obj/item/weapon
	if(!reagents.total_volume)
...
/obj/item/reagent_containers/glass/rag/attack(atom/target as obj|turf|area, mob/user as mob , flag) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/reagent_containers/glass/rag/afterattack(atom/A as obj|turf|area, mob/user as mob, proximity) // CHOMPEdit - Removal of obj/item/weapon
	if(!proximity)
		return

	if(istype(A, /obj/structure/reagent_dispensers) || istype(A, /obj/item/reagent_containers/glass/bucket) || istype(A, /obj/structure/mopbucket))  //VOREStation Edit - "Allows rags to be used on buckets and mopbuckets" // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/reagent_containers/glass/rag/fire_act(datum/gas_mixture/air, exposed_temperature, exposed_volume) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/reagent_containers/glass/rag/proc/can_ignite() // CHOMPEdit - Removal of obj/item/weapon
	var/fuel
...
/obj/item/reagent_containers/glass/rag/proc/ignite() // CHOMPEdit - Removal of obj/item/weapon
	if(on_fire)
		return
	if(!can_ignite())
		return
...
/obj/item/reagent_containers/glass/rag/proc/extinguish() // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/reagent_containers/glass/rag/process() // CHOMPEdit - Removal of obj/item/weapon
	if(!can_ignite())
