...
/obj/item/uv_light/attack_self(var/mob/user) // CHOMPEdit - Removal of obj/item/device
	on = !on
	if(on)
...
/obj/item/uv_light/proc/clear_last_scan() // CHOMPEdit - Removal of obj/item/device
	if(scanned.len)
...
/obj/item/uv_light/process() // CHOMPEdit - Removal of obj/item/device
	clear_last_scan()
	if(on)
