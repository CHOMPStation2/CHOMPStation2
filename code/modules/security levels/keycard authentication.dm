...
GLOBAL_VAR_INIT(maint_all_access, FALSE) // CHOMPEdit - Globals

/proc/make_maint_all_access()
	GLOB.maint_all_access = TRUE // CHOMPEdit - Globals
...
/proc/revoke_maint_all_access()
	GLOB.maint_all_access = FALSE // CHOMPEdit - Globals
	to_world(span_red("<font size=4>Attention!</font>"))
	to_world(span_red("The maintenance access requirement has been readded on all maintenance airlocks."))

/obj/machinery/door/airlock/allowed(mob/M)
	if(GLOB.maint_all_access && src.check_access_list(list(access_maint_tunnels))) // CHOMPEdit - Globals
		return 1
	return ..(M)
