...
/obj/structure/table/attackby(obj/item/W as obj, mob/user as mob, var/hit_modifier, var/click_parameters)
		...
		if (istype(G.affecting, /mob/living))
			...
			if (G.state < 2)
				if(user.a_intent == I_HURT)
					...
					for(var/obj/item/material/shard/S in L) // CHOMPEdit - Removal of obj/item/weapon
						if(prob(50))
	...
	if(istype(W, /obj/item/melee/changeling/arm_blade)) // CHOMPEdit - Removal of obj/item/weapon
		user.visible_message("<span class='danger'>\The [src] was sliced apart by [user]!</span>")
		break_to_parts()
		return
