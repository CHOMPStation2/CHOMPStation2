...
/obj/machinery/clonepod/transhuman
	...
	circuit = /obj/item/circuitboard/transhuman_clonepod // CHOMPEdit - Removal of obj/item/weapon

//A full version of the pod
/obj/machinery/clonepod/transhuman/full/Initialize()
	. = ..()
	for(var/i = 1 to container_limit)
		containers += new /obj/item/reagent_containers/glass/bottle/biomass(src) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/clonepod/transhuman/growclone(var/datum/transhuman/body_record/current_project)
	...
	if(current_project.breath_type != "oxygen")
		...
		var/obj/item/tank/tankpath // CHOMPEdit - Removal of obj/item/weapon
		if(current_project.breath_type == "phoron")
			tankpath = /obj/item/tank/vox // CHOMPEdit - Removal of obj/item/weapon
		else
			tankpath = text2path("/obj/item/tank/" + current_project.breath_type) // CHOMPEdit - Removal of obj/item/weapon

		if(tankpath)
			H.equip_to_slot_or_del(new tankpath(H), slot_back)
			H.internal = H.back
			if(istype(H.internal,/obj/item/tank) && H.internals) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/transhuman/synthprinter
	...
	icon_state = "pod_0"
	circuit = /obj/item/circuitboard/transhuman_synthprinter // CHOMPEdit - Removal of obj/item/weapon
	density = TRUE
	anchored = TRUE
...
/obj/machinery/transhuman/synthprinter/New()
	...
	component_parts += new /obj/item/stock_parts/matter_bin(src) // CHOMPEdit - Removal of obj/item/weapon
	component_parts += new /obj/item/stock_parts/scanning_module(src) // CHOMPEdit - Removal of obj/item/weapon
	component_parts += new /obj/item/stock_parts/manipulator(src) // CHOMPEdit - Removal of obj/item/weapon
	component_parts += new /obj/item/stock_parts/manipulator(src) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/transhuman/synthprinter/RefreshParts()
	...
	for(var/obj/item/stock_parts/scanning_module/SM in component_parts) // CHOMPEdit - Removal of obj/item/weapon
	...
	for(var/obj/item/stock_parts/manipulator/M in component_parts) // CHOMPEdit - Removal of obj/item/weapon
	...
	for(var/obj/item/stock_parts/matter_bin/MB in component_parts) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/transhuman/resleever
	...
	circuit = /obj/item/circuitboard/transhuman_resleever // CHOMPEdit - Removal of obj/item/weapon
	density = TRUE
	opacity = 0
	anchored = TRUE
	var/blur_amount
...
/obj/machinery/transhuman/resleever/New()
	...
	component_parts += new /obj/item/stock_parts/manipulator(src) // CHOMPEdit - Removal of obj/item/weapon
	component_parts += new /obj/item/stock_parts/manipulator(src) // CHOMPEdit - Removal of obj/item/weapon
	component_parts += new /obj/item/stock_parts/console_screen(src) // CHOMPEdit - Removal of obj/item/weapon
	component_parts += new /obj/item/stack/cable_coil(src, 2)
	RefreshParts()
	update_icon()
...
/obj/machinery/transhuman/resleever/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(default_part_replacement(user, W))
		return
	if(istype(W, /obj/item/grab)) // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/grab/G = W // CHOMPEdit - Removal of obj/item/weapon
	...
	if(istype(W, /obj/item/paicard/sleevecard)) // CHOMPEdit - Removal of obj/item/device
		var/obj/item/paicard/sleevecard/C = W // CHOMPEdit - Removal of obj/item/device
		user.unEquip(C)
		C.removePersonality()
		qdel(C)
		sleevecards++
...
/obj/machinery/transhuman/resleever/proc/putmind(var/datum/transhuman/mind_record/MR, mode = 1, var/mob/living/carbon/human/override = null, var/db_key)
	...
	if(mode == 2 && sleevecards) //Card sleeving
		var/obj/item/paicard/sleevecard/card = new /obj/item/paicard/sleevecard(get_turf(src)) // CHOMPEdit - Removal of obj/item/device
	...
	if(MR.nif_path)
		var/obj/item/nif/nif = new MR.nif_path(occupant,null,MR.nif_savedata) // CHOMPEdit - Removal of obj/item/device
	...
	var/obj/item/implant/backup/new_imp = new() // CHOMPEdit - Removal of obj/item/weapon
