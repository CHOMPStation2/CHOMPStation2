...
/obj/item/organ/external/Destroy()
	...
	for(var/obj/item/implant/I as anything in implants) // CHOMPEdit - Removal of obj/item/weapon
		if(!istype(I))
			continue
...
/obj/item/organ/external/attackby(obj/item/W as obj, mob/living/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	switch(stage)
		if(0)
			if(istype(W,/obj/item/surgical/scalpel)) // CHOMPEdit - Removal of obj/item/weapon
				user.visible_message("<span class='danger'><b>[user]</b> cuts [src] open with [W]!</span>")
				stage++
				return
		if(1)
			if(istype(W,/obj/item/surgical/retractor)) // CHOMPEdit - Removal of obj/item/weapon
				user.visible_message("<span class='danger'><b>[user]</b> cracks [src] open like an egg with [W]!</span>")
				stage++
				return
		if(2)
			if(istype(W,/obj/item/surgical/hemostat)) // CHOMPEdit - Removal of obj/item/weapon
				if(contents.len)
...
/obj/item/organ/external/take_damage(brute, burn, sharp, edge, used_weapon = null, list/forbidden_limbs = list(), permutation = 0)
	...
	if(status & ORGAN_BROKEN && brute)
		jostle_bone(brute)
		if(organ_can_feel_pain() && prob(40) && !isbelly(owner.loc) && !istype(owner.loc, /obj/item/dogborg/sleeper)) //VOREStation Edit // CHOMPEdit - Removal of obj/item/device
...
/obj/item/organ/external/rejuvenate(var/ignore_prosthetic_prefs)
	...
	for(var/obj/implanted_object in implants)
		if(istype(implanted_object,/obj/item/implant) || istype(implanted_object,/obj/item/nif))	// We don't want to remove REAL implants. Just shrapnel etc. //VOREStation Edit - NIFs pls // CHOMPEdit - Removal of obj/item/weapon // CHOMPEdit - Removal of obj/item/device
			continue
...
/obj/item/organ/external/proc/droplimb(var/clean, var/disintegrate = DROPLIMB_EDGE, var/ignore_children = null)
	...
	if(victim.l_hand)
		if(istype(victim.l_hand,/obj/item/material/twohanded)) //if they're holding a two-handed weapon, drop it now they've lost a hand // CHOMPEdit - Removal of obj/item/weapon
			victim.l_hand.update_held_icon()
	if(victim.r_hand)
		if(istype(victim.r_hand,/obj/item/material/twohanded)) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/organ/external/proc/embed(var/obj/item/W, var/silent = 0) // CHOMPEdit - Removal of obj/item/weapon
	if(!owner || loc != owner)
		return
	if(!silent)
...
/mob/living/carbon/human/proc/has_embedded_objects()
	...
	for(var/obj/item/organ/external/L in organs)
		for(var/obj/item/I in L.implants)
			if(!istype(I,/obj/item/implant) && !istype(I,/obj/item/nif)) //VOREStation Add - NIFs // CHOMPEdit - Removal of obj/item/weapon // CHOMPEdit - Removal of obj/item/device
				return 1
