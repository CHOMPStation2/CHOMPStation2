...
/obj/item/integrated_circuit/input/externalbm/do_work()
	...
	if(AM)
		...
		if(istype(AM, /obj/item/cell)) // Is this already a cell? // CHOMPEdit - Removal of obj/item/weapon
			cell = AM
		else // If not, maybe there's a cell inside it?
			for(var/obj/item/cell/C in AM.contents) // CHOMPEdit - Removal of obj/item/weapon
