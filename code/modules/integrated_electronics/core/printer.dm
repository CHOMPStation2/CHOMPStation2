...
/obj/item/integrated_circuit_printer // CHOMPEdit - Removal of obj/item/device
...
//	var/static/list/recipe_list = list()
	var/obj/item/electronic_assembly/assembly_to_clone = null // Not implemented x3 // CHOMPEdit - Removal of obj/item/device
	var/dirty_items = FALSE

/obj/item/integrated_circuit_printer/all_upgrades // CHOMPEdit - Removal of obj/item/device
	upgraded = TRUE
	illegal_upgraded = TRUE
	can_clone = TRUE

/obj/item/integrated_circuit_printer/illegal // CHOMPEdit - Removal of obj/item/device
	illegal_upgraded = TRUE
	can_clone = TRUE

/obj/item/integrated_circuit_printer/upgraded // CHOMPEdit - Removal of obj/item/device
	upgraded = TRUE
	can_clone = TRUE

/obj/item/integrated_circuit_printer/debug // CHOMPEdit - Removal of obj/item/device
...
/obj/item/integrated_circuit_printer/attack_robot(mob/user as mob) // CHOMPEdit - Removal of obj/item/device
	if(Adjacent(user))
		return tgui_interact(user)
	else
		return ..()

/obj/item/integrated_circuit_printer/attackby(var/obj/item/O, var/mob/user) // CHOMPEdit - Removal of obj/item/device
	...
	if(istype(O,/obj/item/disk/integrated_circuit/upgrade/advanced)) // CHOMPEdit - Removal of obj/item/weapon
		if(upgraded)
	...
	if(istype(O,/obj/item/disk/integrated_circuit/upgrade/illegal)) // CHOMPEdit - Removal of obj/item/weapon
		if(illegal_upgraded)
			to_chat(user, span("warning", "\The [src] already has this upgrade."))
			return TRUE
		to_chat(user, span("notice", "You install \the [O] into  \the [src]."))
	...
	if(istype(O,/obj/item/disk/integrated_circuit/upgrade/clone)) // CHOMPEdit - Removal of obj/item/weapon
		if(can_clone)
			to_chat(user, span("warning", "\The [src] already has this upgrade."))
			return TRUE
		to_chat(user, span("notice", "You install \the [O] into  \the [src]."))
...
/obj/item/integrated_circuit_printer/vv_edit_var(var_name, var_value) // CHOMPEdit - Removal of obj/item/device
	// Gotta update the static data in case an admin VV's the upgraded var for some reason..!
	if(var_name == "upgraded")
		dirty_items = TRUE
	return ..()

/obj/item/integrated_circuit_printer/attack_self(var/mob/user) // CHOMPEdit - Removal of obj/item/device
	tgui_interact(user)

/obj/item/integrated_circuit_printer/tgui_state(mob/user) // CHOMPEdit - Removal of obj/item/device
	return GLOB.tgui_physical_state

/obj/item/integrated_circuit_printer/tgui_interact(mob/user, datum/tgui/ui) // CHOMPEdit - Removal of obj/item/device
	if(dirty_items)
...
/obj/item/integrated_circuit_printer/tgui_static_data(mob/user) // CHOMPEdit - Removal of obj/item/device
	var/list/data = ..()
	...
	for(var/category in SScircuit.circuit_fabricator_recipe_list)
		...
		for(var/path in circuit_list)
			...
			var/cost = 1
			if(ispath(path, /obj/item/electronic_assembly)) // CHOMPEdit - Removal of obj/item/device
				var/obj/item/electronic_assembly/E = path // CHOMPEdit - Removal of obj/item/device
...
/obj/item/integrated_circuit_printer/tgui_data(mob/user, datum/tgui/ui, datum/tgui_state/state) // CHOMPEdit - Removal of obj/item/device
	var/list/data = ..()
...
/obj/item/integrated_circuit_printer/tgui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state) // CHOMPEdit - Removal of obj/item/device
	if(..())
		return TRUE
	...
	switch(action)
		if("build")
			...
			if(ispath(build_type, /obj/item/electronic_assembly)) // CHOMPEdit - Removal of obj/item/device
				var/obj/item/electronic_assembly/E = build_type // CHOMPEdit - Removal of obj/item/device
				cost = round( (initial(E.max_complexity) + initial(E.max_components) ) / 4)
			else
...
/obj/item/disk/integrated_circuit/upgrade // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/disk/integrated_circuit/upgrade/illegal // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/disk/integrated_circuit/upgrade/clone // CHOMPEdit - Removal of obj/item/weapon
