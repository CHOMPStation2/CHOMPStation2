...
/obj/item/electronic_assembly // CHOMPEdit - Removal of obj/item/device
	...
	var/obj/item/cell/device/battery = null // Internal cell which most circuits need to work. // CHOMPEdit - Removal of obj/item/weapon
	var/net_power = 0 // Set every tick, to display how much power is being drawn in total.
	var/detail_color = COLOR_ASSEMBLY_BLACK


/obj/item/electronic_assembly/Initialize() // CHOMPEdit - Removal of obj/item/device
	battery = new(src)
	START_PROCESSING(SSobj, src)
	return ..()

/obj/item/electronic_assembly/Destroy() // CHOMPEdit - Removal of obj/item/device
	battery = null // It will be qdel'd by ..() if still in our contents
	STOP_PROCESSING(SSobj, src)
	return ..()

/obj/item/electronic_assembly/process() // CHOMPEdit - Removal of obj/item/device
	handle_idle_power()

/obj/item/electronic_assembly/proc/handle_idle_power() // CHOMPEdit - Removal of obj/item/device
...
/obj/item/electronic_assembly/proc/check_interactivity(mob/user) // CHOMPEdit - Removal of obj/item/device
	return tgui_status(user, GLOB.tgui_physical_state) == STATUS_INTERACTIVE

/obj/item/electronic_assembly/get_cell() // CHOMPEdit - Removal of obj/item/device
	return battery

// TGUI
/obj/item/electronic_assembly/tgui_state(mob/user) // CHOMPEdit - Removal of obj/item/device
	return GLOB.tgui_physical_state

/obj/item/electronic_assembly/tgui_interact(mob/user, datum/tgui/ui, datum/tgui/parent_ui) // CHOMPEdit - Removal of obj/item/device
	ui = SStgui.try_update_ui(user, src, ui)
	if(!ui)
		ui = new(user, src, "ICAssembly", name, parent_ui)
		ui.open()

/obj/item/electronic_assembly/tgui_data(mob/user, datum/tgui/ui, datum/tgui_state/state) // CHOMPEdit - Removal of obj/item/device
	var/list/data = ..()
...
/obj/item/electronic_assembly/tgui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state) // CHOMPEdit - Removal of obj/item/device
	if(..())
		return TRUE
	...
	switch(action)
		...
		if("scan_circuit")
			...
			if(!istype(C))
				return
			if(istype(held_item, /obj/item/integrated_electronics/debugger)) // CHOMPEdit - Removal of obj/item/device
				var/obj/item/integrated_electronics/debugger/D = held_item // CHOMPEdit - Removal of obj/item/device
				if(D.accepting_refs)
...
/obj/item/electronic_assembly/verb/rename() // CHOMPEdit - Removal of obj/item/device
...
/obj/item/electronic_assembly/proc/can_move() // CHOMPEdit - Removal of obj/item/device
	return FALSE

/obj/item/electronic_assembly/update_icon() // CHOMPEdit - Removal of obj/item/device
	if(opened)
...
/obj/item/electronic_assembly/GetAccess() // CHOMPEdit - Removal of obj/item/device
	. = list()
	for(var/obj/item/integrated_circuit/part in contents)
		. |= part.GetAccess()

/obj/item/electronic_assembly/GetIdCard() // CHOMPEdit - Removal of obj/item/device
	. = list()
	for(var/obj/item/integrated_circuit/part in contents)
...
/obj/item/electronic_assembly/examine(mob/user) // CHOMPEdit - Removal of obj/item/device
	. = ..()
	if(Adjacent(user))
		for(var/obj/item/integrated_circuit/IC in contents)
...
/obj/item/electronic_assembly/proc/get_part_complexity() // CHOMPEdit - Removal of obj/item/device
	. = 0
	for(var/obj/item/integrated_circuit/part in contents)
		. += part.complexity

/obj/item/electronic_assembly/proc/get_part_size() // CHOMPEdit - Removal of obj/item/device
	. = 0
	for(var/obj/item/integrated_circuit/part in contents)
		. += part.size

// Returns true if the circuit made it inside.
/obj/item/electronic_assembly/proc/add_circuit(var/obj/item/integrated_circuit/IC, var/mob/user) // CHOMPEdit - Removal of obj/item/device
	if(!opened)
...
/obj/item/electronic_assembly/proc/force_add_circuit(var/obj/item/integrated_circuit/IC) // CHOMPEdit - Removal of obj/item/device
	IC.forceMove(src)
	IC.assembly = src

/obj/item/electronic_assembly/afterattack(atom/target, mob/user, proximity) // CHOMPEdit - Removal of obj/item/device
	if(proximity)
		var/scanned = FALSE
...
/obj/item/electronic_assembly/attackby(var/obj/item/I, var/mob/user) // CHOMPEdit - Removal of obj/item/device
	...
	else if(istype(I, /obj/item/integrated_electronics/wirer) || istype(I, /obj/item/integrated_electronics/debugger) || I.has_tool_quality(TOOL_SCREWDRIVER)) // CHOMPEdit - Removal of obj/item/device
		if(opened)
			tgui_interact(user)
			return TRUE
		else
	...
	else if(istype(I, /obj/item/integrated_electronics/detailer)) // CHOMPEdit - Removal of obj/item/device
		var/obj/item/integrated_electronics/detailer/D = I // CHOMPEdit - Removal of obj/item/device
		detail_color = D.detail_color
		update_icon()

	else if(istype(I, /obj/item/cell/device)) // CHOMPEdit - Removal of obj/item/weapon
		if(!opened)
			to_chat(user, "<span class='warning'>\The [src] isn't opened, so you can't put anything inside.  Try using a crowbar.</span>")
			return FALSE
		if(battery)
			...
			return FALSE
		var/obj/item/cell/device/cell = I // CHOMPEdit - Removal of obj/item/weapon
		user.drop_item(cell)
		cell.forceMove(src)
		battery = cell
		playsound(src, 'sound/items/Deconstruct.ogg', 50, 1)
...
/obj/item/electronic_assembly/attack_self(mob/user) // CHOMPEdit - Removal of obj/item/device
...
/obj/item/electronic_assembly/attack_robot(mob/user as mob) // CHOMPEdit - Removal of obj/item/device
	if(Adjacent(user))
		return attack_self(user)
	else
		return ..()

/obj/item/electronic_assembly/emp_act(severity) // CHOMPEdit - Removal of obj/item/device
	..()
	for(var/atom/movable/AM in contents)
		AM.emp_act(severity)

// Returns true if power was successfully drawn.
/obj/item/electronic_assembly/proc/draw_power(amount) // CHOMPEdit - Removal of obj/item/device
	if(battery)
...
// Ditto for giving.
/obj/item/electronic_assembly/proc/give_power(amount) // CHOMPEdit - Removal of obj/item/device
	if(battery)
...
/obj/item/electronic_assembly/proc/on_anchored() // CHOMPEdit - Removal of obj/item/device
	for(var/obj/item/integrated_circuit/IC in contents)
		IC.on_anchored()

/obj/item/electronic_assembly/proc/on_unanchored() // CHOMPEdit - Removal of obj/item/device
	for(var/obj/item/integrated_circuit/IC in contents)
		IC.on_unanchored()
...
/obj/item/electronic_assembly/proc/is_valid_tool(var/obj/item/I) // CHOMPEdit - Removal of obj/item/device
	return I.has_tool_quality(TOOL_CROWBAR) || I.has_tool_quality(TOOL_SCREWDRIVER) || istype(I, /obj/item/integrated_circuit) || istype(I, /obj/item/cell/device) || istype(I, /obj/item/integrated_electronics) // CHOMPEdit - Removal of obj/item/weapon // CHOMPEdit - Removal of obj/item/device
