...
/obj/structure/noticeboard/Initialize()
	...
	notices = list()
	for(var/obj/item/paper/note in get_turf(src)) // CHOMPEdit - Removal of obj/item/weapon
		note.forceMove(src)
		LAZYADD(notices, note)
		if(LAZYLEN(notices) >= max_notices)
			break
	//ChompEDIT START - notices in contents
	for(var/obj/item/paper/note in contents) // CHOMPEdit - Removal of obj/item/weapon
		LAZYADD(notices, note)
		if(LAZYLEN(notices) >= max_notices)
			break
	//ChompEDIT END
...
/obj/structure/noticeboard/attackby(obj/item/I, mob/user)
	if(I.has_tool_quality(TOOL_SCREWDRIVER))
		...
		if(do_after(user, 50, src))
			...
			dismantle()
		return
	else if(istype(I, /obj/item/paper) || istype(I, /obj/item/photo)) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/structure/noticeboard/tgui_data(mob/user)
	...
	for(var/obj/item/I in src.notices)
		tgui_notices.Add(list(list(
			"ispaper" = istype(I, /obj/item/paper), // CHOMPEdit - Removal of obj/item/weapon
			"isphoto" = istype(I, /obj/item/photo), // CHOMPEdit - Removal of obj/item/weapon
			"name" = I.name,
			"ref" = "\ref[I]",
		)))
...
/obj/structure/noticeboard/tgui_act(action, params)
	...
	switch(action)
		if("read")
			var/obj/item/paper/P = locate(params["ref"]) // CHOMPEdit - Removal of obj/item/weapon
			if(P && P.loc == src)
				P.show_content(usr)
			. = TRUE

		if("look")
			var/obj/item/photo/P = locate(params["ref"]) // CHOMPEdit - Removal of obj/item/weapon
			if(P && P.loc == src)
				P.show(usr)
			. = TRUE
		...
		if("write")
			...
			if((P && P.loc == src)) //if the paper's on the board
				...
				if(istype(M))
					var/obj/item/pen/E = M.get_type_in_hands(/obj/item/pen) // CHOMPEdit - Removal of obj/item/weapon
					if(E)
						add_fingerprint(M)
						P.attackby(E, usr)
					else
...
/obj/structure/noticeboard/anomaly/Initialize() //ChompEDIT New --> Initialize
	...
	P.stamped = list(/obj/item/stamp/rd) // CHOMPEdit - Removal of obj/item/weapon
