...
/datum/persistent/storage/smartfridge/produce/create_item(var/seedtype)
	return new /obj/item/reagent_containers/food/snacks/grown(null, seedtype) // Smartfridge will be stock()ed with it, loc is unimportant // CHOMPEdit - Removal of obj/item/weapon
...
/datum/persistent/storage/smartfridge/produce/get_storage_list(var/obj/machinery/smartfridge/produce/entry)
	...
	for(var/datum/stored_item/I in entry.item_records)
		...
		if(LAZYLEN(I.instances))
			var/obj/item/reagent_containers/food/snacks/grown/G = I.instances[1] // CHOMPEdit - Removal of obj/item/weapon
			if(!istype(G))
				continue
