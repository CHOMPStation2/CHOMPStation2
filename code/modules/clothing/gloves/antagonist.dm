...
/obj/item/clothing/gloves/sterile/thieves/proc/pickpocket(var/mob/living/carbon/human/user, var/mob/living/carbon/human/target, var/proximity)
	...
	if(user.a_intent == I_HELP)
		if(istype(target.back,/obj/item/storage) && do_after(user, 3 SECONDS, target)) // CHOMPEdit - Removal of obj/item/weapon
			var/obj/item/storage/Backpack = target.back // CHOMPEdit - Removal of obj/item/weapon
			Backpack.open(user)
		else if(istype(target.belt, /obj/item/storage) && do_after(user, 5 SECONDS, target)) // CHOMPEdit - Removal of obj/item/weapon
			var/obj/item/storage/Belt = target.belt // CHOMPEdit - Removal of obj/item/weapon
			Belt.open(user)
		return 1
...
/obj/item/clothing/gloves/ring/buzzer
	...
	icon_state = "material"
	var/battery_type = /obj/item/cell/device/weapon/recharge // CHOMPEdit - Removal of obj/item/weapon
