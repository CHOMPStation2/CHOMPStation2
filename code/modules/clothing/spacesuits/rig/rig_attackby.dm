/obj/item/rig/attackby(obj/item/W as obj, mob/living/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if(!istype(user))
		return 0
	...
	if(open)
		// Hacking.
		if(W.has_tool_quality(TOOL_WIRECUTTER) || istype(W, /obj/item/multitool)) // CHOMPEdit - Removal of obj/item/device
			if(open)
				wires.Interact(user)
			else
		...
		else if(!cell && istype(W,/obj/item/cell)) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/rig/attack_hand(var/mob/user) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/rig/emag_act(var/remaining_charges, var/mob/user) // CHOMPEdit - Removal of obj/item/weapon
	if(!subverted)
