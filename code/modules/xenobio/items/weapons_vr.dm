/obj/item/xenobio // CHOMPEdit - Removal of obj/item/weapon
	name = "xenobio gun"
	...
	var/firable = TRUE
/obj/item/xenobio/examine(var/mob/user) // CHOMPEdit - Removal of obj/item/weapon
	. = ..()
	if(loaded_item)
...
/obj/item/xenobio/attack_hand(mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/xenobio/attackby(obj/item/I as obj, mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/xenobio/attack_self(mob/living/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if(loaded_item)
		user.put_in_hands(loaded_item)
		user.visible_message("<span class='notice'>[user] removes [loaded_item] from [src].</span>", "<span class='notice'>You remove [loaded_item] from [src].</span>")
		loaded_item = null
...
/obj/item/xenobio/afterattack(atom/A, mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if(!loaded_item)
...
/obj/item/xenobio/monkey_gun // CHOMPEdit - Removal of obj/item/weapon
	...
	loadable_item = /obj/item/reagent_containers/food/snacks/monkeycube // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/xenobio/monkey_gun/afterattack(atom/A, mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	..()
	...
	if(istype(A, /mob/living))
		...
		return
	if(loaded_item)
		var/obj/item/reagent_containers/food/snacks/monkeycube/cube = loaded_item // CHOMPEdit - Removal of obj/item/weapon
		cube.loc = A
		cube.Expand()
		loaded_item = null
		firable = FALSE
...
/obj/item/slime_grinder // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/slime_grinder/proc/extract(var/atom/movable/AM, var/mob/living/user) // CHOMPEdit - Removal of obj/item/weapon
	processing = TRUE
	...
	if(istype(AM, /mob/living/carbon/human/monkey))
		...
		while(monkeys_recycled >= 4)
			new /obj/item/reagent_containers/food/snacks/monkeycube(get_turf(src)) // CHOMPEdit - Removal of obj/item/weapon
			playsound(src, 'sound/effects/splat.ogg', 50, 1)
			monkeys_recycled -= 4
			sleep(1 SECOND)
	processing = FALSE

/obj/item/slime_grinder/proc/can_insert(var/atom/movable/AM) // CHOMPEdit - Removal of obj/item/weapon
	if(istype(AM, /mob/living/simple_mob/slime))
		var/mob/living/simple_mob/slime/S = AM
		if(S.stat != DEAD)
			return FALSE
...
/obj/item/slime_grinder/attack(mob/M as mob, mob/living/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if(processing)
		return
	if(!can_insert(M))
