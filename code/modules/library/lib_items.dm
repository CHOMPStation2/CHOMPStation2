...
/obj/structure/bookcase/attackby(obj/item/O as obj, mob/user as mob)
	if(istype(O, /obj/item/book)) // CHOMPEdit - Removal of obj/item/weapon
		user.drop_item()
		O.loc = src
		update_icon()
	else if(istype(O, /obj/item/pen)) // CHOMPEdit - Removal of obj/item/weapon
		...
		if(do_after(user,25 * O.toolspeed))
			...
			for(var/obj/item/book/b in contents) // CHOMPEdit - Removal of obj/item/weapon
				b.loc = (get_turf(src))
			qdel(src)
...
/obj/structure/bookcase/attack_hand(var/mob/user as mob)
	if(contents.len)
		var/obj/item/book/choice = tgui_input_list(usr, "Which book would you like to remove from the shelf?", "Book Selection", contents) // CHOMPEdit - Removal of obj/item/weapon
		if(choice)
...
/obj/structure/bookcase/manuals/medical/New()
	..()
	new /obj/item/book/manual/medical_cloning(src) // CHOMPEdit - Removal of obj/item/weapon
	new /obj/item/book/manual/wiki/medical_diagnostics_manual(src) // CHOMPEdit // CHOMPEdit - Removal of obj/item/weapon
	new /obj/item/book/manual/wiki/medical_diagnostics_manual(src) // CHOMPEdit // CHOMPEdit - Removal of obj/item/weapon
	new /obj/item/book/manual/wiki/medical_diagnostics_manual(src) // CHOMPEdit // CHOMPEdit - Removal of obj/item/weapon
	update_icon()
...
/obj/structure/bookcase/manuals/engineering/New()
	..()
	new /obj/item/book/manual/wiki/engineering_construction(src) // CHOMPEdit // CHOMPEdit - Removal of obj/item/weapon
	new /obj/item/book/manual/engineering_particle_accelerator(src) // CHOMPEdit - Removal of obj/item/weapon
	new /obj/item/book/manual/wiki/engineering_hacking(src) // CHOMPEdit // CHOMPEdit - Removal of obj/item/weapon
	new /obj/item/book/manual/wiki/engineering_guide(src) // CHOMPEdit // CHOMPEdit - Removal of obj/item/weapon
	new /obj/item/book/manual/atmospipes(src) // CHOMPEdit - Removal of obj/item/weapon
	new /obj/item/book/manual/engineering_singularity_safety(src) // CHOMPEdit - Removal of obj/item/weapon
	new /obj/item/book/manual/evaguide(src) // CHOMPEdit - Removal of obj/item/weapon
	update_icon()
...
/obj/structure/bookcase/manuals/research_and_development/New()
	..()
	new /obj/item/book/manual/research_and_development(src) // CHOMPEdit - Removal of obj/item/weapon
	update_icon()
...
/obj/item/book/attack_self(var/mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if(carved)
		if(store)
...
/obj/item/book/proc/display_content(mob/living/user) // CHOMPEdit - Removal of obj/item/weapon
	user << browse("<TT><I>Penned by [author].</I></TT> <BR>" + "[dat]", "window=book")
// CHOMPEdit End

/obj/item/book/attackby(obj/item/W as obj, mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if(carved)
		if(!store)
	...
	if(istype(W, /obj/item/pen)) // CHOMPEdit - Removal of obj/item/weapon
		...
		switch(choice)
			...
			if("Author")
				...
				return
	else if(istype(W, /obj/item/barcodescanner)) // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/barcodescanner/scanner = W // CHOMPEdit - Removal of obj/item/weapon
		if(!scanner.computer)
			...
			switch(scanner.mode)
				...
				if(3)
					scanner.book = src
					for(var/obj/item/book in scanner.computer.inventory) // CHOMPEdit - Removal of obj/item/weapon
						if(book == src)
	...
	else if(istype(W, /obj/item/material/knife) || W.has_tool_quality(TOOL_WIRECUTTER)) // CHOMPEdit - Removal of obj/item/weapon
		if(carved)	return
		to_chat(user, "<span class='notice'>You begin to carve out [title].</span>")
		if(do_after(user, 30))
			to_chat(user, "<span class='notice'>You carve out the pages from [title]! You didn't want to read it anyway.</span>")
			playsound(src, 'sound/bureaucracy/papercrumple.ogg', 50, 1)
			new /obj/item/shreddedp(get_turf(src)) // CHOMPEdit - Removal of obj/item/weapon
			carved = 1
			return
	else
		..()

/obj/item/book/attack(mob/living/carbon/M as mob, mob/living/carbon/user as mob) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/book/bundle // CHOMPEdit - Removal of obj/item/weapon
	var/page = 1 //current page
	var/list/pages = list() //the contents of each page

/obj/item/book/bundle/proc/show_content(mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if(!pages.len)
		return
	var/dat
	var/obj/item/W = pages[page] // CHOMPEdit - Removal of obj/item/weapon
	// first
	if(page == 1)
	...
	// middle pages
		...
		dat+= "<DIV STYLE='float:left; text-align:left; width:33.33333%'><A href='?src=\ref[src];prev_page=1'>Previous Page</A></DIV>"
		dat+= "<DIV STYLE='float:right; text-align:right; width:33.33333%'><A href='?src=\ref[src];next_page=1'>Next Page</A></DIV><BR><HR>"
	if(istype(pages[page], /obj/item/paper)) // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/paper/P = W // CHOMPEdit - Removal of obj/item/weapon
		if(!(istype(usr, /mob/living/carbon/human) || isobserver(usr) || istype(usr, /mob/living/silicon)))
			dat += "<HTML><HEAD><TITLE>[P.name]</TITLE></HEAD><BODY>[stars(P.info)][P.stamps]</BODY></HTML>"
		else
			dat += "<HTML><HEAD><TITLE>[P.name]</TITLE></HEAD><BODY>[P.info][P.stamps]</BODY></HTML>"
		user << browse(dat, "window=[name]")
	else if(istype(pages[page], /obj/item/photo)) // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/photo/P = W // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/book/bundle/attack_self(mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	src.show_content(user)
	add_fingerprint(usr)
	update_icon()
	return

/obj/item/book/bundle/Topic(href, href_list) // CHOMPEdit - Removal of obj/item/weapon
	if(..())
		return 1
	if((src in usr.contents) || (istype(src.loc, /obj/item/folder) && (src.loc in usr.contents))) // CHOMPEdit - Removal of obj/item/weapon
		usr.set_machine(src)
 ...
 * Barcode Scanner
 */
/obj/item/barcodescanner // CHOMPEdit - Removal of obj/item/weapon
	...
	var/obj/item/book/book	 //  Currently scanned book // CHOMPEdit - Removal of obj/item/weapon
	var/mode = 0 					// 0 - Scan only, 1 - Scan and Set Buffer, 2 - Scan and Attempt to Check In, 3 - Scan and Attempt to Add to Inventory

/obj/item/barcodescanner/attack_self(mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	mode += 1
	if(mode > 3)
		mode = 0
