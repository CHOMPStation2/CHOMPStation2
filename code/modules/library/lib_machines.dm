...
/obj/machinery/librarycomp/Initialize()
	...
	if(!all_books || !all_books.len)
		...
		for(var/path in subtypesof(/obj/item/book/codex/lore)) // CHOMPEdit - Removal of obj/item/weapon
			var/obj/item/book/C = new path(null) // CHOMPEdit - Removal of obj/item/weapon
			all_books[C.name] = C

		for(var/path in subtypesof(/obj/item/book/custom_library) - base_genre_books) // CHOMPEdit - Removal of obj/item/weapon
			var/obj/item/book/B = new path(null) // CHOMPEdit - Removal of obj/item/weapon
			all_books[B.title] = B

		for(var/path in subtypesof(/obj/item/book/bundle/custom_library) - base_genre_books) // CHOMPEdit - Removal of obj/item/weapon
			var/obj/item/book/M = new path(null) // CHOMPEdit - Removal of obj/item/weapon
			all_books[M.title] = M
	...
	switch(screenstate)
		if(0)
			...
			if(src.arcanecheckout)
				new /obj/item/book/tome(src.loc) // CHOMPEdit - Removal of obj/item/weapon
		...
		if(1)
			...
			for(var/obj/item/book/b in inventory) // CHOMPEdit - Removal of obj/item/weapon
		...
		if(4)
			...
			if(!all_books || !all_books.len)
				...
				for(var/name in all_books)
					var/obj/item/book/masterbook = all_books[name] // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/librarycomp/attackby(obj/item/W as obj, mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if(istype(W, /obj/item/barcodescanner)) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/librarycomp/Topic(href, href_list)
	...
	if(href_list["switchscreen"])
		switch(href_list["switchscreen"])
			...
			if("6")
				if(!bibledelay)
					new /obj/item/storage/bible(src.loc) // CHOMPEdit - Removal of obj/item/weapon
					bibledelay = 1
					spawn(60)
						bibledelay = 0
	...
	if(href_list["delbook"])
		var/obj/item/book/b = locate(href_list["delbook"]) // CHOMPEdit - Removal of obj/item/weapon
		inventory.Remove(b)
	if(href_list["setauthor"])
	...
	if(href_list["targetid"])
		...
		if(bibledelay)
			...
			while(query.NextRow())
				...
				var/obj/item/book/B = new(src.loc) // CHOMPEdit - Removal of obj/item/weapon
				B.name = "Book: [title]"
				B.title = title
				B.author = author
				B.dat = content
	...
	if(href_list["hardprint"])
		...
		var/obj/item/book/NewBook = new newpath(get_turf(src)) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/libraryscanner
	...
	anchored = TRUE
	density = TRUE
	var/obj/item/book/cache		// Last scanned book // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/bookbinder/attackby(var/obj/O as obj, var/mob/user as mob)
	if(istype(O, /obj/item/paper) || istype(O, /obj/item/paper_bundle)) // CHOMPEdit - Removal of obj/item/weapon
		if(istype(O, /obj/item/paper)) // CHOMPEdit - Removal of obj/item/weapon
			...
			sleep(rand(300,500))
			src.visible_message("[src] whirs as it prints and binds a new book.")
			var/obj/item/book/bundle/b = new(src.loc) // CHOMPEdit - Removal of obj/item/weapon
			b.pages = O:pages
			for(var/obj/item/paper/P in O.contents) // CHOMPEdit - Removal of obj/item/weapon
				P.forceMove(b)
			for(var/obj/item/photo/P in O.contents) // CHOMPEdit - Removal of obj/item/weapon
				P.forceMove(b)
			b.name = "Print Job #" + "[rand(100, 999)]"
			b.icon_state = "book[rand(1,7)]"
			qdel(O)
