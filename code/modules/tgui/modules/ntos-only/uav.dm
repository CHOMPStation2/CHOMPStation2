/datum/tgui_module/uav
	...
	tgui_id = "UAV"
	ntos = TRUE
	var/obj/item/uav/current_uav = null //The UAV we're watching // CHOMPEdit - Removal of obj/item/device
...
/datum/tgui_module/uav/tgui_data(mob/user, datum/tgui/ui, datum/tgui_state/state)
	...
	if(istype(mc_host))
		for(var/datum/weakref/wr as anything in mc_host.paired_uavs)
			var/obj/item/uav/U = wr.resolve() // CHOMPEdit - Removal of obj/item/device
...
/datum/tgui_module/uav/tgui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state)
	...
	switch(action)
		if("switch_uav")
			var/obj/item/uav/U = locate(params["switch_uav"]) //This is a \ref to the UAV itself // CHOMPEdit - Removal of obj/item/device
			if(!istype(U))
...
/datum/tgui_module/uav/proc/set_current(var/obj/item/uav/U) // CHOMPEdit - Removal of obj/item/device
	if(current_uav == U)
		return
