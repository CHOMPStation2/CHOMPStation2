...
/datum/nifsoft/commlink/uninstall()
	var/obj/item/nif/lnif = nif //Awkward. Parent clears it in an attempt to clean up. // CHOMPEdit - Removal of obj/item/device
	if((. = ..()) && lnif)
		QDEL_NULL(lnif.comm)
...
/obj/item/communicator/commlink // CHOMPEdit - Removal of obj/item/device
	name = "commlink"
...
/obj/item/communicator/commlink/New(var/newloc,var/soft) // CHOMPEdit - Removal of obj/item/device
	..()
	nif = newloc
	nifsoft = soft

/obj/item/communicator/commlink/Destroy() // CHOMPEdit - Removal of obj/item/device
	if(nif)
		nif.comm = null
		nif = null
	nifsoft = null
...
/obj/item/communicator/commlink/register_device(var/new_name) // CHOMPEdit - Removal of obj/item/device
	owner = new_name
	name = "[owner]'s [initial(name)]"
	nif.save_data["commlink_name"] = owner

//So that only the owner's chat is relayed to others.
/obj/item/communicator/commlink/hear_talk(mob/living/M, list/message_pieces, verb) // CHOMPEdit - Removal of obj/item/device
	if(M != nif.human)
		return
...
/obj/item/communicator/commlink/show_message(msg, type, alt, alt_type) // CHOMPEdit - Removal of obj/item/device
	return

//The silent treatment
/obj/item/communicator/commlink/request(var/atom/candidate) // CHOMPEdit - Removal of obj/item/device
...
//Similar reason
/obj/item/communicator/commlink/request_im(var/atom/candidate, var/origin_address, var/text) // CHOMPEdit - Removal of obj/item/device
	var/who = null
	if(isobserver(candidate))
