...
/obj/machinery/mineral/processing_unit_console
	...
	var/obj/item/card/id/inserted_id	// Inserted ID card, for points // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/mineral/processing_unit_console/tgui_act(action, list/params)
	...
	switch(action)
		...
		if("claim")
			...
			. = TRUE
		if("insert")
			var/obj/item/card/id/I = usr.get_active_hand() // CHOMPEdit - Removal of obj/item/weapon
			if(istype(I))
				usr.drop_item()
				I.forceMove(src)
				inserted_id = I
...
/obj/machinery/mineral/processing_unit/process()
	...
	for(var/obj/item/ore/O in input.loc) // CHOMPEdit - Removal of obj/item/weapon
	...
	for(var/metal in ores_stored)
		...
		if(ores_stored[metal] > 0 && ores_processing[metal] != 0)
			...
			if(ores_processing[metal] == PROCESS_ALLOY && O.alloy) //Alloying.
				...
				ores_stored[metal]--
				sheets++
				new /obj/item/ore/slag(output.loc) // CHOMPEdit - Removal of obj/item/weapon
		else
			continue
