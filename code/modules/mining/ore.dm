/obj/item/ore // CHOMPEdit - Removal of obj/item/weapon
	name = "small rock"
	icon = 'icons/obj/mining.dmi'
	icon_state = "ore2"
	randpixel = 8
	w_class = ITEMSIZE_SMALL
	var/datum/geosample/geologic_data
	var/material

/obj/item/ore/uranium // CHOMPEdit - Removal of obj/item/weapon
	name = "pitchblende"
	icon_state = "ore_uranium"
	origin_tech = list(TECH_MATERIAL = 5)
	material = "uranium"

/obj/item/ore/iron // CHOMPEdit - Removal of obj/item/weapon
	name = "hematite"
	icon_state = "ore_iron"
	origin_tech = list(TECH_MATERIAL = 1)
	material = "hematite"

/obj/item/ore/coal // CHOMPEdit - Removal of obj/item/weapon
	name = "raw carbon"
	icon_state = "ore_coal"
	origin_tech = list(TECH_MATERIAL = 1)
	material = "carbon"

/obj/item/ore/marble // CHOMPEdit - Removal of obj/item/weapon
	name = "recrystallized carbonate"
	icon_state = "ore_marble"
	origin_tech = list(TECH_MATERIAL = 1)
	material = "marble"

/obj/item/ore/glass // CHOMPEdit - Removal of obj/item/weapon
	name = "sand"
	icon_state = "ore_glass"
	origin_tech = list(TECH_MATERIAL = 1)
	material = "sand"
	slot_flags = SLOT_HOLSTER

// POCKET SAND!
/obj/item/ore/glass/throw_impact(atom/hit_atom) // CHOMPEdit - Removal of obj/item/weapon
	..()
	var/mob/living/carbon/human/H = hit_atom
	if(istype(H) && H.has_eyes() && prob(85))
		to_chat(H, "<span class='danger'>Some of \the [src] gets in your eyes!</span>")
		H.Blind(5)
		H.eye_blurry += 10
		spawn(1)
			if(istype(loc, /turf/)) qdel(src)


/obj/item/ore/phoron // CHOMPEdit - Removal of obj/item/weapon
	name = "phoron crystals"
	icon_state = "ore_phoron"
	origin_tech = list(TECH_MATERIAL = 2)
	material = "phoron"

/obj/item/ore/silver // CHOMPEdit - Removal of obj/item/weapon
	name = "native silver ore"
	icon_state = "ore_silver"
	origin_tech = list(TECH_MATERIAL = 3)
	material = "silver"

/obj/item/ore/gold // CHOMPEdit - Removal of obj/item/weapon
	name = "native gold ore"
	icon_state = "ore_gold"
	origin_tech = list(TECH_MATERIAL = 4)
	material = "gold"

/obj/item/ore/diamond // CHOMPEdit - Removal of obj/item/weapon
	name = "diamonds"
	icon_state = "ore_diamond"
	origin_tech = list(TECH_MATERIAL = 6)
	material = "diamond"

/obj/item/ore/osmium // CHOMPEdit - Removal of obj/item/weapon
	name = "raw platinum"
	icon_state = "ore_platinum"
	material = "platinum"

/obj/item/ore/hydrogen // CHOMPEdit - Removal of obj/item/weapon
	name = "raw hydrogen"
	icon_state = "ore_hydrogen"
	material = "mhydrogen"

/obj/item/ore/verdantium // CHOMPEdit - Removal of obj/item/weapon
	name = "verdantite dust"
	icon_state = "ore_verdantium"
	material = MAT_VERDANTIUM
	origin_tech = list(TECH_MATERIAL = 7)

// POCKET ... Crystal dust.
/obj/item/ore/verdantium/throw_impact(atom/hit_atom) // CHOMPEdit - Removal of obj/item/weapon
	..()
	var/mob/living/carbon/human/H = hit_atom
	if(istype(H) && H.has_eyes() && prob(85))
		to_chat(H, "<span class='danger'>Some of \the [src] gets in your eyes!</span>")
		H.Blind(10)
		H.eye_blurry += 15
		spawn(1)
			if(istype(loc, /turf/)) qdel(src)

/obj/item/ore/lead // CHOMPEdit - Removal of obj/item/weapon
	name = "lead glance"
	icon_state = "ore_lead"
	material = MAT_LEAD
	origin_tech = list(TECH_MATERIAL = 3)
/*
/obj/item/ore/copper // CHOMPEdit - Removal of obj/item/weapon
	name = "raw copper"
	icon_state = "ore_copper"
	material = "copper"

/obj/item/ore/tin // CHOMPEdit - Removal of obj/item/weapon
	name = "raw tin"
	icon_state = "ore_tin"
	material = "tin"

/obj/item/ore/bauxite // CHOMPEdit - Removal of obj/item/weapon
	name = "raw bauxite"
	icon_state = "ore_bauxite"
	material = "bauxite"
*/
/obj/item/ore/rutile // CHOMPEdit - Removal of obj/item/weapon
	name = "raw rutile"
	icon_state = "ore_rutile"
	material = "rutile"
/*
/obj/item/ore/void_opal // CHOMPEdit - Removal of obj/item/weapon
	name = "raw void opal"
	icon_state = "ore_void_opal"
	material = "void opal"

/obj/item/ore/painite // CHOMPEdit - Removal of obj/item/weapon
	name = "raw painite"
	icon_state = "ore_painite"
	material = "painite"

/obj/item/ore/quartz // CHOMPEdit - Removal of obj/item/weapon
	name = "raw quartz"
	icon_state = "ore_quartz"
	material = "quartz"
*/
/obj/item/ore/slag // CHOMPEdit - Removal of obj/item/weapon
	name = "Slag"
	desc = "Someone screwed up..."
	icon_state = "slag"
	material = null

/obj/item/ore/Initialize() // CHOMPEdit - Removal of obj/item/weapon
	. = ..()
	randpixel_xy()

/obj/item/ore/attackby(obj/item/W as obj, mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if(istype(W,/obj/item/core_sampler)) // CHOMPEdit - Removal of obj/item/device
		var/obj/item/core_sampler/C = W // CHOMPEdit - Removal of obj/item/device
		C.sample_item(src, user)
	else
		return ..()

//VOREStation Add
/obj/item/ore/attack(mob/living/M as mob, mob/living/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if(M.handle_eat_minerals(src, user))
		return
	..()

/obj/item/ore/attack_generic(var/mob/living/user) //Allow adminbussed mobs to eat ore if they click it while NOT on help intent. // CHOMPEdit - Removal of obj/item/weapon
	if(user.handle_eat_minerals(src))
		return
	..()
//VOREStation Add End

/obj/item/ore_chunk
	name = "ore chunk"
	desc = "A conglomerate of ore."
	icon = 'icons/obj/xenoarchaeology.dmi' //CHOMPEdit
	icon_state = "strange"
	randpixel = 8
	w_class = ITEMSIZE_LARGE //CHOMPEdit
	var/list/stored_ore = list(
		"sand" = 0,
		"hematite" = 0,
		"carbon" = 0,
		"raw copper" = 0,
		"raw tin" = 0,
		"void opal" = 0,
		"painite" = 0,
		"quartz" = 0,
		"raw bauxite" = 0,
		"phoron" = 0,
		"silver" = 0,
		"gold" = 0,
		"marble" = 0,
		"uranium" = 0,
		"diamond" = 0,
		"platinum" = 0,
		"lead" = 0,
		"mhydrogen" = 0,
		"verdantium" = 0,
		"rutile" = 0)

/obj/item/ore_chunk/examine(mob/user)
	. = ..()

	if(!Adjacent(user)) //Can only check the contents of ore boxes if you can physically reach them.
		return .

	add_fingerprint(user) //You pick it up to look at it.

	. += "It is composed of:"
	var/has_ore = 0
	for(var/ore in stored_ore)
		if(stored_ore[ore] > 0)
			. += "- [stored_ore[ore]] [ore]"
			has_ore = 1
	if(!has_ore)
		. += "Nothing. You should contact a developer."