/obj/item/weapon/pickaxe/heavydutydrill //AEIOU project
	...
	digspeed = 120			//VERY SLOW WHEN NOT RUNNING. Digspeed is a delay. Lower is better.
...
/obj/item/weapon/pickaxe/heavydutydrill/proc/turnOn(mob/user as mob)
	...
	if(max_fuel <= 0)
		...
		if(do_after(user, 15))
			...
			playsound(src, 'sound/weapons/chainsaw_startup.ogg',40,3)
			force = active_force
			edge = 1
			sharp = 1
			on = 1
...
/obj/item/weapon/pickaxe/heavydutydrill/proc/turnOff(mob/user as mob)
	...
	playsound(user, 'sound/weapons/chainsaw_turnoff.ogg',30,3)
...
/obj/item/weapon/pickaxe/heavydutydrill/afterattack(atom/A as mob|obj|turf|area, mob/user as mob, proximity)
	...
	..()
	if(on)
		playsound(src, 'sound/weapons/chainsaw_attack.ogg',25,3)
	if(A && on)
		if(get_fuel() > 0)
	...
	if(istype(A, /obj/structure/reagent_dispensers/fueltank) && get_dist(src,A) <= 1)
		...
		if(do_after(usr, 15))
			...
			playsound(src.loc, 'sound/effects/refill.ogg', 40, 1, -6)
...
/obj/item/weapon/pickaxe/heavydutydrill/attackby(obj/item/weapon/W, mob/user)
	if(on) //Don't work on active machinery!
		enginefail()
	...
	if(open)
		...
		if(istype(W, /obj/item/drillparts/drillbit))
			if(!drill_bit)
				...
				user << "<span class='notice'>You install \the [W] in [src].</span>"
				update_icon()
				//active_digspeed = W.sharpness
			else
				user << "<span class='notice'>[src] already has a drill tip.</span>"
			return
...
/obj/item/weapon/pickaxe/heavydutydrill/proc/enginefail()//This is the process for a jammed engine. You need to activate it in hand to solve the issue.
	...
	enginefailed = 1
	attack_verb = list("bluntly hit", "beat", "knocked")
	playsound(src, 'sound/weapons/chainsaw_turnoff.ogg',30,1)
...
/obj/item/drillparts
	...
	icon = 'icons/obj/mining.dmi' //TO BE CHANGED TO ANOTHER THING
