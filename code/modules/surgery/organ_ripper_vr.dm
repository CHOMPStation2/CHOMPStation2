...
/datum/surgery_step/generic/ripper/tear_vessel/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	user.balloon_alert_visible("[user] starts ripping into [target] with \the [tool].", \
	"You start ripping into [target] with \the [tool].") // CHOMPEdit - Ballon alert
...
/datum/surgery_step/generic/ripper/tear_vessel/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	var/obj/item/organ/external/affected = target.get_organ(target_zone)
	user.balloon_alert_visible("[user] has ripped [target]'s [affected] \the [tool], blood and viscera spraying everywhere!", \
	"You have ripped [target]'s [target.op_stage.current_organ] out with \the [tool], spraying blood all through the room!") // CHOMPEdit - Ballon alert
...
/datum/surgery_step/generic/ripper/tear_vessel/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	var/obj/item/organ/external/affected = target.get_organ(target_zone)
	user.balloon_alert_visible("[user]'s hand slips, damaging [target]'s [affected.name] with \the [tool]!", \
	"Your hand slips, damaging [target]'s [affected.name] with \the [tool]!") // CHOMPEdit - Ballon alert
...
/datum/surgery_step/generic/ripper/break_bone/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	var/obj/item/organ/external/affected = target.get_organ(target_zone)
	user.balloon_alert_visible("[user] starts violently shifting \the [tool] in [target]'s [affected.name]!", \
	"You start violently moving the [tool] in [target]'s [affected.name]!") // CHOMPEdit - Ballon alert
	target.custom_pain("[user] is ripping into your [target.op_stage.current_organ]!", 100)
	..()
...
/datum/surgery_step/generic/ripper/break_bone/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	var/obj/item/organ/external/affected = target.get_organ(target_zone)
	user.balloon_alert_visible("[user] has destroyed the bones within [target]'s [affected] with \the [tool]", \
	"You have destroyed the bones in [target]'s [affected] with \the [tool]!") // CHOMPEdit - Ballon alert
	affected.fracture()
...
/datum/surgery_step/generic/ripper/destroy_organ/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	if(!organ_to_destroy) //They decided to cancel. Let's slowly pull the tool back...
		user.balloon_alert(user, "You decide against mutilating any organs.")
		...
		user.balloon_alert_visible("[user] starts ripping into [target]'s [target.op_stage.current_organ] with \the [tool].", \
		"You start ripping [target]'s [target.op_stage.current_organ] with \the [tool].") // CHOMPEdit - Ballon alert
