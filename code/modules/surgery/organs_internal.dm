...
/datum/surgery_step/internal/fix_organ/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	user.balloon_alert_visible("[user]'s hand slips, getting mess and tearing the inside of [target]'s [affected.name] with \the [tool]!", \
...
/* CHOMPedit begin, allow rotten/damaged organs to be inserted again to allow for organ repair in the case of worst-case-scenerio gib situation. Also to make a funny if lets say, a doctor didnt examine a damaged organ and inserted it anyway.
	if(O.damage > (O.max_damage * 0.75))
		balloon_alert(user, "\The [O.organ_tag] [o_is] in no state to be transplanted.") // CHOMPEdit - Ballon alert
		return SURGERY_FAILURE
*/
	if(!target.internal_organs_by_name[O.organ_tag])
		organ_missing = 1
	else
		user.balloon_alert(user, "\The [target] already has [o_a][O.organ_tag].") // CHOMPEdit - Ballon alert
		return SURGERY_FAILURE
	...
	if(O && affected.organ_tag == O.parent_organ)
		...
		user.balloon_alert(user, "\The [O.organ_tag] [o_do] normally go in \the [affected.name].") // CHOMPEdit - Ballon alert
		return SURGERY_FAILURE
...
/datum/surgery_step/internal/replace_organ/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	user.balloon_alert_visible("[user]'s hand slips, damaging \the [tool]!", \
...
/datum/surgery_step/internal/attach_organ/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	var/obj/item/organ/external/affected = target.get_organ(target_zone)
	user.balloon_alert_visible("[user]'s hand slips, damaging the flesh in [target]'s [affected.name] with \the [tool]!", \
