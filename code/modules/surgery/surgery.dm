...
/obj/item/proc/do_surgery(mob/living/carbon/M, mob/living/user)
	...
	for(var/datum/surgery_step/S in GLOB.surgery_steps) // CHOMPEdit - Globals
...
/proc/sort_surgeries()
	var/gap = GLOB.surgery_steps.len // CHOMPEdit - Globals
	var/swapped = 1
	while (gap > 1 || swapped)
		swapped = 0
		if(gap > 1)
		...
		if(gap < 1)
			gap = 1
		for(var/i = 1; gap + i <= GLOB.surgery_steps.len; i++) // CHOMPEdit - Globals
			var/datum/surgery_step/l = GLOB.surgery_steps[i]		//Fucking hate // CHOMPEdit - Globals
			var/datum/surgery_step/r = GLOB.surgery_steps[gap+i]	//how lists work here // CHOMPEdit - Globals
			if(l.priority < r.priority)
				GLOB.surgery_steps.Swap(i, gap + i) // CHOMPEdit - Globals
				swapped = 1
