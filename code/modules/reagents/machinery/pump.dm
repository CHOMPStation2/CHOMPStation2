/obj/machinery/pump
	...
	circuit = /obj/item/circuitboard/fluidpump // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/pump/RefreshParts()
	var/obj/item/stock_parts/manipulator/SM = locate() in component_parts // CHOMPEdit - Removal of obj/item/weapon
	active_power_usage = initial(active_power_usage) / SM.rating

	var/motor_power = 0
	for(var/obj/item/stock_parts/motor/M in component_parts) // CHOMPEdit - Removal of obj/item/weapon
		motor_power += M.rating
	reagents_per_cycle = initial(reagents_per_cycle) * motor_power / 2

	var/bin_size = 0
	for(var/obj/item/stock_parts/matter_bin/SB in component_parts) // CHOMPEdit - Removal of obj/item/weapon
		bin_size += SB.rating
...
/obj/machinery/pump/attackby(obj/item/W, mob/user) // CHOMPEdit - Removal of obj/item/weapon
	. = TRUE
	...
	else if(istype(W, /obj/item/cell) && open) // CHOMPEdit - Removal of obj/item/weapon
		if(istype(cell))
