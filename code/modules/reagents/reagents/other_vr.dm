	...
	if(ishuman(M))
		...
		if(H.species.name != "Promethean")
			...
			for(var/obj/item/organ/I in H.organs)
				for(var/obj/item/implant/backup/BI in I.contents) // CHOMPEdit - Removal of obj/item/weapon
					backup_implants += BI
			if(backup_implants.len)
				for(var/obj/item/implant/backup/BI in backup_implants) // CHOMPEdit - Removal of obj/item/weapon
					BI.forceMove(src)
...
/datum/reagent/nif_repair_nanites/affect_blood(var/mob/living/carbon/M, var/alien, var/removed)
	if(ishuman(M))
		var/mob/living/carbon/human/H = M
		if(H.nif)
			var/obj/item/nif/nif = H.nif //L o c a l // CHOMPEdit - Removal of obj/item/device
