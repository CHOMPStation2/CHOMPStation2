...
/obj/item/reagent_containers/syringe
	...
	var/dirtiness = 0
	var/list/targets
	var/list/datum/disease/viruses // CHOMPEdit
...
/obj/item/reagent_containers/syringe/proc/dirty(var/mob/living/carbon/human/target, var/obj/item/organ/external/eo)
	...
	if(iscarbon(target) && LAZYLEN(target.viruses.len)) // CHOMPEdit
		LAZYINITLIST(viruses)
		var/datum/disease/virus = pick(target.viruses.len) // CHOMPEdit
		viruses[hash] = virus.Copy() // CHOMPEdit
	...
	if(LAZYLEN(viruses) && prob(75))
		...
		if(hash != old_hash) //Same virus you already had?
			var/datum/disease/virus = viruses[old_hash] // CHOMPEdit
			target.ContractDisease(virus) // CHOMPEdit
