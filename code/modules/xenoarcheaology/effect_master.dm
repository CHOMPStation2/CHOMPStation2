...
/datum/component/artifact_master/proc/on_attackby()
	var/obj/item/W = args[2] // CHOMPEdit - Removal of obj/item/weapon
	for(var/datum/artifact_effect/my_effect in my_effects)
		...
		if (istype(W, /obj/item/reagent_containers)) // CHOMPEdit - Removal of obj/item/weapon
			if(W.reagents.has_reagent("hydrogen", 1) || W.reagents.has_reagent("water", 1))
				...
				istype(W,/obj/item/multitool)) // CHOMPEdit - Removal of obj/item/device
			if (my_effect.trigger == TRIGGER_ENERGY)
				my_effect.ToggleActivate()

		else if (istype(W,/obj/item/flame) && W:lit ||\
				istype(W,/obj/item/weldingtool) && W:welding) // CHOMPEdit - Removal of obj/item/weapon
