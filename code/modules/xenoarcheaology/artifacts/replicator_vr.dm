...
/obj/machinery/replicator/vore/process()
	if(spawning_types.len && powered())
		...
		if(spawn_progress_time > max_spawn_time)
			...
			if(istype(source_material,/obj/item/holder/micro)) // CHOMPEdit - Removal of obj/item/weapon
				var/obj/item/holder/micro/micro_holder = source_material // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/replicator/vore/attackby(obj/item/W as obj, mob/living/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if(!W.canremove || !user.canUnEquip(W) || W.possessed_voice || is_type_in_list(W,item_vore_blacklist)) //No armblades, no putting possessed items in it!
		...
		return
	if(istype(W, /obj/item/holder/micro)) //Are you putting a micro in it? // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/holder/micro/micro_holder = W // CHOMPEdit - Removal of obj/item/weapon
		...
		if(inserted_mob.client)
			...
			return
	else if(istype(W,/obj/item/grab)) //Is someone being shoved into the machine? // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/grab/the_grab = W // CHOMPEdit - Removal of obj/item/weapon
		...
		if(inserted_mob.client)
			...
			to_chat(user, "<span class='notice'>You cannot put \the [W] into the machine. ((The micro must be connected to the server.))</span>")
			return
	else if(istype(W, /obj/item/holder/mouse)) //No you can't turn your army of mice into giant rats. // CHOMPEdit - Removal of obj/item/weapon
	...
	if(istype(W, /obj/item/holder/micro)) //I hate this but it's the only way to get their stuff to drop. // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/holder/micro/micro_holder = W // CHOMPEdit - Removal of obj/item/weapon
		var/mob/living/inserted_mob = micro_holder.held_mob //Get the actual mob.
...
/obj/machinery/replicator/clothing
	...
	/obj/item/storage/smolebrickcase, // CHOMPEdit - Removal of obj/item/weapon
	/obj/item/lipstick, // CHOMPEdit - Removal of obj/item/weapon
	/obj/item/material/fishing_rod/modern, // CHOMPEdit - Removal of obj/item/weapon
	/obj/item/inflatable_duck, // CHOMPEdit - Removal of obj/item/weapon
	/obj/item/toy/syndicateballoon,
	/obj/item/towel, // CHOMPEdit - Removal of obj/item/weapon
	/obj/item/bedsheet/rainbowdouble // CHOMPEdit - Removal of obj/item/weapon
			...
			if(source_material)
				...
				if(length(source_material.desc) < MAX_MESSAGE_LEN * 2)
					if(spawned_obj.desc)
						...
						spawned_obj.desc = "It is made of [source_material]."
			if(istype(source_material,/obj/item/holder/micro)) // CHOMPEdit - Removal of obj/item/weapon
				var/obj/item/holder/micro/micro_holder = source_material //Tells the machine that a micro is the material being used // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/replicator/clothing/attackby(obj/item/W as obj, mob/living/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if(!W.canremove || !user.canUnEquip(W) || W.possessed_voice || is_type_in_list(W,item_vore_blacklist)) //No armblades, no putting already possessed items in it!
		to_chat(user, "<span class='notice'>You cannot put \the [W] into the machine.</span>")
		return
	if(istype(W, /obj/item/holder/micro) || istype(W, /obj/item/holder/mouse)) //Are you putting a micro/mouse in it? // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/holder/micro/micro_holder = W // CHOMPEdit - Removal of obj/item/weapon
		var/mob/living/inserted_mob = micro_holder.held_mob //Get the actual mob.
		if(!inserted_mob.allow_spontaneous_tf) //Do they allow TF?
			to_chat(user, "<span class='notice'>You cannot put \the [W] into the machine. ((The prefs of the micro forbid this action.))</span>")
