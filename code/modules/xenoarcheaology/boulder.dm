...
/obj/structure/boulder/attackby(var/obj/item/I, var/mob/user)
	if(istype(I, /obj/item/core_sampler)) // CHOMPEdit - Removal of obj/item/device
		src.geological_data.artifact_distance = rand(-100,100) / 100
		src.geological_data.artifact_id = artifact_find.artifact_id

		var/obj/item/core_sampler/C = I // CHOMPEdit - Removal of obj/item/device
		C.sample_item(src, user)
		return

	if(istype(I, /obj/item/depth_scanner)) // CHOMPEdit - Removal of obj/item/device
		var/obj/item/depth_scanner/C = I // CHOMPEdit - Removal of obj/item/device
		C.scan_atom(user, src)
		return

	if(istype(I, /obj/item/xenoarch_multi_tool)) // CHOMPEdit - Removal of obj/item/device
		var/obj/item/xenoarch_multi_tool/C = I // CHOMPEdit - Removal of obj/item/device
	...
	if(istype(I, /obj/item/measuring_tape)) // CHOMPEdit - Removal of obj/item/device
		var/obj/item/measuring_tape/P = I // CHOMPEdit - Removal of obj/item/device
		user.visible_message("<b>\The [user]</b> extends \the [P] towards \the [src].", "<span class='notice'>You extend \the [P] towards \the [src].</span>")
		if(do_after(user, 15))
			to_chat(user, "<span class='notice'>\The [src] has been excavated to a depth of [2 * src.excavation_level]cm.</span>")
		return

	if(istype(I, /obj/item/pickaxe)) // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/pickaxe/P = I // CHOMPEdit - Removal of obj/item/weapon
