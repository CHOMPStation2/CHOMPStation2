...
/decl/emote/proc/do_sound(var/atom/user)
	...
	if(sound_to_play)
		...
		if(istype(user, /mob))
			var/mob/u = user
			playsound(user.loc, sound_to_play, use_sound["vol"], u.read_preference(/datum/preference/toggle/random_emote_pitchh) && sound_vary, extrarange = use_sound["exr"], frequency = u.voice_freq, preference = sound_preferences, volume_channel = use_sound["volchannel"]) //CHOMPEdit
		else
