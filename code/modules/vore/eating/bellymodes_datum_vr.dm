...
/datum/digest_mode/egg/handle_atoms(obj/belly/B, list/touchable_atoms)
	...
	for(var/E in touchable_atoms)
		if(istype(E, /mob/observer))
			continue
		if(istype(E, /obj/item/storage/vore_egg)) // Don't egg other eggs. // CHOMPEdit - Removal of obj/item/weapon
			var/obj/item/storage/vore_egg/EG = E // CHOMPEdit - Removal of obj/item/weapon
	...
	if(egg_contents.len)
		...
		for(var/atom/movable/C in egg_contents)
			...
			if(isliving(C))
				var/mob/living/M = C
				var/mob_holder_type = M.holder_type || /obj/item/holder // CHOMPEdit - Removal of obj/item/weapon
				...
				var/obj/item/holder/H = new mob_holder_type(B.ownegg, M) // CHOMPEdit - Removal of obj/item/weapon
