...
/obj/belly
	...
	var/obj/item/storage/vore_egg/ownegg	// Is this belly creating an egg? // CHOMPEdit - Removal of obj/item/weapon
	var/egg_type = "Egg"					// Default egg type and path.
	var/egg_path = /obj/item/storage/vore_egg // CHOMPEdit - Removal of obj/item/weapon
...
/obj/belly/Entered(atom/movable/thing, atom/OldLoc)
	...
	if(istype(OldLoc, /mob/observer) || istype(OldLoc, /obj/item/mmi)) // Prevent reforming causing a lot of log spam/sounds // CHOMPEdit - Removal of obj/item/device
...
/obj/belly/proc/release_specific_contents(atom/movable/M, silent = FALSE)
	...
	for(var/obj/item/holder/H in M.contents) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/belly/proc/digestion_death(mob/living/M)
	...
	var/obj/item/mmi/hasMMI // CHOMPEdit - Adjust how MMI's are handled // CHOMPEdit - Removal of obj/item/device
	...
	if(CONFIG_GET(flag/items_survive_digestion)) // CHOMPEdit
		for(var/obj/item/W in M)
			if(istype(W, /obj/item/organ/internal/mmi_holder/posibrain))
				...
				var/obj/item/mmi/brainbox = MMI.removed() // CHOMPEdit - Removal of obj/item/device
				if(brainbox)
			...
			for(var/slot in slots)
				...
				if(I)
					...
					if(item_digest_mode == IM_HOLD)
						items_preserved |= I
					else if(item_digest_mode == IM_DIGEST_FOOD && !(istype(I,/obj/item/reagent_containers/food) || istype(I,/obj/item/organ))) // CHOMPEdit - Removal of obj/item/weapon
						items_preserved |= I
	...
	if(isrobot(M))
		...
		if(R.mmi && R.mind && R.mmi.brainmob)
			if((R.soulcatcher_pref_flags & SOULCATCHER_ALLOW_CAPTURE) && owner.soulgem && owner.soulgem.flag_check(SOULGEM_ACTIVE | NIF_SC_CATCHING_OTHERS, TRUE))
				...
				var/obj/item/robot_module/MB = locate() in R.contents // CHOMPEdit - Removal of obj/item/weapon
				if(MB)
...
/obj/belly/proc/autotransfer_filter(var/atom/movable/prey, var/whitelist, var/blacklist)
	if(ismob(prey))
		...
		if(blacklist & autotransfer_flags_list_items["Eggs"])
			if(istype(prey, /obj/item/storage/vore_egg)) return FALSE // CHOMPEdit - Removal of obj/item/weapon
		if(blacklist & autotransfer_flags_list_items["Remains"])
			if(istype(prey, /obj/item/digestion_remains)) return FALSE // CHOMPEdit - Removal of obj/item/weapon
		...
		if(blacklist & autotransfer_flags_list_items["Ores"])
			if(istype(prey, /obj/item/ore)) return FALSE // CHOMPEdit - Removal of obj/item/weapon
		if(blacklist & autotransfer_flags_list_items["Clothes and Bags"])
			if(istype(prey, /obj/item/clothing) || istype(prey, /obj/item/storage)) return FALSE // CHOMPEdit - Removal of obj/item/weapon
		if(blacklist & autotransfer_flags_list_items["Food"])
			if(istype(prey, /obj/item/reagent_containers/food)) return FALSE // CHOMPEdit - Removal of obj/item/weapon
		if(whitelist == 0) return TRUE
		...
		if(whitelist & autotransfer_flags_list_items["Eggs"])
			if(istype(prey, /obj/item/storage/vore_egg)) return TRUE // CHOMPEdit - Removal of obj/item/weapon
		if(whitelist & autotransfer_flags_list_items["Remains"])
			if(istype(prey, /obj/item/digestion_remains)) return TRUE // CHOMPEdit - Removal of obj/item/weapon
		...
		if(whitelist & autotransfer_flags_list_items["Ores"])
			if(istype(prey, /obj/item/ore)) return TRUE // CHOMPEdit - Removal of obj/item/weapon
		if(whitelist & autotransfer_flags_list_items["Clothes and Bags"])
			if(istype(prey, /obj/item/clothing) || istype(prey, /obj/item/storage)) return TRUE // CHOMPEdit - Removal of obj/item/weapon
		if(whitelist & autotransfer_flags_list_items["Food"])
			if(istype(prey, /obj/item/reagent_containers/food)) return TRUE // CHOMPEdit - Removal of obj/item/weapon
