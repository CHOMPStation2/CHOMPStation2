...
/mob/living/proc/vore_transfer_reagents()
	...
	switch(input(user,"Choose what to transfer to","Select Target") in list("Vore belly", "Stomach", "Container", "Floor", "Cancel"))
		...
		if("Container")
			...
			for(var/obj/item/reagent_containers/rc in view(1,user.loc)) // CHOMPEdit - Removal of obj/item/weapon
				choices += rc
			var/obj/item/reagent_containers/T = input(user,"Choose what to transfer to","Select Target") as null|anything in choices // CHOMPEdit - Removal of obj/item/weapon
			if(!T)
				return FALSE
