...
/datum/vore_look/proc/pick_from_outside(mob/user, params)
	...
	if(isobserver(target) || istype(target,/obj/item/mmi)) // CHOMPEdit - Removal of obj/item/device
	...
	switch(intent)
		...
		if("Reform")
			...
			else if(istype(target,/obj/item/mmi)) // A good bit of repeated code, sure, but... cleanest way to do this. // CHOMPEdit - Removal of obj/item/device
				var/obj/item/mmi/MMI = target // CHOMPEdit - Removal of obj/item/device
