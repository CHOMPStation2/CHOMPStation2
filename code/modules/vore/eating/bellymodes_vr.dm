			...
			if(M.stat == DEAD)
				...
				if(compensation > 0)
					if(isrobot(owner))
						...
						if(reagentbellymode == TRUE && reagent_mode_flags & DM_FLAG_REAGENTSDIGEST && reagents.total_volume < custom_max_volume) //CHOMP digestion producing reagents
		...
		for (var/target in touchable_mobs)
			...
			if(M.absorbable == FALSE)
				...
				if(M.nutrition >= 100) //Drain them until there's no nutrients left. Slowly "absorb" them.
					...
					if(reagentbellymode == TRUE && reagent_mode_flags & DM_FLAG_REAGENTSABSORB && reagents.total_volume < custom_max_volume) //CHOMP absorption reagent production
...
//////////////////////////// DM_DRAIN ////////////////////////////
		...
		for (var/target in touchable_mobs)
			...
			if(M.nutrition >= 100) //Drain them until there's no nutrients left.
				var/oldnutrition = (M.nutrition * 0.05)
				M.nutrition = (M.nutrition * 0.95)
				if(reagentbellymode == TRUE && reagent_mode_flags & DM_FLAG_REAGENTSDRAIN && reagents.total_volume < custom_max_volume)   //CHOMP draining reagent production
					owner.nutrition += oldnutrition * 0.75 //keeping the price static, due to how much nutrition can flunctuate
