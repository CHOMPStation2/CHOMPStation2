...
/mob/living/simple_mob/attackby(var/obj/item/O, var/mob/user)
	if(istype(O, /obj/item/newspaper) && !(ckey || (ai_holder.hostile && faction != user.faction)) && isturf(user.loc)) // CHOMPEdit - Removal of obj/item/weapon
	...
	else if(istype(O, /obj/item/healthanalyzer)) // CHOMPEdit - Removal of obj/item/device
