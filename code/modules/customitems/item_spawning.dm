...
GLOBAL_LIST_EMPTY(custom_items) // CHOMPEdit - Globals
	...
	for(var/line in splittext(file2text("config/custom_items.txt"), "\n"))
		...
		if(findtext(line, "{", 1, 2) || findtext(line, "}", 1, 2)) // New block!
			if(current_data && current_data.assoc_key)
				if(!GLOB.custom_items[current_data.assoc_key]) // CHOMPEdit - Globals
					GLOB.custom_items[current_data.assoc_key] = list() // CHOMPEdit - Globals
				var/list/L = GLOB.custom_items[current_data.assoc_key] // CHOMPEdit - Globals
				L |= current_data
			current_data = null
...
/proc/equip_custom_items(mob/living/carbon/human/M)
	var/list/key_list = GLOB.custom_items[M.ckey] // CHOMPEdit - Globals
