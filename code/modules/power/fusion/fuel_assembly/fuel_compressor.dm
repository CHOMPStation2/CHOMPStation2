...
/obj/machinery/fusion_fuel_compressor
	...
	circuit = /obj/item/circuitboard/fusion_fuel_compressor // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/fusion_fuel_compressor/proc/do_special_fuel_compression(var/obj/item/thing, var/mob/user)
	if(istype(thing) && thing.reagents && thing.reagents.total_volume && thing.is_open_container())
		...
		var/obj/item/fuel_assembly/F = new(get_turf(src), R.id, R.color) // CHOMPEdit - Removal of obj/item/weapon
		thing.reagents.remove_reagent(R.id, R.volume)
		user.put_in_hands(F)

	else if(istype(thing, /obj/machinery/power/supermatter))
		var/obj/item/fuel_assembly/F = new(get_turf(src), "supermatter") // CHOMPEdit - Removal of obj/item/weapon
	...
	if(istype(thing, /obj/item/stack/material))
		...
		if(!blitzprogress)
			...
			if(M.get_amount() < FUSION_ROD_SHEET_AMT)
				...
				return
			var/obj/item/fuel_assembly/F = new(get_turf(src), mat.name) // CHOMPEdit - Removal of obj/item/weapon
			visible_message("<b>\The [src]</b> compresses \the [thing] into a new fuel assembly.")
			...
			if(mat.name=="phoron")
				if(M.get_amount() < 25)
					...
					return
				var/obj/item/fuel_assembly/blitz/unshielded/F = new(get_turf(src)) // CHOMPEdit - Removal of obj/item/weapon
