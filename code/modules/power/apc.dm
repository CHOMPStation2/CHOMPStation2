...
/obj/machinery/power/apc/attackby(obj/item/W, mob/user)
	...
	if(W.has_tool_quality(TOOL_CROWBAR) && opened)
			...
			if(do_after(user, 50 * W.toolspeed))
				if(has_electronics == APC_HAS_ELECTRONICS_WIRED)
					...
					if((stat & BROKEN))
						...
						new /obj/item/module/power_control(loc) // CHOMPEdit - Removal of obj/item/weapon
	...
	else if(istype(W, /obj/item/card/id)||istype(W, /obj/item/pda))			// trying to unlock the interface with an ID card // CHOMPEdit - Removal of obj/item/weapon // CHOMPEdit - Removal of obj/item/device
		togglelock(user)
		...
		if(do_after(user, 50 * W.toolspeed))
			if(terminal && opened && has_electronics != APC_HAS_ELECTRONICS_SECURED)
				...
				qdel(terminal)
	else if(istype(W, /obj/item/module/power_control) && opened && has_electronics == APC_HAS_ELECTRONICS_NONE && !((stat & BROKEN))) // CHOMPEdit - Removal of obj/item/weapon
		...
		if(istype(W, /obj/item/frame/apc) && (stat & BROKEN))
			...
			if(do_after(user, 50))
				...
				if(opened==2)
					opened = 1
				update_icon()
		else if(istype(W, /obj/item/multitool) && (hacker || emagged)) // CHOMPEdit - Removal of obj/item/device
			if(cell)
				to_chat(user, "<span class='warning'>You need to remove the power cell first.</span>")
				return
		...
		if((stat & BROKEN) \
			...
			if(istype(user, /mob/living/silicon))
				return attack_hand(user)
			if(!opened && wiresexposed && (istype(W, /obj/item/multitool) || W.has_tool_quality(TOOL_WIRECUTTER) || istype(W, /obj/item/assembly/signaler))) // CHOMPEdit - Removal of obj/item/device
				return attack_hand(user)
