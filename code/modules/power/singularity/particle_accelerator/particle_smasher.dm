...
/obj/machinery/particle_smasher
	...
	var/obj/item/reagent_containers/reagent_container		// Holds the beaker. The process will consume ALL reagents inside it. // CHOMPEdit - Removal of obj/item/weapon
	var/beaker_type = /obj/item/reagent_containers/glass/beaker // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/particle_smasher/attackby(obj/item/W as obj, mob/user as mob)
	if(W.type == /obj/item/analyzer) // CHOMPEdit - Removal of obj/item/device
		return
		...
		if(reagent_container)
			...
			return
		if(isrobot(user) && istype(W.loc, /obj/item/gripper)) // CHOMPEdit - Removal of obj/item/weapon
			var/obj/item/gripper/G = W.loc // CHOMPEdit - Removal of obj/item/weapon
			G.drop_item()
		else
		...
		update_icon()
		return
	else if(istype(W, /obj/item/card/id)) // CHOMPEdit - Removal of obj/item/weapon
		to_chat(user, "<span class='notice'>Swiping \the [W] on \the [src] doesn't seem to do anything...</span>")
		return ..()
	else if(((isrobot(user) && istype(W.loc, /obj/item/gripper)) || (!isrobot(user) && W.canremove)) && storage.len < max_storage) // CHOMPEdit - Removal of obj/item/weapon
		if(isrobot(user) && istype(W.loc, /obj/item/gripper)) // CHOMPEdit - Removal of obj/item/weapon
			var/obj/item/gripper/G = W.loc // CHOMPEdit - Removal of obj/item/weapon
			G.drop_item()
		else
...
/datum/particle_smasher_recipe
	...
	var/list/items		// example: = list(/obj/item/tool/crowbar, /obj/item/welder) Place /foo/bar before /foo. Do not include fruit. Maximum of 3 items. // CHOMPEdit - Removal of obj/item/weapon
	...
	if (items && items.len)
		...
		if(istype(container, /obj/machinery/particle_smasher))
			...
			for(var/obj/O in machine.storage)
				if(istype(O,/obj/item/reagent_containers/food/snacks/grown)) // CHOMPEdit - Removal of obj/item/weapon
...
/datum/particle_smasher_recipe/donkpockets_coal
	items = list(/obj/item/reagent_containers/food/snacks/donkpocket) // CHOMPEdit - Removal of obj/item/weapon

	recipe_type = PS_RESULT_ITEM

	result = /obj/item/ore/coal // CHOMPEdit - Removal of obj/item/weapon
...
/datum/particle_smasher_recipe/donkpockets_ascend
	...
	result = /obj/item/reagent_containers/food/snacks/donkpocket/ascended // CHOMPEdit - Removal of obj/item/weapon
