...
/obj/machinery/telecomms/attackby(obj/item/P as obj, mob/user as mob)
	...
	if(istype(P, /obj/item/multitool)) // CHOMPEdit - Removal of obj/item/device
		attack_hand(user)
...
/obj/machinery/telecomms/tgui_data(mob/user)
	...
	if(on || interact_offline)
		...
		var/obj/item/multitool/P = get_multitool(user) // CHOMPEdit - Removal of obj/item/device
		data["multitool"] = !!P
		data["multitool_buffer"] = null
		if(P && P.buffer)
			P.update_icon()
...
/obj/machinery/telecomms/tgui_status(mob/user)
	if(!issilicon(user))
		if(!istype(user.get_active_hand(), /obj/item/multitool)) // CHOMPEdit - Removal of obj/item/device
			return STATUS_CLOSE
	. = ..()
...
/obj/machinery/proc/get_multitool(mob/user as mob)	//No need to have this being a telecomms specific proc.
	...
	var/obj/item/multitool/P = null // CHOMPEdit - Removal of obj/item/device
	// Let's double check
	if(!issilicon(user) && istype(user.get_active_hand(), /obj/item/multitool)) // CHOMPEdit - Removal of obj/item/device
...
/obj/machinery/telecomms/tgui_act(action, params)
	...
	var/obj/item/multitool/P = get_multitool(usr) // CHOMPEdit - Removal of obj/item/device
