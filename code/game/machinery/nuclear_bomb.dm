...
/obj/machinery/nuclearbomb
	...
	var/yes_code = 0.0
	var/safety = 1.0
	var/obj/item/disk/nuclear/auth = null // CHOMPEdit - Removal of obj/item/weapon
	var/list/wires = list()
	var/light_wire
	var/safety_wire
	var/timing_wire
...
/obj/machinery/nuclearbomb/attackby(obj/item/O as obj, mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if(O.has_tool_quality(TOOL_SCREWDRIVER))
		...
		return
	if(O.has_tool_quality(TOOL_WIRECUTTER) || istype(O, /obj/item/multitool)) // CHOMPEdit - Removal of obj/item/device
		if(opened == 1)
			nukehack_win(user)
		return

	if(extended)
		if(istype(O, /obj/item/disk/nuclear)) // CHOMPEdit - Removal of obj/item/weapon
			usr.drop_item()
			O.loc = src
			auth = O
			add_fingerprint(user)
		...
		switch(removal_stage)
			if(0)
				if(O.has_tool_quality(TOOL_WELDER))
					...
					var/obj/item/weldingtool/WT = O.get_welder() // CHOMPEdit - Removal of obj/item/weapon
					if(!WT.isOn()) return
	...
	if((usr.contents.Find(src) || (in_range(src, usr) && istype(src.loc, /turf))))
		...
		if(href_list["act"])
			...
			if(href_list["act"] == "pulse")
				if(!istype(usr.get_active_hand(), /obj/item/multitool)) // CHOMPEdit - Removal of obj/item/device
...
/obj/item/disk/nuclear/New() // CHOMPEdit - Removal of obj/item/weapon
	..()
	nuke_disks |= src

/obj/item/disk/nuclear/Destroy() // CHOMPEdit - Removal of obj/item/weapon
	if(!nuke_disks.len && blobstart.len > 0)
		var/obj/D = new /obj/item/disk/nuclear(pick(blobstart)) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/disk/nuclear/touch_map_edge() // CHOMPEdit - Removal of obj/item/weapon
	qdel(src)
