...
/obj/machinery/portable_atmospherics/canister/attackby(var/obj/item/W as obj, var/mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if(W.has_tool_quality(TOOL_WELDER)) //Vorestart: Deconstructable Canisters
		...
		if(do_after(user, 20 * WT.toolspeed))
			...
			return
	//Voreend
	if(!W.has_tool_quality(TOOL_WRENCH) && !istype(W, /obj/item/tank) && !istype(W, /obj/item/analyzer) && !istype(W, /obj/item/pda)) // CHOMPEdit - Removal of obj/item/weapon // CHOMPEdit - Removal of obj/item/device
		visible_message("<span class='warning'>\The [user] hits \the [src] with \a [W]!</span>")
		src.health -= W.force
		src.add_fingerprint(user)
		healthcheck()

	if(istype(user, /mob/living/silicon/robot) && istype(W, /obj/item/tank/jetpack)) // CHOMPEdit - Removal of obj/item/weapon
		...
		if("eject")
			if(holding)
				if(valve_open)
					valve_open = 0
					release_log += "Valve was <b>closed</b> by [usr] ([usr.ckey]), stopping the transfer into the [holding]<br>"
				if(istype(holding, /obj/item/tank)) // CHOMPEdit - Removal of obj/item/weapon
