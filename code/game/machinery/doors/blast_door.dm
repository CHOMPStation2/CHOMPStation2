...
/obj/machinery/door/blast/attackby(obj/item/C as obj, mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	src.add_fingerprint(user)
	if(istype(C, /obj/item)) // For reasons unknown, sometimes C is actually not what it is advertised as, like a mob. // CHOMPEdit - Removal of obj/item/weapon
		if(C.pry == 1 && (user.a_intent != I_HURT || (stat & BROKEN))) // Can we pry it open with something, like a crowbar/fireaxe/lingblade?
			if(istype(C,/obj/item/material/twohanded/fireaxe)) // Fireaxes need to be in both hands to pry. // CHOMPEdit - Removal of obj/item/weapon
