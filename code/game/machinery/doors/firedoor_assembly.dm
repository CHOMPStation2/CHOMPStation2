...
/obj/structure/firedoor_assembly/attackby(obj/item/C, mob/user as mob)
	if(istype(C, /obj/item/stack/cable_coil) && !wired && anchored)
	...
	else if(istype(C, /obj/item/circuitboard/airalarm) && wired) // CHOMPEdit - Removal of obj/item/weapon
		if(anchored)
		...
		var/obj/item/weldingtool/WT = C.get_welder() // CHOMPEdit - Removal of obj/item/weapon
