...
/obj/machinery/door/airlock
	...
	var/safe = 1
	normalspeed = 1
	var/obj/item/airlock_electronics/electronics = null // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/door/airlock/attackby(obj/item/C, mob/user as mob)
	...
	if(C.has_tool_quality(TOOL_WELDER) && !( src.operating > 0 ) && src.density)
		if(health < maxhealth && user.a_intent == I_HELP)
			..()
			return
		var/obj/item/weldingtool/W = C.get_welder() // CHOMPEdit - Removal of obj/item/weapon
		...
		return src.attack_hand(user)
	else if(istype(C, /obj/item/multitool)) // CHOMPEdit - Removal of obj/item/device
		return src.attack_hand(user)
	else if(istype(C, /obj/item/assembly/signaler)) // CHOMPEdit - Removal of obj/item/device
		return src.attack_hand(user)
	else if(istype(C, /obj/item/pai_cable))	// -- TLE // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/pai_cable/cable = C // CHOMPEdit - Removal of obj/item/weapon
		...
		if(can_remove_electronics())
			...
			if(do_after(user,40 * C.toolspeed))
				...
				if(operating == -1 || (stat & BROKEN))
					new /obj/item/circuitboard/broken(src.loc) // CHOMPEdit - Removal of obj/item/weapon
					operating = 0
				else
	...
	else if(istype(C, /obj/item)) // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/W = C // CHOMPEdit - Removal of obj/item/weapon
		if((W.pry == 1) && !arePowerSystemsOn())
			if(locked)
				to_chat(user, "<span class='notice'>The airlock's bolts prevent it from being forced.</span>")
			else if( !welded && !operating )
				if(istype(C, /obj/item/material/twohanded/fireaxe)) // If this is a fireaxe, make sure it's held in two hands. // CHOMPEdit - Removal of obj/item/weapon
					var/obj/item/material/twohanded/fireaxe/F = C // CHOMPEdit - Removal of obj/item/weapon
					if(!F.wielded)
...
/obj/machinery/door/airlock/proc/create_electronics()
	...
	if (secured_wires)
		src.electronics = new/obj/item/airlock_electronics/secure( src.loc ) // CHOMPEdit - Removal of obj/item/weapon
	else
		src.electronics = new/obj/item/airlock_electronics( src.loc ) // CHOMPEdit - Removal of obj/item/weapon
