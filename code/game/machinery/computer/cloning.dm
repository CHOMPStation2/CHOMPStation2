...
/obj/machinery/computer/cloning
	...
	icon_screen = "dna"
	circuit = /obj/item/circuitboard/cloning // CHOMPEdit - Removal of obj/item/weapon
	...
	var/obj/item/disk/data/diskette = null //Mostly so the geneticist can steal everything. // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/computer/cloning/attackby(obj/item/W as obj, mob/user as mob, params)
	if(istype(W, /obj/item/disk/data)) //INSERT SOME DISKETTES // CHOMPEdit - Removal of obj/item/weapon
		if(!diskette)
			user.drop_item()
			W.loc = src
			diskette = W
		...
		if("view_rec")
			...
			if(istype(active_record))
				if(isnull(active_record.ckey))
					...
					var/obj/item/implant/health/H = null // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/computer/cloning/proc/scan_mob(mob/living/carbon/human/subject as mob, var/scan_brain = 0)
	...
	var/obj/item/implant/health/imp = locate(/obj/item/implant/health, subject) // CHOMPEdit - Removal of obj/item/weapon
	if (isnull(imp))
		imp = new /obj/item/implant/health(subject) // CHOMPEdit - Removal of obj/item/weapon
