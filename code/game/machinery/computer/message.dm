...
/obj/machinery/computer/message_monitor/LateInitialize()
	...
	if(!linkedServer)
		if(GLOB.message_servers && GLOB.message_servers.len > 0) // CHOMPEdit - Globals
			linkedServer = GLOB.message_servers[1] // CHOMPEdit - Globals
...
/obj/machinery/computer/message_monitor/tgui_data(mob/user)
	...
	if(linkedServer && auth)
		...
		for(var/obj/item/device/pda/P in GLOB.PDAs) // CHOMPEdit - Globals
			if(!P.owner || P.hidden)
				continue
...
/obj/machinery/computer/message_monitor/tgui_act(action, params)
	...
	switch(action)
		...
		//Find a server
		if("find")
			if(GLOB.message_servers && GLOB.message_servers.len > 1) // CHOMPEdit - Globals
				linkedServer = tgui_input_list(usr,"Please select a server.", "Select a server.", GLOB.message_servers) // CHOMPEdit - Globals
				set_temp("NOTICE: Server selected.", "alert")
			else if(GLOB.message_servers && GLOB.message_servers.len > 0) // CHOMPEdit - Globals
				linkedServer = GLOB.message_servers[1] // CHOMPEdit - Globals
...
/obj/item/weapon/paper/monitorkey/LateInitialize()
	if(GLOB.message_servers) // CHOMPEdit - Globals
		for(var/obj/machinery/message_server/server in GLOB.message_servers) // CHOMPEdit - Globals
