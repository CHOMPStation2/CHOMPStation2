...
/obj/machinery/bomb_tester
	...
	circuit = /obj/item/circuitboard/bomb_tester // CHOMPEdit - Removal of obj/item/weapon

	var/icon_name = "generic"

	var/obj/item/tank/tank1 // CHOMPEdit - Removal of obj/item/weapon
	var/obj/item/tank/tank2 // CHOMPEdit - Removal of obj/item/weapon
...
/obj/machinery/bomb_tester/attackby(var/obj/item/I, var/mob/user)
	...
	if(default_part_replacement(user, I))
		return
	if(istype(I, /obj/item/tank)) // CHOMPEdit - Removal of obj/item/weapon
		if(!tank1 || !tank2)
			user.drop_item(I)
			I.forceMove(src)
			if(!tank1)
...
/obj/machinery/bomb_tester/tgui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state)
	...
	switch(action)
		...
		if("add_tank")
			if(istype(usr.get_active_hand(), /obj/item/tank)) // CHOMPEdit - Removal of obj/item/weapon
				var/obj/item/tank/T = usr.get_active_hand() // CHOMPEdit - Removal of obj/item/weapon
		...
		if("remove_tank")
			var/obj/item/tank/T = locate(params["ref"]) in list(tank1, tank2) // CHOMPEdit - Removal of obj/item/weapon
			if(istype(T))
				if(T == tank1)
					tank1 = null
				if(T == tank2)
...
/obj/machinery/bomb_tester/proc/simulation_finish(cancelled = 0)
	...
	if(simulation_results == "Error")
		...
		var/obj/item/paper/P = new(get_turf(src)) // CHOMPEdit - Removal of obj/item/weapon
