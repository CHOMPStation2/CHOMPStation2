/turf/simulated/floor/attackby(var/obj/item/C, var/mob/user)
	...
	if(isliving(user) && istype(C, /obj/item)) //CHOMPEDIT START - Making engraving require disarm intent (and simplifying the proc) // CHOMPEdit - Removal of obj/item/weapon
	...
	if(!is_plating())
		if(istype(C, /obj/item)) // CHOMPEdit - Removal of obj/item/weapon
	...
	// Floor is plating (or no flooring)
		...
		// Plating repairs and removal
			...
			var/obj/item/weldingtool/welder = C.get_welder() // CHOMPEdit - Removal of obj/item/weapon
			if(welder.isOn())
				// Needs repairs
				if(broken || burnt)
...
/turf/simulated/floor/proc/try_deconstruct_tile(obj/item/W as obj, mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if(W.has_tool_quality(TOOL_CROWBAR))
		...
		playsound(src, W.usesound, 80, 1)
		return 1
	else if(istype(W, /obj/item/shovel) && (flooring.flags & TURF_REMOVE_SHOVEL)) // CHOMPEdit - Removal of obj/item/weapon
...
/turf/simulated/floor/proc/try_replace_tile(obj/item/stack/tile/T as obj, mob/user as mob)
	if(T.type == flooring.build_type)
		return
	var/obj/item/W = user.is_holding_item_of_type(/obj/item) // CHOMPEdit - Removal of obj/item/weapon
	if(!istype(W))
		return
...
/turf/simulated/floor/proc/do_remove_plating(obj/item/W, mob/user, base_type) // CHOMPEdit - Removal of obj/item/weapon
