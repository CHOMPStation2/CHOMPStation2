...
/obj/item/circuitboard/proc/construct(var/obj/machinery/M) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/circuitboard/proc/deconstruct(var/obj/machinery/M) // CHOMPEdit - Removal of obj/item/weapon
	if(istype(M, build_path))
		return 1
	return 0

//Should be called from the constructor of any machine to automatically populate the default parts
/obj/item/circuitboard/proc/apply_default_parts(var/obj/machinery/M) // CHOMPEdit - Removal of obj/item/weapon
	if(!istype(M))
		return
	if(!req_components)
		return
