...
/obj/item/tape_roll/proc/can_place(var/mob/living/carbon/human/H, var/mob/user) // CHOMPEdit - Removal of obj/item/weapon
	if(istype(user, /mob/living/silicon/robot) || user == H)
		return TRUE

	for (var/obj/item/grab/G in H.grabbed_by) // CHOMPEdit - Removal of obj/item/weapon
		if (G.loc == user && G.state >= GRAB_AGGRESSIVE)
			return TRUE

	return FALSE

/obj/item/tape_roll/attack(var/mob/living/carbon/human/H, var/mob/user) // CHOMPEdit - Removal of obj/item/weapon
	if(istype(H))
		...
		if(!can_place(H, user))
			...
			if(user.zone_sel.selecting == O_EYES)
				...
				var/obj/item/handcuffs/cable/tape/T = new(user) // CHOMPEdit - Removal of obj/item/weapon
				playsound(src, 'sound/effects/tape.ogg',25)
...
/obj/item/tape_roll/proc/stick(var/obj/item/W, mob/user) // CHOMPEdit - Removal of obj/item/weapon
	if(!istype(W, /obj/item/paper) || istype(W, /obj/item/paper/sticky) || !user.unEquip(W)) // CHOMPEdit - Removal of obj/item/weapon
		return
	user.drop_from_inventory(W)
	var/obj/item/ducttape/tape = new(get_turf(src)) // CHOMPEdit - Removal of obj/item/weapon
	tape.attach(W)
	user.put_in_hands(tape)
	playsound(src, 'sound/effects/tape.ogg',25)

/obj/item/ducttape // CHOMPEdit - Removal of obj/item/weapon
	name = "tape"
	...
	var/obj/item/stuck = null // CHOMPEdit - Removal of obj/item/weapon

/obj/item/ducttape/New() // CHOMPEdit - Removal of obj/item/weapon
	..()
	flags |= NOBLUDGEON

/obj/item/ducttape/examine(mob/user) // CHOMPEdit - Removal of obj/item/weapon
	return stuck.examine(user)

/obj/item/ducttape/proc/attach(var/obj/item/W) // CHOMPEdit - Removal of obj/item/weapon
	stuck = W
	W.forceMove(src)
...
/obj/item/ducttape/attack_self(mob/user) // CHOMPEdit - Removal of obj/item/weapon
	if(!stuck)
		return
...
/obj/item/ducttape/attackby(var/obj/item/I, var/mob/user) // CHOMPEdit - Removal of obj/item/weapon
	if(!(istype(src, /obj/item/handcuffs/cable/tape) || istype(src, /obj/item/clothing/mask/muzzle/tape))) // CHOMPEdit - Removal of obj/item/weapon
		return ..()
	else
...
/obj/item/ducttape/attack_hand(mob/living/L) // CHOMPEdit - Removal of obj/item/weapon
	anchored = FALSE
	return ..() // Pick it up now that it's unanchored.

/obj/item/ducttape/afterattack(var/A, mob/user, flag, params) // CHOMPEdit - Removal of obj/item/weapon
