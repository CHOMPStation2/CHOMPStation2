...
/obj/item/melee/classic_baton/attack(mob/M as mob, mob/living/user as mob) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/melee/telebaton/attack_self(mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	on = !on
	if(on)
...
/obj/item/melee/telebaton/attack(mob/target as mob, mob/living/user as mob) // CHOMPEdit - Removal of obj/item/weapon
	if(on)
		if ((CLUMSY in user.mutations) && prob(50))
			to_chat(user, "<span class='warning'>You club yourself over the head.</span>")
			user.Weaken(3 * force)
