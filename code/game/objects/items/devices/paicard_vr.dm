...
/obj/item/paicard/attackby(var/obj/item/I as obj, mob/user as mob) // CHOMPEdit - Removal of obj/item/device
	if(istype(I,/obj/item/tool/screwdriver)) // CHOMPEdit - Removal of obj/item/weapon
		if(panel_open)
			panel_open = FALSE
			...
			if(do_after(user, 3 SECONDS))
				...
				playsound(src, 'sound/items/Screwdriver.ogg', 50, 1)
	if(istype(I,/obj/item/robotanalyzer)) // CHOMPEdit - Removal of obj/item/device
		if(!panel_open)
...
/obj/item/paicard/attack_self(mob/user) // CHOMPEdit - Removal of obj/item/device
	if(!panel_open)
		access_screen(user)
		return
	var/list/parts = list()
...
/obj/item/paicard/proc/death_damage() // CHOMPEdit - Removal of obj/item/device
...
/obj/item/paicard/proc/damage_random_component(nonfatal = FALSE) // CHOMPEdit - Removal of obj/item/device
...
/obj/item/paicard/proc/is_damage_critical() // CHOMPEdit - Removal of obj/item/device
