...
/obj/item/starcaster_news // CHOMPEdit - Removal of obj/item/device
...
/obj/item/starcaster_news/attack_self(mob/user as mob) // CHOMPEdit - Removal of obj/item/device

	user.set_machine(src)
	tgui_interact(user) //Activates tgui. Bless tgui.
	return

/obj/item/starcaster_news/tgui_interact(mob/user, datum/tgui/ui, datum/tgui/parent_ui) // CHOMPEdit - Removal of obj/item/device
	ui = SStgui.try_update_ui(user, src, ui)
	if(!ui)
		ui = new(user, src, "StarcasterCh", name) 
		ui.open()

/obj/item/starcaster_news/tgui_data(mob/user, datum/tgui/ui, datum/tgui_state/state) //Mostly ripped off from news_browser.dm // CHOMPEdit - Removal of obj/item/device
	var/list/data = list()
...
/obj/item/starcaster_news/tgui_act(action, list/params, datum/tgui/ui) //Also ripped from news_browser.dm. Bless tgui // CHOMPEdit - Removal of obj/item/device
	if(..())
		return TRUE
	switch(action)
		if("PRG_openarticle")
...
/obj/item/starcaster_news/Topic(href, href_list) //Mostly ripped off from news_browser.dm // CHOMPEdit - Removal of obj/item/device
	if(..())
		return 1
...
/obj/item/starcaster_news/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1) //Also ripped from news_browser.dm. Fuck nanoUI // CHOMPEdit - Removal of obj/item/device
	var/obj/item/starcaster_news/PRG = src //If you're trying to understand this shit go to nano/templates/starcaster_ch.tmpl // CHOMPEdit - Removal of obj/item/device
