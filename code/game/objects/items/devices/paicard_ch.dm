/obj/item/paicard/attackby(obj/item/W as obj, mob/user as mob) // CHOMPEdit - Removal of obj/item/weapon // CHOMPEdit - Removal of obj/item/device
	...
	if(ID)
		if (src.pai.idaccessible == 1)
			switch(alert(user, "Do you wish to add access to [src] or remove access from [src]?",,"Add Access","Remove Access", "Cancel"))
				...
				if("Cancel")
					return
		else if (istype(W, /obj/item/card/id) && src.pai.idaccessible == 0) // CHOMPEdit - Removal of obj/item/weapon
