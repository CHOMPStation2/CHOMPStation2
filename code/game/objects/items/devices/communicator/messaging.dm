...
/obj/item/communicator/receive_exonet_message(var/atom/origin_atom, origin_address, message, text) // CHOMPEdit - Removal of obj/item/device
	if(message == "voice")
		if(isobserver(origin_atom) || istype(origin_atom, /obj/item/communicator)) // CHOMPEdit - Removal of obj/item/device
...
/mob/observer/dead/receive_exonet_message(origin_atom, origin_address, message, text)
	if(message == "voice")
		if(istype(origin_atom, /obj/item/communicator)) // CHOMPEdit - Removal of obj/item/device
			var/obj/item/communicator/comm = origin_atom // CHOMPEdit - Removal of obj/item/device
...
/obj/item/communicator/proc/request_im(var/atom/candidate, var/origin_address, var/text) // CHOMPEdit - Removal of obj/item/device
	var/who = null
...
/obj/item/communicator/Topic(href, href_list) // CHOMPEdit - Removal of obj/item/device
	switch(href_list["action"])
		if("Reply")
			var/obj/item/communicator/comm = locate(href_list["target"]) // CHOMPEdit - Removal of obj/item/device
			...
			if(message)
				...
				to_chat(usr, "<span class='notice'>[icon2html(src,usr.client)] Sent message to [istype(comm, /obj/item/communicator) ? comm.owner : comm.name], <b>\"[message]\"</b> (<a href='?src=\ref[src];action=Reply;target=\ref[exonet.get_atom_from_address(comm.exonet.address)]'>Reply</a>)</span>") // CHOMPEdit - Removal of obj/item/device
