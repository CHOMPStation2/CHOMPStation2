...
var/global/list/obj/item/communicator/all_communicators = list() // CHOMPEdit - Removal of obj/item/device
...
/obj/item/communicator // CHOMPEdit - Removal of obj/item/device
	...
	var/obj/item/card/id/id = null //CHOMPADDITION: Making it possible to slot an ID card into the Communicator so it can function as both. // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/communicator/Initialize() // CHOMPEdit - Removal of obj/item/device
	. = ..()
...
//CHOMPADDITION START: Ayo communicator are better than PDAs /obj/item/communicator // CHOMPEdit - Removal of obj/item/device
// Proc: AltClick()
// Parameters: None
...
/obj/item/communicator/AltClick() // CHOMPEdit - Removal of obj/item/device
	if(issilicon(usr))
		return
...
/obj/item/communicator/GetAccess() // CHOMPEdit - Removal of obj/item/device
	if(id)
		return id.GetAccess()
	else
		return ..()
...
/obj/item/communicator/GetID() // CHOMPEdit - Removal of obj/item/device
	return id
...
/obj/item/communicator/proc/remove_id() // CHOMPEdit - Removal of obj/item/device
	if (id)
		if (ismob(loc))
			var/mob/M = loc
			M.put_in_hands(id)
...
//              returns 0.
/obj/item/communicator/proc/id_check(mob/user as mob, choice as num) // CHOMPEdit - Removal of obj/item/device
	if(choice == 1)
		if (id)
			remove_id()
			return 1
...
/obj/item/communicator/proc/register_to_holder() // CHOMPEdit - Removal of obj/item/device
	if(ismob(loc))
		register_device(loc.name)
		initialize_exonet(loc)
	else if(istype(loc, /obj/item/storage)) // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/storage/S = loc // CHOMPEdit - Removal of obj/item/weapon
		if(ismob(S.loc))
...
//				list.
/obj/item/communicator/proc/initialize_exonet(mob/user) // CHOMPEdit - Removal of obj/item/device
...
/obj/item/communicator/examine(mob/user) // CHOMPEdit - Removal of obj/item/device
	. = ..()
...
/obj/item/communicator/emp_act() // CHOMPEdit - Removal of obj/item/device
...
/obj/item/communicator/proc/add_to_EPv2(var/hex) // CHOMPEdit - Removal of obj/item/device
...
/obj/item/communicator/proc/populate_known_devices(mob/user) // CHOMPEdit - Removal of obj/item/device
	if(!exonet)
		exonet = new(src)
	src.known_devices.Cut()
	if(!get_connection_to_tcomms()) //If the network's down, we can't see anything.
		return
	for(var/obj/item/communicator/comm in all_communicators) // CHOMPEdit - Removal of obj/item/device
...
/obj/item/communicator/proc/get_connection_to_tcomms() // CHOMPEdit - Removal of obj/item/device
...
/obj/item/communicator/process() // CHOMPEdit - Removal of obj/item/device
	update_ticks++
	if(update_ticks % 5)
		if(!node)
			node = get_exonet_node()
...
/obj/item/communicator/attackby(obj/item/C as obj, mob/user as mob) // CHOMPEdit - Removal of obj/item/device
	..()
	if(istype(C, /obj/item/card/id)) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/communicator/attack_self(mob/user) // CHOMPEdit - Removal of obj/item/device
...
// Proc: MouseDrop()
//Same thing PDAs do
/obj/item/communicator/MouseDrop(obj/over_object as obj) // CHOMPEdit - Removal of obj/item/device
	var/mob/M = usr
...
/obj/item/communicator/attack_ghost(mob/user) // CHOMPEdit - Removal of obj/item/device
...
/obj/item/communicator/proc/register_device(new_name) // CHOMPEdit - Removal of obj/item/device
	if(!new_name)
		return
	owner = new_name
...
/obj/item/communicator/Destroy() // CHOMPEdit - Removal of obj/item/device
...
/obj/item/communicator/update_icon() // CHOMPEdit - Removal of obj/item/device
	if(video_source)
...
/obj/item/communicator/watch // CHOMPEdit - Removal of obj/item/device
...
/obj/item/communicator/watch/update_icon() // CHOMPEdit - Removal of obj/item/device
	if(video_source)
