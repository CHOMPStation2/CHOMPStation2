...
/obj/item/communicator/proc/setup_tgui_camera() // CHOMPEdit - Removal of obj/item/device
...
/obj/item/communicator/proc/update_active_camera_screen() // CHOMPEdit - Removal of obj/item/device
	...
	var/obj/item/communicator/communicator = video_source.loc // CHOMPEdit - Removal of obj/item/device
...
/obj/item/communicator/proc/show_static() // CHOMPEdit - Removal of obj/item/device
...
/obj/item/communicator/tgui_state(mob/user) // CHOMPEdit - Removal of obj/item/device
	return GLOB.tgui_inventory_state

// Proc: tgui_interact()
// Parameters: User, UI, Parent UI
// Description: This proc handles opening the UI. It's basically just a standard stub.
/obj/item/communicator/tgui_interact(mob/user, datum/tgui/ui, datum/tgui/parent_ui, datum/tgui_state/custom_state) // CHOMPEdit - Removal of obj/item/device
...
/obj/item/communicator/tgui_data(mob/user, datum/tgui/ui, datum/tgui_state/state) // CHOMPEdit - Removal of obj/item/device
	...
	for(var/obj/item/communicator/comm in known_devices) // CHOMPEdit - Removal of obj/item/device
	...
	for(var/obj/item/communicator/comm in voice_invites) // CHOMPEdit - Removal of obj/item/device
		if(comm.exonet)
	...
	for(var/obj/item/communicator/comm in voice_requests) // CHOMPEdit - Removal of obj/item/device
		if(comm.exonet)
	...
	for(var/obj/item/communicator/comm in im_contacts) // CHOMPEdit - Removal of obj/item/device
		if(comm.exonet)
...
/obj/item/communicator/tgui_static_data(mob/user, datum/tgui/ui, datum/tgui_state/state) // CHOMPEdit - Removal of obj/item/device
	var/list/data = ..()
	// Update manifest'
	if(data_core)
...
/obj/item/communicator/tgui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state) // CHOMPEdit - Removal of obj/item/device
	if(..())
		return TRUE
	...
	switch(action)
		...
		if("message")
			...
			if(text)
				...
				var/obj/item/communicator/comm = exonet.get_atom_from_address(their_address) // CHOMPEdit - Removal of obj/item/device
				to_chat(usr, "<span class='notice'>[icon2html(src, usr.client)] Sent message to [istype(comm, /obj/item/communicator) ? comm.owner : comm.name], <b>\"[text]\"</b> (<a href='?src=\ref[src];action=Reply;target=\ref[exonet.get_atom_from_address(comm.exonet.address)]'>Reply</a>)</span>") // CHOMPEdit - Removal of obj/item/device
		...
		if("startvideo")
			...
			var/obj/item/communicator/comm = locate(ref_to_video) // CHOMPEdit - Removal of obj/item/device
			if(comm)
