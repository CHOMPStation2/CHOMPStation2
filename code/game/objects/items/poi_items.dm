...
/obj/structure/closet/crate/oldreactor
	...
	starts_with = list(
		/obj/item/fuel_assembly/deuterium = 6) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/poi/broken_drone_circuit/attackby(obj/item/I as obj, mob/living/user as mob)
	...
	if(istype(I, /obj/item/tool/wirecutters)) // CHOMPEdit - Removal of obj/item/weapon
		wirecutted = !wirecutted
		to_chat(user, "You [wirecutted ? "cut" : "mend"] the power wire to the blackbox")

	if(istype(I, /obj/item/paper) && unscrewed) // CHOMPEdit - Removal of obj/item/weapon
		if(!has_paper)
