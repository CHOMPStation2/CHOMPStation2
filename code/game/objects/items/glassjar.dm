...
/obj/item/glass_jar/attackby(var/obj/item/W, var/mob/user)
	if(istype(W, /obj/item/spacecash)) // CHOMPEdit - Removal of obj/item/weapon
		...
		if(contains != JAR_MONEY)
			return
		var/obj/item/spacecash/S = W // CHOMPEdit - Removal of obj/item/weapon
		user.visible_message("<span class='notice'>[user] puts [S.worth] [S.worth > 1 ? "thalers" : "thaler"] into \the [src].</span>")
		user.drop_from_inventory(S)
		S.loc = src
		update_icon()
	//CHOMPDDITION: your god can not help you
	if(istype(W,/obj/item/holder/micro)) // CHOMPEdit - Removal of obj/item/weapon
		var/full = 0
		for(var/mob/M in src)
		...
		if(full >= 2)
			...
			var/obj/item/holder/micro/holder = W // CHOMPEdit - Removal of obj/item/weapon
