...
//	causeerrorheresoifixthis
	...
	var/obj/item/uplink/hidden/hidden_uplink = null // All items can have an uplink hidden inside, just remember to add the triggers. // CHOMPEdit - Removal of obj/item/device
...
/obj/item/attack_hand(mob/living/user as mob)
	...
	if (hasorgans(user))
		...
		if(!temp)
			...
			return
	if(istype(src, /obj/item/holder)) // CHOMPEdit - Removal of obj/item/weapon
		var/obj/item/holder/D = src // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/attack_ai(mob/user as mob)
	if (istype(src.loc, /obj/item/robot_module)) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/proc/on_exit_storage(obj/item/storage/S as obj) // CHOMPEdit - Removal of obj/item/weapon
	return

// called when this item is added into a storage item, which is passed on as S. The loc variable is already set to the storage item.
/obj/item/proc/on_enter_storage(obj/item/storage/S as obj) // CHOMPEdit - Removal of obj/item/weapon
	return
	...
	switch(slot)
		...
		if(slot_s_store)
			...
			if(!H.wear_suit.allowed)
				...
				return 0
			if( !(istype(src, /obj/item/pda) || istype(src, /obj/item/pen) || is_type_in_list(src, H.wear_suit.allowed)) ) // CHOMPEdit - Removal of obj/item/weapon // CHOMPEdit - Removal of obj/item/device
				return 0
		if(slot_legcuffed) //Going to put this check above the handcuff check because the survival of the universe depends on it.
			if(!istype(src, /obj/item/handcuffs/legcuffs)) //Putting it here might actually do nothing. // CHOMPEdit - Removal of obj/item/weapon
				return 0
		if(slot_handcuffed)
			if(!istype(src, /obj/item/handcuffs) || istype(src, /obj/item/handcuffs/legcuffs)) //Legcuffs are a child of handcuffs, but we don't want to use legcuffs as handcuffs... // CHOMPEdit - Removal of obj/item/weapon
				return 0 //In theory, this would never happen, but let's just do the legcuff check anyways.
		if(slot_in_backpack) //used entirely for equipping spawned mobs or at round start
			var/allow = 0
			if(H.back && istype(H.back, /obj/item/storage/backpack)) // CHOMPEdit - Removal of obj/item/weapon
				var/obj/item/storage/backpack/B = H.back // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/add_blood(mob/living/carbon/human/M as mob)
	...
	if(istype(src, /obj/item/melee/energy)) // CHOMPEdit - Removal of obj/item/weapon
		return
...
/obj/item/proc/on_holder_escape(var/obj/item/holder/H) // CHOMPEdit - Removal of obj/item/weapon
	return
