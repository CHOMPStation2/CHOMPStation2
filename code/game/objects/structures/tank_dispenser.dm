...
/obj/structure/dispenser/Initialize()
	...
	for(var/i in 1 to oxygentanks)
		new /obj/item/tank/oxygen(src) // CHOMPEdit - Removal of obj/item/weapon
	for(var/i in 1 to phorontanks)
		new /obj/item/tank/phoron(src) // CHOMPEdit - Removal of obj/item/weapon
	update_icon()
...
/obj/structure/dispenser/attackby(obj/item/I, mob/user)
	var/full
	if(istype(I, /obj/item/tank/oxygen) || istype(I, /obj/item/tank/air) || istype(I, /obj/item/tank/anesthetic)) // CHOMPEdit - Removal of obj/item/weapon
		if(oxygentanks < TANK_DISPENSER_CAPACITY)
			oxygentanks++
		else
			full = TRUE
	else if(istype(I, /obj/item/tank/phoron)) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/structure/dispenser/tgui_act(action, params)
	...
	switch(action)
		if("plasma")
			var/obj/item/tank/phoron/tank = locate() in src // CHOMPEdit - Removal of obj/item/weapon
		...
		if("oxygen")
			...
			for(var/obj/item/tank/T in src) // CHOMPEdit - Removal of obj/item/weapon
				if(istype(T, /obj/item/tank/oxygen) || istype(T, /obj/item/tank/air) || istype(T, /obj/item/tank/anesthetic)) // CHOMPEdit - Removal of obj/item/weapon
					tank = T
					break
			if(tank && Adjacent(usr))
				usr.put_in_hands(tank)
