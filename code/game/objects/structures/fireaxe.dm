...
/obj/structure/fireaxecabinet
	...
	var/obj/item/material/twohanded/fireaxe/fireaxe // CHOMPEdit - Removal of obj/item/weapon
...
/obj/structure/fireaxecabinet/Initialize()
	. = ..()
	if(starts_with_axe)
		fireaxe = new /obj/item/material/twohanded/fireaxe() // CHOMPEdit - Removal of obj/item/weapon
	update_icon()
	...
	if (isrobot(user) || locked)
		if(istype(O, /obj/item/multitool)) // CHOMPEdit - Removal of obj/item/device
			...
			if(do_after(user, 20 * O.toolspeed))
				...
				update_icon()
			return
		else if(istype(O, /obj/item)) // CHOMPEdit - Removal of obj/item/weapon
			var/obj/item/W = O // CHOMPEdit - Removal of obj/item/weapon
			if(smashed || open)
				if(open)
					toggle_close_open()
				return
			...
			update_icon()
		return
	if (istype(O, /obj/item/material/twohanded/fireaxe) && open) // CHOMPEdit - Removal of obj/item/weapon
		if(!fireaxe)
			if(O:wielded)
				O:wielded = 0
				O.update_icon()
