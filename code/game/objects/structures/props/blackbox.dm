...
/datum/category_item/catalogue/information/blackbox/snowfield_base
	...
	desc = {"
		...
		Nor we, exactly.<BR>
		But! Since it is also causing a mess on our side, we tried to be in the 'accurate time' as possible to stop this keep on going.<BR>
		Juuust enough to stop this time paradox. Maybe. Someone got to stop it, you know.<BR>
		Hey, don't blame us, by the way. Passing through time is near impossible even for us, and it is a chore to do so.<BR>
		But, some dumb-ass has dropped through time and causing a crack on our clock, while this gateway got 'something.'<BR>
