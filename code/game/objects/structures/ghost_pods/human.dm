...
/obj/structure/ghost_pod/ghost_activated/human/proc/handle_clothing_setup()
	...
	clothing_possibilities |= /obj/item/radio/headset // CHOMPEdit - Removal of obj/item/device
...
/obj/structure/ghost_pod/ghost_activated/human/create_occupant(var/mob/M)
		...
		for(var/path in clothing_possibilities)
			...
			if(ispath(path, /obj/item/clothing/head))
				if(!head_options)
					head_options = list()
				head_options |= path
			if(ispath(path, /obj/item/radio/headset)) // CHOMPEdit - Removal of obj/item/device
				if(!headset_options)
	...
	if(spawn_with_emag)
		var/obj/item/card/emag/E = new(H) // CHOMPEdit - Removal of obj/item/weapon
		E.name = "broken card"
