...
/datum/antagonist/cultist/equip(var/mob/living/carbon/human/player)
	...
	var/obj/item/paper/talisman/supply/T = new(get_turf(player)) // CHOMPEdit - Removal of obj/item/weapon
	var/list/slots = list (
	...
	for(var/slot in slots)
		...
		if(T.loc == player)
			break
	var/obj/item/storage/S = locate() in player.contents // CHOMPEdit - Removal of obj/item/weapon
	if(S && istype(S))
		T.loc = S
...
/datum/antagonist/cultist/can_become_antag(var/datum/mind/player)
	if(!..())
		return 0
	for(var/obj/item/implant/loyalty/L in player.current) // CHOMPEdit - Removal of obj/item/weapon
