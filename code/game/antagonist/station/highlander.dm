...
/datum/antagonist/highlander/equip(var/mob/living/carbon/human/player)
	...
	player.equip_to_slot_or_del(new /obj/item/material/sword(player), slot_l_hand) // CHOMPEdit - Removal of obj/item/weapon
	player.equip_to_slot_or_del(new /obj/item/clothing/shoes/boots/combat(player), slot_shoes)
	player.equip_to_slot_or_del(new /obj/item/pinpointer(get_turf(player)), slot_l_store) // CHOMPEdit - Removal of obj/item/weapon

	var/obj/item/card/id/id = create_id("Highlander", player) // CHOMPEdit - Removal of obj/item/weapon
	if(id)
