			...
			switch(href_list["hsb"])
				...
				if("hsbsuit")
					...
					if(P.back)
						...
						P.back = null
					P.back = new/obj/item/tank/jetpack(P) // CHOMPEdit - Removal of obj/item/weapon
					P.back.hud_layerise()
					P.internal = P.back
				if("hsbmetal")
				...
				if("hsbwatertank")
					...
					//hsb.loc = usr.loc
				if("hsbtoolbox")
					var/obj/item/storage/hsb = new/obj/item/storage/toolbox/mechanical // CHOMPEdit - Removal of obj/item/weapon
					for(var/obj/item/radio/T in hsb) // CHOMPEdit - Removal of obj/item/device
						qdel(T)
					new/obj/item/tool/crowbar (hsb) // CHOMPEdit - Removal of obj/item/weapon
					hsb.loc = usr.loc
				if("hsbmedkit")
					var/obj/item/storage/firstaid/hsb = new/obj/item/storage/firstaid/regular // CHOMPEdit - Removal of obj/item/weapon
					hsb.loc = usr.loc
				if("hsbobj")
					...
					//Note, these istypes don't work
						if(istype(O, /obj/item/gun)) // CHOMPEdit - Removal of obj/item/weapon
							continue
						if(istype(O, /obj/item/assembly))
							continue
						if(istype(O, /obj/item/camera)) // CHOMPEdit - Removal of obj/item/device
							continue
						if(istype(O, /obj/item/dummy)) // CHOMPEdit - Removal of obj/item/weapon
							continue
						if(istype(O, /obj/item/melee/energy/sword)) // CHOMPEdit - Removal of obj/item/weapon
							continue
