...
/obj/effect/rune/proc/tomesummon()
	...
	if(istype(src,/obj/effect/rune))
		...
		new /obj/item/book/tome(usr.loc) // CHOMPEdit - Removal of obj/item/weapon
	qdel(src)
	return
...
/obj/effect/rune/proc/manifest()
	...
	D.real_name = "Unknown"
	var/chose_name = 0
	for(var/obj/item/paper/P in this_rune.loc) // CHOMPEdit - Removal of obj/item/weapon
		if(P.info)
...
/obj/effect/rune/proc/talisman()//only hide, emp, teleport, deafen, blind and tome runes can be imbued atm
	var/obj/item/paper/newtalisman // CHOMPEdit - Removal of obj/item/weapon
	var/unsuitable_newtalisman = 0
	for(var/obj/item/paper/P in src.loc) // CHOMPEdit - Removal of obj/item/weapon
		if(!P.info)
			newtalisman = P
			break
		else
	...
	var/obj/item/paper/talisman/T // CHOMPEdit - Removal of obj/item/weapon
...
/obj/effect/rune/proc/sacrifice()
	...
	for(var/obj/item/I in src.loc)//Checks for MMIs/brains/Intellicards
		if(istype(I,/obj/item/organ/internal/brain))
			...
			victims += B.brainmob
		else if(istype(I,/obj/item/mmi)) // CHOMPEdit - Removal of obj/item/device
			var/obj/item/mmi/B = I // CHOMPEdit - Removal of obj/item/device
			victims += B.brainmob
		else if(istype(I,/obj/item/aicard)) // CHOMPEdit - Removal of obj/item/device
...
/obj/effect/rune/proc/revealrunes(var/obj/W as obj)
	...
	if(istype(W,/obj/effect/rune))
		rad = 6
		go = 1
	if (istype(W,/obj/item/paper/talisman)) // CHOMPEdit - Removal of obj/item/weapon
		rad = 4
		go = 1
	if (istype(W,/obj/item/nullrod)) // CHOMPEdit - Removal of obj/item/weapon
		rad = 1
		go = 1
	if(go)
		for(var/obj/effect/rune/R in orange(rad,src))
...
/obj/effect/rune/proc/deafen()
	if(istype(src,/obj/effect/rune))
		...
		for(var/mob/living/carbon/C in range(7,src))
			if (iscultist(C))
				continue
			var/obj/item/nullrod/N = locate() in C // CHOMPEdit - Removal of obj/item/weapon
			if(N)
				continue
			C.ear_deaf += 50
...
//			var/list/cultboil = list(cultists-usr) //and for this words are destroy-see-blood.
	...
	if(cultists.len>=3)
		for(var/mob/living/carbon/M in viewers(usr))
			if(iscultist(M))
				continue
			var/obj/item/nullrod/N = locate() in M // CHOMPEdit - Removal of obj/item/weapon
			if(N)
				continue
...
/obj/effect/rune/proc/runestun(var/mob/living/T as mob)
	if(istype(src,/obj/effect/rune))   ///When invoked as rune, flash and stun everyone around.
		...
		var/obj/item/nullrod/N = locate() in T // CHOMPEdit - Removal of obj/item/weapon
		if(N)
...
/obj/effect/rune/proc/armor()
	...
	user.equip_to_slot_or_del(new /obj/item/storage/backpack/cultpack(user), slot_back) // CHOMPEdit - Removal of obj/item/weapon
	//the above update their overlay icons cache but do not call update_icons()
	//the below calls update_icons() at the end, which will update overlay icons by using the (now updated) cache
	user.put_in_hands(new /obj/item/melee/cultblade(user))	//put in hands or on floor // CHOMPEdit - Removal of obj/item/weapon
