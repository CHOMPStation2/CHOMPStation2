...
/mob/proc/changeling_electric_lockpick()
	...
	if(held_item == null)
		if(changeling_generic_weapon(/obj/item/finger_lockpick,0,5))  //Chemical cost is handled in the equip proc. // CHOMPEdit - Removal of obj/item/weapon
			return 1
		return 0

/obj/item/finger_lockpick // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/finger_lockpick/New() // CHOMPEdit - Removal of obj/item/weapon
	if(ismob(loc))
		to_chat(loc, "<span class='notice'>We shape our finger to fit inside electronics, and are ready to force them open.</span>")

/obj/item/finger_lockpick/dropped(mob/user) // CHOMPEdit - Removal of obj/item/weapon
	to_chat(user, "<span class='notice'>We discreetly shape our finger back to a less suspicious form.</span>")
	spawn(1)
		if(src)
			qdel(src)

/obj/item/finger_lockpick/afterattack(var/atom/target, var/mob/living/user, proximity) // CHOMPEdit - Removal of obj/item/weapon
	if(!target)
		return
	if(!proximity)
		return
