...
/mob/living/carbon/human/proc/changeling_bioelectrogenesis()
	...
	if(held_item == null)
		if(src.mind.changeling.recursive_enhancement)
			if(changeling_generic_weapon(/obj/item/electric_hand/efficent,0)) // CHOMPEdit - Removal of obj/item/weapon
				to_chat(src, "<span class='notice'>We will shock others more efficently.</span>")
				return 1
		else
			if(changeling_generic_weapon(/obj/item/electric_hand,0))  //Chemical cost is handled in the equip proc. // CHOMPEdit - Removal of obj/item/weapon
				return 1
		return 0
		...
		if(istype(held_item,/obj/item/grab)) // CHOMPEdit - Removal of obj/item/weapon
			var/obj/item/grab/G = held_item // CHOMPEdit - Removal of obj/item/weapon
			if(G.affecting)
		...
		//Otherwise, charge up whatever's in their hand.
			...
			//Check our hand.
			if(istype(held_item,/obj/item/cell)) // CHOMPEdit - Removal of obj/item/weapon
				L.Add(held_item)

			//Now check our hand's item's contents, so we can recharge guns and other stuff.
			for(var/obj/item/cell/cell in held_item.contents) // CHOMPEdit - Removal of obj/item/weapon
				L.Add(cell)

			//Now for the actual recharging.
			for(var/obj/item/cell/cell in L) // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/electric_hand // CHOMPEdit - Removal of obj/item/weapon
...
/obj/item/electric_hand/efficent // CHOMPEdit - Removal of obj/item/weapon
	shock_cost = 5
	agony_amount = 80
	electrocute_amount = 20

/obj/item/electric_hand/New() // CHOMPEdit - Removal of obj/item/weapon
	if(ismob(loc))
...
/obj/item/electric_hand/dropped(mob/user) // CHOMPEdit - Removal of obj/item/weapon
	spawn(1)
		if(src)
			qdel(src)

/obj/item/electric_hand/afterattack(var/atom/target, var/mob/living/carbon/human/user, proximity) // CHOMPEdit - Removal of obj/item/weapon
	if(!target)
		return
	if(!proximity)
		return
	...
	if(istype(target,/mob/living/carbon))
		...
		if(istype(target,/obj/))
			...
			for(var/obj/item/cell/cell in T.contents) // CHOMPEdit - Removal of obj/item/weapon
