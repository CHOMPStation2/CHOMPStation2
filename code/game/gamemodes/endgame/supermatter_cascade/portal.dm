...
/obj/singularity/narsie/large/exit/consume(const/atom/A)
	...
	if (istype(A, /mob/living/))
		...
		if(L.buckled && istype(L.buckled,/obj/structure/bed/))
			...
			do_teleport(O, pick(GLOB.endgame_safespawns), local = FALSE) //VOREStation Edit // CHOMPEdit - Globals
			L.loc = O.loc
		else
			do_teleport(L, pick(GLOB.endgame_safespawns), local = FALSE) //dead-on precision //VOREStation Edit // CHOMPEdit - Globals

	else if (istype(A, /obj/mecha/))
		do_teleport(A, pick(GLOB.endgame_safespawns), local = FALSE) //dead-on precision //VOREStation Edit // CHOMPEdit - Globals
