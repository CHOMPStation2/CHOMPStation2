...
/datum/controller/occupations/proc/EquipRank(var/mob/living/carbon/human/H, var/rank, var/joined_late = 0, var/announce = TRUE)
	...
	if(job)
		...
		if(H.client && H.client.prefs && H.client.prefs.gear && H.client.prefs.gear.len && !(job.mob_type & JOB_SILICON))
			for(var/thing in H.client.prefs.gear)
				...
				if(G.slot == "implant")
					var/obj/item/implant/I = G.spawn_item(H, H.client.prefs.gear[G.display_name]) // CHOMPEdit - Removal of obj/item/weapon
					I.invisibility = 100
					I.implant_loadout(H)
					continue
	...
	if(H.mind)
		...
		if(spawn_in_storage && spawn_in_storage.len)
			var/obj/item/storage/B // CHOMPEdit - Removal of obj/item/weapon
			for(var/obj/item/storage/S in H.contents) // CHOMPEdit - Removal of obj/item/weapon
				B = S
				break
	...
	if(istype(H)) //give humans wheelchairs, if they need them.
		...
		var/obj/item/storage/S = locate() in H.contents // CHOMPEdit - Removal of obj/item/weapon
	...
	if(job.has_headset)
		H.equip_to_slot_or_del(new /obj/item/radio/headset(H), slot_l_ear) // CHOMPEdit - Removal of obj/item/device
