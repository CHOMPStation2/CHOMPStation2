...
/datum/controller/subsystem/supply/proc/forbidden_atoms_check(atom/A)
	if(isliving(A))
		return 1
	if(istype(A,/obj/item/disk/nuclear)) // CHOMPEdit - Removal of obj/item/weapon
		return 1
	if(istype(A,/obj/machinery/nuclearbomb))
		return 1
	if(istype(A,/obj/item/radio/beacon)) // CHOMPEdit - Removal of obj/item/device
		return 1
	if(istype(A,/obj/item/perfect_tele_beacon))	//VOREStation Addition: Translocator beacons // CHOMPEdit - Removal of obj/item/device
		...
		for(var/atom/movable/MA in subarea)
			...
			if(istype(MA,/obj/structure/closet/crate))
				...
				for(var/atom/A in CR)
					...
					// Sell manifests
					if(find_slip && istype(A,/obj/item/paper/manifest)) // CHOMPEdit - Removal of obj/item/weapon
						var/obj/item/paper/manifest/slip = A // CHOMPEdit - Removal of obj/item/weapon
					...
					//Sell spacebucks
					if(istype(A, /obj/item/spacecash)) // CHOMPEdit - Removal of obj/item/weapon
						var/obj/item/spacecash/cashmoney = A // CHOMPEdit - Removal of obj/item/weapon
...
/datum/controller/subsystem/supply/proc/buy()
	...
	for(var/datum/supply_order/SO in shoppinglist)
		...
		var/obj/item/paper/manifest/slip // CHOMPEdit - Removal of obj/item/weapon
		if(!SP.contraband)
			if(A)
				slip = new /obj/item/paper/manifest(A) // CHOMPEdit - Removal of obj/item/weapon
			else
				slip = new /obj/item/paper/manifest(pickedloc) // CHOMPEdit - Removal of obj/item/weapon
			slip.is_copy = 0
