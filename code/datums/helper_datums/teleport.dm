var/bluespace_item_types = newlist(/obj/item/storage/backpack/holding, // CHOMPEdit - Removal of obj/item/weapon
...
/datum/teleport/instant/science/teleportChecks()
	if(istype(teleatom, /obj/item/disk/nuclear)) // Don't let nuke disks get teleported --NeoFite // CHOMPEdit - Removal of obj/item/weapon
		teleatom.visible_message("<span class='danger'>\The [teleatom] bounces off of the portal!</span>")
		return 0

	if(!isemptylist(teleatom.search_contents_for(/obj/item/disk/nuclear))) // CHOMPEdit - Removal of obj/item/weapon
		if(istype(teleatom, /mob/living))
			var/mob/living/MM = teleatom
	...
	if(destination.z in using_map.admin_levels) //CentCom z-level
		if(istype(teleatom, /obj/mecha))
			...
			return 0
		if(!isemptylist(teleatom.search_contents_for(/obj/item/storage/backpack/holding))) // CHOMPEdit - Removal of obj/item/weapon
			teleatom.visible_message("<span class='danger'>\The [teleatom] bounces off of the portal!</span>")
			return 0
