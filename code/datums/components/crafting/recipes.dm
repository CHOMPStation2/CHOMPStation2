...
/datum/crafting_recipe/spear/on_craft_completion(mob/user, atom/result)
	var/obj/item/material/M // CHOMPEdit - Removal of obj/item/weapon
	for(var/path in parts)
		var/obj/item/material/N = locate(path) in result // CHOMPEdit - Removal of obj/item/weapon
		if(istype(N, path))
			if(!istype(M))
				M = N
			else
	...
	var/obj/item/material/twohanded/spear/S = result // CHOMPEdit - Removal of obj/item/weapon
